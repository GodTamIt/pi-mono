...
import type {
  ...
  AgentToolRuntime,
  AgentToolSettings,
} from "../../src/tools/agent-tool.ts";
import { makeModel } from "./make-model.ts";
import { createTestSubagent } from "./make-subagent.ts";
import { STUB_SNAPSHOT } from "./stub-ctx.ts";
import { TEST_AGENTS } from "./test-agents.ts";

/** Minimal registry with no user agents — sufficient for tool tests that don't exercise agent-type lookup. */
const defaultRegistry = new AgentTypeRegistry(() => TEST_AGENTS);
