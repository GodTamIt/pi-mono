...
import type { BackgroundManagerDeps } from "../../src/tools/background-spawner.ts";
import type { ForegroundManagerDeps } from "../../src/tools/foreground-runner.ts";
import { createToolDeps } from "./make-deps.ts";
...
describe("createToolDeps", () => {
  describe("manager defaults", () => {
    it("spawn returns 'agent-1'", () => {
      ...
      expect(
        manager.spawn(STUB_SNAPSHOT, "general-purpose", "prompt", { description: "test" }),
      ).toBe("agent-1");
    });

    it("spawnAndWait resolves to a completed record", async () => {
      const { manager } = createToolDeps();
      const record = await manager.spawnAndWait(STUB_SNAPSHOT, "general-purpose", "prompt", {
        description: "test",
      });
