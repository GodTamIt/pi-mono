...
import type { EnvInfo } from "../../src/session/env.ts";
...
describe("buildAgentPrompt", () => {
  ...
  it("Architect prompt is read-only", () => {
    const config = getDefaultConfig("Architect");
    const prompt = buildAgentPrompt(config, "/workspace", env);
    expect(prompt).toContain("READ-ONLY");
  ...
  it("replace mode includes parent prompt as base (no bridge/wrapper)", () => {
    ...
    const prompt = buildAgentPrompt(config, "/workspace", env, {
      systemPrompt: "PARENT parent prompt content",
      cwd: PARENT_CWD,
    });
  ...
  it("replace mode orders: identity → active_agent → env → config.systemPrompt", () => {
    ...
    const prompt = buildAgentPrompt(config, "/workspace", env, {
      systemPrompt: "IDENTITY parent content",
      cwd: PARENT_CWD,
    });
  ...
  describe("active_agent tag injection", () => {
    ...
    it("includes <active_agent name=...> tag in append mode after sub_agent_context", () => {
      ...
      const prompt = buildAgentPrompt(config, "/workspace", env, {
        systemPrompt: "Parent prompt content.",
        cwd: PARENT_CWD,
      });
    ...
    it("active_agent tag appears before envBlock in both modes", () => {
      ...
      const appendPrompt = buildAgentPrompt(appendConfig, "/workspace", env, {
        systemPrompt: "Parent.",
        cwd: PARENT_CWD,
      });
  ...
  describe("inherited working-directory footer", () => {
    ...
    it("leaves the rest of the inherited prompt intact", () => {
      ...
      expect(
        prompt.startsWith("You are a parent coding agent.\nCurrent date: 2026-07-25\n\n"),
      ).toBe(true);
    });
