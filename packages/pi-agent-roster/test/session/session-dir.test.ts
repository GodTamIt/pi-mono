...
import { deriveSubagentSessionDir } from "../../src/session/session-dir.ts";
...
describe("deriveSubagentSessionDir", () => {
  ...
  it("returns a temp directory when parentSessionFile is undefined", () => {
    ...
    // Should contain pi-agent-roster prefix, encoded cwd, and end with tasks
    expect(result).toMatch(/pi-agent-roster/);
