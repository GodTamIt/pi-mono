...
} from "../../src/lifecycle/subagent-manager.ts";
...
import { NotificationManager } from "../../src/observation/notification.ts";
import type { RunConfig } from "../../src/runtime.ts";
import type { Subagent } from "../../src/types.ts";
import { createBlockingFactory, createSessionFactory } from "../helpers/manager-stubs.ts";
import {
  createMockSession,
...
describe("SubagentManager — registerWorkspaceProvider", () => {
  ...
  it("throws when a provider is already registered", () => {
    ...
    expect(() => manager.registerWorkspaceProvider(makeProvider())).toThrow(/already registered/i);
  });
