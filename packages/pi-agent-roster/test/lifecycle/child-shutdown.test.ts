...
} from "../../src/lifecycle/child-shutdown.ts";

type ShutdownSessionOverrides = {
  [K in keyof ShutdownCapableSession]?: ShutdownCapableSession[K] | undefined;
};

/** A child session whose runner records what it was asked to emit. */
function createShutdownCapableSession(overrides: ShutdownSessionOverrides = {}) {
  const emit = vi.fn(
    (_event: { type: "session_shutdown"; reason: "quit" }): Promise<unknown> =>
      Promise.resolve(undefined),
  );
  const session: ShutdownCapableSession = {
    ...("hasExtensionHandlers" in overrides
      ? overrides.hasExtensionHandlers
        ? { hasExtensionHandlers: overrides.hasExtensionHandlers }
        : {}
      : { hasExtensionHandlers: vi.fn((_eventType: string): boolean => true) }),
    ...("extensionRunner" in overrides
      ? overrides.extensionRunner
