...
import { TranscriptContent } from "../../src/ui/transcript-content.ts";
...
describe("TranscriptContent", () => {
      ...
      const messages = [
        {
          role: "user",
          content:
            '<skill name="testing" location="/skills/testing">\nskill body\n</skill>\n\nrun the suite',
        },
      ] as unknown as SessionMessage[];
  ...
  describe("user-message separation", () => {
    ...
    it("costs exactly one extra row when a user message follows other content", () => {
      ...
      const together = allRows(
        contentFrom([assistantText("an answer"), userMessage("a question")]),
      ).length;
  ...
  describe("width handling", () => {
    ...
    it("truncates every row to the requested width", () => {
      const messages = [{ role: "user", content: "x".repeat(500) }] as unknown as SessionMessage[];
  ...
  describe("incremental settling", () => {
    ...
    it("keeps consumed messages aligned across a skipped custom message", () => {
      ...
      history.push({
        role: "user",
        content: "after the custom message",
      } as unknown as SessionMessage);
      content.apply(settled());
