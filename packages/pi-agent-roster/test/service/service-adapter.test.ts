...
import type { WorkspaceProvider } from "../../src/lifecycle/workspace.ts";
import type { SubagentsService } from "../../src/service/service.ts";
import type { ServiceRuntimeLike, SubagentManagerLike } from "../../src/service/service-adapter.ts";
import { SubagentsServiceAdapter, toSubagentRecord } from "../../src/service/service-adapter.ts";
import type { SessionContext, Subagent } from "../../src/types.ts";
...
describe("SubagentsServiceAdapter — getRecord and listAgents", () => {
  ...
  function createService(records: Subagent[]): SubagentsService {
    ...
    manager.listAgents.mockImplementation(() =>
      [...records].sort((a, b) => b.startedAt - a.startedAt),
    );
    return new SubagentsServiceAdapter(manager, () => makeModel({ id: "test" }), makeRuntimeStub());
  }
...
describe("SubagentsServiceAdapter — spawn", () => {
  ...
  it("spawns as foreground when options.foreground is true", () => {
    ...
    svc.spawn("Architect", "design work", { foreground: true });
    expect(mgr.spawn).toHaveBeenCalledWith(
      expect.anything(), // snapshot
      "Architect",
      "design work",
  ...
  describe("steer", () => {
    ...
    it("delegates to session.steer and returns true when session is ready", async () => {
      ...
      record.subagentSession = toSubagentSession(
        createSubagentSessionStub(createMockSession({ steer: mockSteer })),
      );
      const mgr = createManagerStub();
      mgr.getRecord.mockReturnValue(record);
      const svc = createSvc(mgr);
