...
import { loadSettings, persistToastFor, SettingsManager, saveSettings } from "../src/settings.ts";
import { captureWarn } from "./helpers/capture-warn.ts";
import { createSettingsDirs, type SettingsDirs } from "./helpers/tmp-settings-dirs.ts";
 ...
 *   Simulates the user's project root. Settings live at `<projectDir>/.pi/agent-roster.json`.
 */
describe("settings persistence", () => {
  ...
  beforeEach(() => {
    dirs = createSettingsDirs("agent-roster.json");
...
describe("SettingsManager", () => {
  ...
  describe("load()", () => {
    ...
    it("applies merged settings from disk to in-memory values", () => {
      mkdirSync(join(projectDir, ".pi"), { recursive: true });
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({ graceTurns: 7, maxConcurrent: 8 }),
      );
      const emit = vi.fn();
    ...
    it("applies defaultMaxTurns from disk (0 → unlimited)", () => {
      ...
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({ defaultMaxTurns: 0 }),
      );
    ...
    it("applies defaultMaxTurns: 50 from disk", () => {
      ...
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({ defaultMaxTurns: 50 }),
      );
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
    ...
    it("applies retention windows from disk", () => {
      ...
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({
          consumedSessionRetentionMinutes: 20,
    ...
    it("applies abortAllOnInterrupt: false from disk", () => {
      ...
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({ abortAllOnInterrupt: false }),
      );
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
      sm.load();
    ...
    it("returns the loaded settings object", () => {
      ...
      writeFileSync(
        join(projectDir, ".pi", "agent-roster.json"),
        JSON.stringify({ maxConcurrent: 6 }),
      );
      const sm = new SettingsManager({ emit: vi.fn(), cwd: projectDir, agentDir: globalDir });
    ...
    it("loads excluded extension package sources, and clears them when removed", () => {
      mkdirSync(join(projectDir, ".pi"), { recursive: true });
      const settingsPath = join(projectDir, ".pi", "agent-roster.json");
      writeFileSync(
        settingsPath,
  ...
  describe("saveAndNotify()", () => {
    ...
    it("persists snapshot to disk and returns info toast on success", () => {
      ...
      const written = JSON.parse(
        readFileSync(join(projectDir, ".pi", "agent-roster.json"), "utf-8"),
      );
      expect(written).toEqual({
        maxConcurrent: 5,
        defaultMaxTurns: 0,
  ...
  describe("constructor onMaxConcurrentChanged callback", () => {
    it("constructs without callback without throwing", () => {
      expect(
        () => new SettingsManager({ emit: vi.fn(), cwd: "/tmp", agentDir: "/nonexistent" }),
      ).not.toThrow();
    });
