...
export class TranscriptContent {
  ...
  private consumeSettled(): void {
    ...
    for (let i = this.consumedCount; i < messages.length; i++) {
      const message = messages[i];
      if (message) this.consumeMessage(message);
    }
  ...
  private consumeMessage(message: SessionMessage): void {
    switch (message.role) {
      ...
      case "compactionSummary":
        this.consumeSummary(
          new CompactionSummaryMessageComponent(message, this.options.markdownTheme),
        );
        break;
      case "branchSummary":
  ...
  private consumeAssistant(message: AssistantSessionMessage): void {
    ...
    block.container.addChild(
      new AssistantMessageComponent(message, false, this.options.markdownTheme),
    );
...
function addUserComponents(
  container: Container,
  content: string | readonly { type: string; text?: string }[] | undefined,
  markdownTheme: MarkdownTheme,
