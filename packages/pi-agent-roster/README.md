...
```sh
pi install npm:pi-agent-roster@0.3.1
```

For a single run without changing Pi's saved package list:

```sh
pi -e npm:pi-agent-roster@0.3.1
```
...
/stack fast                # set the active primary's stack directly
/stack lead fast           # set an agent's session-local override directly
...
With a roster primary active, `/stack` opens that primary's stack picker directly, and `/stack <stack>` selects one of its stacks without a picker. `/stack <agent>` still opens that agent's picker when the name is not also an active-primary stack. Without an active roster primary, select an agent with `/agent` or use `/stack <agent> <stack>`. Command completion includes direct active-primary stacks and the compatible agent-first forms. When a roster primary is active, stack changes are restricted to that primary because child stack overrides are disabled.
...
- `--stack <name>` requires a non-default `--agent`; it cannot select a stack by itself. It remains authoritative across session replacement and extension reload, ahead of `/stack` overrides and configured defaults.
...
Use `/agent` for the interactive profile picker, `/agent <name>` for direct selection, and `/agent default` to restore the model, thinking level, prompt, and tools captured at session start. The unified one-line footer shows a compact, truncated summary of the current parent prompt, the selected primary display name and effective stack when present, and active background counts such as `agents: N running, M queued`. Restoring Pi's default removes the agent and stack segments but leaves task and count segments as applicable. The stack name is the effective resolved profile, including `default` when the synthetic fallback is in use. Resetting does not switch or fork the Pi session. Profile and stack changes wait until the parent is idle.
