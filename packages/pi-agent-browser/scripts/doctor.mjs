...
Checks:
  ...
  2. agent-browser --version is compared with the recommended baseline (advisory only).
  3. pi --version is at least the minimum Pi runtime version for this release.
  4. Pi settings and repo-local autoload locations do not point at multiple active pi-agent-browser sources.
...
async function checkAgentBrowserVersion({ runAgentBrowser }) {
	try {
		...
		if (!parseVersionParts(version)) {
			return { status: "fail", title: "agent-browser --version returned an empty or malformed value.", lines: [] };
		}
		return version === RECOMMENDED_VERSION
			? { status: "pass", title: `agent-browser version matches recommended baseline: ${version}`, lines: [] }
			: {
				status: "warn",
				title: `agent-browser ${version} differs from recommended ${RECOMMENDED_VERSION}.`,
				lines: [`Version drift is advisory and does not block browser calls. Maintainers verify the exact command reference against ${TARGET_AGENT_BROWSER_SOURCE} and ${CAPABILITY_BASELINE_SOURCE}.`],
			};
	} catch (error) {
		const code = error && typeof error === "object" ? error.code : undefined;
		return {
...
async function checkPiSources({ cwd, agentDir, settingsPaths, readText, pathExists }) {
	...
	if (sources.length > 1) {
		return {
			status: "fail",
			title: "Duplicate pi-agent-browser sources detected.",
			lines: [
	...
	if (sources.length === 1) {
		return {
			status: "pass",
			title: "No duplicate pi-agent-browser sources detected.",
...
export function formatDoctorReport(report) {
	const lines = ["pi-agent-browser doctor", ""];
