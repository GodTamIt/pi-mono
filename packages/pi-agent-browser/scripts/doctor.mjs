...
function parseSettingsText(text, _path) {
...
async function checkAgentBrowserVersion({ runAgentBrowser }) {
  try {
    ...
    if (!parseVersionParts(version)) {
      return {
        status: "fail",
        title: "agent-browser --version returned an empty or malformed value.",
        lines: [],
      };
    }
    return version === RECOMMENDED_VERSION
      ? {
          status: "pass",
          title: `agent-browser version matches recommended baseline: ${version}`,
          lines: [],
        }
      : {
          status: "warn",
          ...
          lines: [
            `Version drift is advisory and does not block browser calls. Maintainers verify the exact command reference against ${TARGET_AGENT_BROWSER_SOURCE} and ${CAPABILITY_BASELINE_SOURCE}.`,
          ],
        };
  } catch (error) {
    const code = error && typeof error === "object" ? error.code : undefined;
