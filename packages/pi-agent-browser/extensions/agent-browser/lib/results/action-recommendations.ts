...
export function buildAgentBrowserNextActions(options: {
  ...
  if (options.electron?.launchId) {
    ...
    if (options.resultCategory === "success" && status !== "cleaned") {
      ...
      if (sessionName) {
        actions.push(
          buildNextToolAction({
            ...
            reason:
              "Inspect attached Electron page/webview targets before choosing the active tab.",
          }),
          buildNextToolAction({
      ...
      options.resultCategory === "failure" &&
      options.failureCategory === "cleanup-failed"
    ) {
      actions.push(
        buildElectronToolAction({
          action: "status",
  ...
  if (options.resultCategory === "success") {
    ...
    const savedFileArtifact = options.savedFilePath
      ? artifacts.find((artifact) => artifact.path === options.savedFilePath)
      : undefined;
    ...
    switch (options.failureCategory) {
        ...
        if (options.confirmationId) {
          actions.push(
            buildNextToolAction({
              ...
              reason:
                "Approve the pending upstream confirmation when the requested action is safe.",
      ...
      case "download-not-verified":
        {
          ...
          actions.push(
            buildNextToolAction({
              ...
              id: "wait-for-download",
              reason:
                "Wait for the browser download and let the wrapper verify saved-file metadata.",
              safety:
                "Use an explicit wait timeout; if you set top-level timeoutMs, keep it above the wait duration plus a small grace window.",
            }),
          );
      ...
      case "timeout":
        {
          const textAssertion =
            options.command === "wait" && options.args?.includes("--text") === true;
          const urlAssertion =
            options.command === "wait" && options.args?.includes("--url") === true;
          actions.push(
            buildNextToolAction({
              args: ["snapshot", "-i"],
        ...
        if (isOpenNavigationCommand(options.command)) {
          actions.push(
            buildNextToolAction({
              ...
              reason:
                "Check which page, if any, remains active after the navigation or network error.",
              safety:
