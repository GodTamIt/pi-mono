...
export const SESSION_ARTIFACT_MANIFEST_MAX_ENTRIES_ENV =
...
export function mergeSessionArtifactManifest(options: {
  ...
  const entries = [...byPath.values()]
    .sort((left, right) => {
      ...
      return (
        rightTime - leftTime ||
        Number(isPendingRecordingCommand(right.command, right.subcommand, right.kind)) -
          Number(isPendingRecordingCommand(left.command, left.subcommand, left.kind)) ||
