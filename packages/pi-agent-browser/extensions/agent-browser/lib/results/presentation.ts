...
export async function buildToolPresentation(options: {
      ...
      content: [
        {
          ...
          text:
            artifactText ??
            formatPresentationContentText(commandInfoWithTokens, data, compiledSemanticAction),
        },
      ],
  ...
  const presentationWithImage = imagePath
    ? await attachInlineImage(presentation, imagePath)
    : presentation;
