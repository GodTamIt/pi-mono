...
function looksLikeBrowserProfileConfigError(errorText: string): boolean {
  return /\b(?:No Chrome user data directory found|Cannot resolve profile name|Chrome user data directory|Chrome profile\s+.+?\s+not found|Available profiles|If you meant a directory path)\b/i.test(
    errorText,
  );
}
  ...
  if (commandInfo.command !== "profiles") {
    actions.push(
      buildNextToolAction({
        ...
        reason:
          "List browser profiles/user-data-dir candidates before retrying profile-based launch.",
