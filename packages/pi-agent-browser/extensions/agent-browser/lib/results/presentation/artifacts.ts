...
async function buildFileArtifactMetadata(options: {
  ...
  const pendingRecording = isPendingRecordingCommand(
    options.commandInfo.command,
    options.commandInfo.subcommand,
    kind,
  );
...
export async function extractFileArtifacts(options: {
  ...
  const currentArtifacts = (
    await Promise.all(candidates.map((path) => buildFileArtifactMetadata({ ...options, path })))
  ).filter((artifact): artifact is FileArtifactMetadata => artifact !== undefined);
  const previousRestartRecordingArtifact = await buildPreviousRestartRecordingArtifact({
  ...
  switch (artifact.kind) {
    ...
    case "video":
      ...
      return artifact.subcommand === "restart"
        ...
        : "Recording started in a fresh active page; output will be written on stop";
  }
}
  ...
  return artifacts.map((artifact, index) => {
    if (isPendingRecordingArtifact(artifact)) {
      return [
        ...
        artifact.subcommand === "start"
          ? "Page state: record start uses a fresh active page for video capture; prior in-page DOM and JavaScript state does not carry over. Take a fresh snapshot before continuing."
          : undefined,
