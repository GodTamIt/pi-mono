...
const NETWORK_FILTER_OPAQUE_SEGMENT_PATTERN =
  /^(?:[A-Fa-f0-9]{16,}|(?=.*[A-Za-z])(?=.*\d)[A-Za-z0-9_-]{16,})$/;
...
export function getTabSummary(data: Record<string, unknown>): string | undefined {
  ...
  const lines = tabs.map((tab, index) => {
    ...
    const label =
      typeof tab.label === "string" && tab.label.trim().length > 0 ? tab.label.trim() : undefined;
    const tabSelector =
      typeof tab.tabId === "string" && tab.tabId.trim().length > 0
        ...
        : label
          ...
          : typeof tab.index === "number"
            ...
            : String(index);
    const labelText =
      label && label !== tabSelector ? ` label=${redactModelFacingText(label)}` : "";
    const targetId =
      typeof tab.targetId === "string" && tab.targetId.trim().length > 0
        ? tab.targetId.trim()
        : undefined;
    const targetText =
      targetId && targetId !== tabSelector ? ` target=${redactModelFacingText(targetId)}` : "";
...
export function enrichStreamStatusData(commandInfo: CommandInfo, data: unknown): unknown {
  if (
    commandInfo.command !== "stream" ||
    commandInfo.subcommand !== "status" ||
    !isRecord(data) ||
...
export function formatDiagnosticSummary(
  ...
  if (commandInfo.command === "auth") {
    ...
    const name =
      getStringField(data, "name") ?? getStringField(data, "profile") ?? commandInfo.subcommand;
  ...
  if (commandInfo.command === "storage") {
    ...
    if (
      key &&
      (commandInfo.subcommand === "set" || data.set === true || Object.hasOwn(data, "value"))
    )
  ...
  if (commandInfo.command === "network") {
    if (commandInfo.subcommand === "requests") {
      ...
      if (requests)
        return isClearDiagnosticCommand(commandInfo)
          ? `Network requests reset: ${requests.length} cleared`
          : `Network requests: ${requests.length}`;
    }
    if (commandInfo.subcommand === "route") {
      const routed =
        getStringField(data, "routed") ??
        getStringField(data, "url") ??
        getStringField(data, "pattern");
      return routed
        ? `Network route: ${redactModelFacingTextIfSensitive(routed)}`
        : "Network route configured";
    }
    if (commandInfo.subcommand === "unroute") {
      const unrouted =
        getStringField(data, "unrouted") ??
        getStringField(data, "url") ??
        getStringField(data, "pattern");
      return unrouted
        ? `Network unroute: ${redactModelFacingTextIfSensitive(unrouted)}`
        : "Network route removed";
    }
    if (commandInfo.subcommand === "har") {
      const state =
        getStringField(data, "state") ?? getStringField(data, "status") ?? commandInfo.subcommand;
  ...
  if (commandInfo.command === "console") {
    ...
    if (messages)
      return isClearDiagnosticCommand(commandInfo)
        ? `Console reset: ${messages.length} cleared`
  ...
  if (commandInfo.command === "errors") {
    ...
    if (errors)
      return isClearDiagnosticCommand(commandInfo)
        ? `Page errors reset: ${errors.length} cleared`
...
function formatNetworkRequestLine(item: Record<string, unknown>, index: number): string[] {
  ...
  const impactText = failureClassification
    ? ` [${failureClassification.impact}: ${failureClassification.reason}]`
    : "";
  const lines = [
    `${index + 1}. ${status} ${method} ${truncateText(redactSensitiveText(url), 180)}${type ? ` (${type})` : ""}${idText}${impactText}`,
  ];
  appendNetworkPreview(
    lines,
    "Payload",
    getPreviewCandidate(item, NETWORK_PREVIEW_FIELD_CANDIDATES.request),
    NETWORK_BODY_PREVIEW_MAX_CHARS,
  );
  appendNetworkPreview(
    lines,
    "Response",
    getPreviewCandidate(item, NETWORK_PREVIEW_FIELD_CANDIDATES.response),
    NETWORK_BODY_PREVIEW_MAX_CHARS,
  );
  appendNetworkPreview(
    lines,
    "Error",
    getPreviewCandidate(item, NETWORK_PREVIEW_FIELD_CANDIDATES.error),
    NETWORK_ERROR_PREVIEW_MAX_CHARS,
  );
  return lines;
}

function formatNetworkRequestsText(
  data: Record<string, unknown>,
  commandInfo: CommandInfo,
): string | undefined {
  const requests = getArrayField(data, "requests");
  ...
  const artifactNoiseRequests = indexedRequests.filter(
    (indexed) => isRecord(indexed.item) && isNetworkArtifactNoiseRequest(indexed.item),
  );
  const previewRequests = indexedRequests.filter(
    (indexed) => !(isRecord(indexed.item) && isNetworkArtifactNoiseRequest(indexed.item)),
  );
  const networkFailureSummary = summarizeNetworkFailures(
    previewRequests.map((indexed) => indexed.item),
  );
  ...
  if (artifactNoiseRequests.length > 0) {
    shown.push(
      `Diagnostic noise hidden from preview: ${artifactNoiseRequests.length} data:image/artifact request row${artifactNoiseRequests.length === 1 ? "" : "s"}; raw rows remain in details.data.requests.`,
    );
  }
  failedRequests.sort((left, right) => {
    const leftClassification = isRecord(left.item)
      ? classifyNetworkRequestFailure(left.item)
      : undefined;
    const rightClassification = isRecord(right.item)
      ? classifyNetworkRequestFailure(right.item)
      : undefined;
  ...
  shown.push(
    ...prioritizedRequests.slice(0, DIAGNOSTIC_REQUEST_PREVIEW_LIMIT).flatMap(({ item, index }) => {
      if (!isRecord(item)) return [`${index + 1}. ${stringifyModelFacing(item)}`];
      return formatNetworkRequestLine(item, index);
    }),
  );
  const omittedPreviewCount = Math.max(
    0,
    prioritizedRequests.length - DIAGNOSTIC_REQUEST_PREVIEW_LIMIT,
  );
  if (omittedPreviewCount > 0) {
    shown.push(
      `... (${omittedPreviewCount} additional non-noise requests omitted from preview; failed requests are shown first when present)`,
    );
  }
  return shown.join("\n");
}
...
function isSensitiveNetworkPathSegment(segment: string): boolean {
  ...
  return (
    normalized === "auth" ||
    NETWORK_FILTER_SENSITIVE_SEGMENT_TERMS.some((term) => normalized.includes(term))
  );
}
...
function pathFilterMayExposeSensitiveSegment(filter: string): boolean {
  ...
  return decoded
    ...
    .some(
      (segment) =>
        isSensitiveNetworkPathSegment(segment) ||
        NETWORK_FILTER_OPAQUE_SEGMENT_PATTERN.test(segment),
    );
}
...
function getNetworkRequestPathFilter(item: Record<string, unknown>): string | undefined {
  ...
  if (
    !filter ||
    filter === "/" ||
    filter.length > NETWORK_FILTER_MAX_CHARS ||
    pathFilterMayExposeSensitiveSegment(filter)
  )
    return undefined;
  return getSafeNetworkActionValue(filter);
}

function getNetworkRequestActionCandidate(
  item: Record<string, unknown>,
): NetworkRequestActionCandidate | undefined {
  ...
  const kind: NetworkRequestActionCandidate["kind"] =
    classification?.impact === "actionable"
      ? "actionable"
...
function chooseNetworkRequestActionCandidate(
  candidates: NetworkRequestActionCandidate[],
): NetworkRequestActionCandidate | undefined {
  return (
...
export function buildNetworkRouteDiagnosticsNextActions(
  diagnostics: NetworkRouteDiagnostic[] | undefined,
...
export function buildNetworkRequestsNextActions(
  data: unknown,
  sessionName: string | undefined,
  routeDiagnostics?: NetworkRouteDiagnostic[],
): AgentBrowserNextAction[] | undefined {
  ...
  if (selected.kind === "actionable") {
    actions.push({
      ...
      safety:
        "Read-only experimental helper; it reports bounded candidates and may miss bundled or dynamic call sites.",
      tool: "agent_browser",
    });
  }
  if (selected.filter) {
  ...
  return [
    ...(buildNetworkRouteDiagnosticsNextActions(routeDiagnostics, sessionName) ?? []),
    ...actions,
  ].slice(0, NETWORK_NEXT_ACTION_LIMIT);
}
...
function formatConsoleText(
  ...
  shown.push(
    ...messages.slice(0, DIAGNOSTIC_LOG_PREVIEW_LIMIT).map((item, index) => {
      if (!isRecord(item)) return `${index + 1}. ${stringifyModelFacing(item)}`;
  ...
  if (messages.length > previewedMessageCount) {
    shown.push(
      `... (${messages.length - previewedMessageCount} additional console messages omitted from preview)`,
    );
  }
  return shown.join("\n");
}
...
function formatA11yText(data: Record<string, unknown>): string | undefined {
  ...
  const violationCount =
    typeof counts?.violations === "number" ? counts.violations : violations.length;
  const incompleteCount =
    typeof counts?.incomplete === "number" ? counts.incomplete : incomplete.length;
  const passCount = typeof counts?.passes === "number" ? counts.passes : undefined;
  const inapplicableCount =
    typeof counts?.inapplicable === "number" ? counts.inapplicable : undefined;
  const countParts = [
...
function formatChatText(data: Record<string, unknown>): string | undefined {
  ...
  const lines = [
    model ? `Model: ${redactModelFacingText(model)}` : undefined,
    provider ? `Provider: ${redactModelFacingText(provider)}` : undefined,
  ].filter(Boolean);
  return lines.length > 0 ? lines.join("\n") : undefined;
}
...
function formatDoctorText(data: Record<string, unknown>): string | undefined {
  ...
  if (checks) {
    ...
    for (const [index, item] of checks.slice(0, 30).entries()) {
      ...
      lines.push(
        `${index + 1}. [${redactModelFacingText(checkStatus)}]${label ? ` ${redactModelFacingText(label)}:` : ""} ${firstLine(redactModelFacingText(message), 220)}`,
      );
...
function formatCookieRecordText(item: Record<string, unknown>, fallbackName: string): string {
  ...
  return [
    name,
    location ? `(${redactModelFacingText(location)})` : undefined,
...
function valueContainsStorageSecret(value: unknown): boolean {
  if (typeof value === "string") {
    ...
    if (
      STORAGE_TOKEN_VALUE_PATTERN.test(trimmed) ||
      STORAGE_SECRET_VALUE_WORD_PATTERN.test(trimmed) ||
      STORAGE_EMAIL_VALUE_PATTERN.test(trimmed) ||
      STORAGE_IDENTITY_VALUE_PATTERN.test(trimmed)
    )
      return true;
    try {
    ...
    if (
      redactSensitiveText(trimmed) !== trimmed ||
      redactModelFacingTextIfSensitive(trimmed) !== trimmed
    )
      return true;
    try {
...
function redactStorageEntryValue(item: Record<string, unknown>): Record<string, unknown> {
  if (!Object.hasOwn(item, "value"))
    return redactStructuredPresentationValue(item) as Record<string, unknown>;
  const key = getStringField(item, "key") ?? getStringField(item, "name");
  const value = item.value;
  if (shouldRevealStorageValue(key, value))
    return redactStructuredPresentationValue(item) as Record<string, unknown>;
  return {
    ...(redactStructuredPresentationValue({ ...item, value: undefined }) as Record<
      string,
      unknown
    >),
    value: "[REDACTED]",
    valueRedacted: true,
    valueRedactionReason:
      key && STORAGE_SECRET_KEY_PATTERN.test(key) ? "sensitive-key" : "sensitive-value",
  };
}
    ...
    Object.entries(value).map(([key, entryValue]) => {
      if ((key === "entries" || key === "items") && Array.isArray(entryValue))
        return [
          ...
          entryValue.map((item) =>
            isRecord(item)
              ? redactStorageEntryValue(item)
              : redactStructuredPresentationValue(item),
          ),
        ];
      if (key === "value") {
        ...
        return [
          key,
          shouldRevealStorageValue(itemKey, entryValue)
            ? redactStructuredPresentationValue(entryValue)
            : "[REDACTED]",
        ];
      }
    ...
    return entries
      .map((item, index) => {
        ...
        return Object.hasOwn(item, "value")
          ? `${key}: ${formatStorageValue(rawKey, item.value)}`
          : key;
      })
      .join("\n");
  }
...
function formatDialogText(data: Record<string, unknown>): string | undefined {
  ...
  if (message)
    lines.push(
      `Message: ${/(?:auth|authorization|bearer|cookie|pass(?:word)?|secret|session|token)/i.test(message) ? "[REDACTED]" : redactModelFacingText(message)}`,
    );
...
function formatFrameText(data: Record<string, unknown>): string | undefined {
  ...
  const lines = [
    frame ? `Frame: ${redactModelFacingText(frame)}` : undefined,
    title ? `Title: ${redactModelFacingText(title)}` : undefined,
    url ? `URL: ${redactModelFacingTextIfSensitive(url)}` : undefined,
  ].filter(Boolean);
  return lines.length > 0 ? lines.join("\n") : undefined;
}
  ...
  if (states) {
    ...
    return states
      .map((item, index) => {
        ...
        const url = getStringField(item, "url");
        return url
          ? `${index + 1}. ${redactModelFacingText(name)} — ${redactModelFacingTextIfSensitive(url)}`
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, entryValue]) => [
      ...
      sensitiveKeys.has(key.toLowerCase())
        ? "[REDACTED]"
        : redactStatefulValues(entryValue, sensitiveKeys),
    ]),
  );
}
