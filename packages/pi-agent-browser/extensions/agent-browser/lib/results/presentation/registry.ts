...
const COMMAND_PRESENTERS: Record<string, CommandPresenter> = {
  ...
  read: {
    summary: (_commandInfo, data) =>
      isRecord(data) && typeof (data.finalUrl ?? data.url) === "string"
        ? `Read: ${data.finalUrl ?? data.url}`
        : undefined,
    text: (_commandInfo, data) =>
  ...
  tab: {
    summary: (_commandInfo, data) =>
      isRecord(data) && Array.isArray(data.tabs)
        ? `Tabs: ${data.tabs.length}`
        : isRecord(data) && data.closed === true
          ? "Tab closed"
          : undefined,
    text: (_commandInfo, data) =>
      isRecord(data) && data.closed === true
...
export function formatPresentationSummary(
  ...
  if (isRecord(data)) {
    ...
    if (
      ...
      isNavigationObservableCommand(
        ...
        presentationCommandInfo.subcommand,
      )
    ) {
  ...
  const presenterSummary = commandInfo.command
    ? COMMAND_PRESENTERS[commandInfo.command]?.summary?.(commandInfo, data)
    : undefined;
...
export function formatPresentationContentText(
  ...
  const presenterText = commandInfo.command
    ? COMMAND_PRESENTERS[commandInfo.command]?.text?.(commandInfo, data)
    : undefined;
