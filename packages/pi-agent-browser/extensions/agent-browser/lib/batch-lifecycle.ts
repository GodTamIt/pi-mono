...
function getRowBrowserLaunched(row: Record<string, unknown>): boolean | undefined {
  ...
  const lifecycle = isRecord(row.lifecycle)
    ? row.lifecycle
    : isRecord(result?.lifecycle)
      ? result.lifecycle
      : undefined;
  const effectiveLaunch = isRecord(lifecycle?.effectiveLaunch)
    ? lifecycle.effectiveLaunch
    : undefined;
  return typeof effectiveLaunch?.browserLaunched === "boolean"
    ? effectiveLaunch.browserLaunched
    : undefined;
}
