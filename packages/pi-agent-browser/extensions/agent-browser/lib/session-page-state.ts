...
function getRestoredSessionTabTarget(
  ...
  const storedTarget = isRecord(details.sessionTabTarget)
    ? normalizeSessionTabTarget({
        title:
          typeof details.sessionTabTarget.title === "string"
            ? details.sessionTabTarget.title
            : undefined,
        url:
          typeof details.sessionTabTarget.url === "string"
...
function getRestoredRefSnapshotInvalidation(
  ...
  if (invalidation?.reason === "page-transition")
    return buildPageTransitionRefSnapshotInvalidation(
      typeof invalidation.summary === "string" ? invalidation.summary : undefined,
    );
  const errorText =
...
function getRestoredRefSnapshot(details: Record<string, unknown>): SessionRefSnapshot | undefined {
  ...
  const refIds = refSnapshot.refIds.filter(
    (refId): refId is string => typeof refId === "string" && /^e\d+$/.test(refId),
  );
    ...
    ? Object.fromEntries(
        refIds.flatMap((refId) => {
          ...
          const isContentEditable =
            typeof entry.isContentEditable === "boolean" ? entry.isContentEditable : undefined;
          const isEditable = typeof entry.isEditable === "boolean" ? entry.isEditable : undefined;
          return [
            [
              refId,
              {
                ...(isContentEditable !== undefined ? { isContentEditable } : {}),
  ...
  return {
    ...
    target: isRecord(refSnapshot.target)
      ? normalizeSessionTabTarget({
          title:
            typeof refSnapshot.target.title === "string" ? refSnapshot.target.title : undefined,
...
export class SessionPageState {
  ...
  static fromBranch(branch: unknown[]): SessionPageState {
    ...
    for (const entry of branch) {
      ...
      const subcommand =
        typeof details.subcommand === "string" ? details.subcommand : commandTokens[1];
