...
const PROMPT_ARTIFACT_PATH_PATTERN =
  /(?:^|[\s"'`(:])((?:\/[^\s"'`),;]+|[A-Za-z]:[\\/][^\s"'`),;]+|\.{1,2}[\\/][^\s"'`),;]+|[^\s"'`()[\],;:\\/]+(?:[\\/][^\s"'`()[\],;\\/]+)+|[^\s"'`()[\],;:\\/]+)\.(?:png|jpe?g|webp|gif|webm|mp4|har|pdf|trace|json))(?=[\s"'`),;.!?]|$)/gi;
const PROMPT_ARTIFACT_COLON_OUTPUT_INTENT_PATTERN =
...
const PROMPT_ARTIFACT_OUTPUT_INTENT_PATTERN =
...
const PROMPT_ARTIFACT_UNSAFE_OUTPUT_PREFIX_PATTERN =
...
const PROMPT_ARTIFACT_AFFIRMATIVE_PREFIX_PATTERN =
...
const PROMPT_ARTIFACT_STANDALONE_OPTIONAL_RECORDING_PATTERN =
  /^\s*(?:if|when)\s+(?:recordings?\s+)?(?:(?:are|is)\s+)?available[.:;]?\s*$/i;
const PROMPT_ARTIFACT_EXPLICIT_OPTIONAL_PATTERN =
...
const PROMPT_ARTIFACT_REFERENCE_INTENT_PATTERN =
...
function extractPromptRequestedArtifacts(prompt: string): PromptRequestedArtifact[] {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (
      ...
      PROMPT_ARTIFACT_STANDALONE_OPTIONAL_RECORDING_PATTERN.test(line)
    ) {
    ...
    if (
      pathMatches.length === 0 &&
      pendingRecordingAvailability &&
      /\brecordings?\b/i.test(line) &&
      hasPromptArtifactOutputIntent(line)
    )
      continue;
    const pendingRecordingAvailabilityForLine =
      pathMatches.length > 0 && pendingRecordingAvailability;
    pendingRecordingAvailability = false;
    ...
    const isPathList =
      pathMatches.length > 0 && PROMPT_ARTIFACT_LIST_CONNECTOR_PATTERN.test(listSyntax);
    const groupOptional = new Map<number, boolean>();
    const candidates: Array<{
      continuedFromPriorLine: boolean;
      group: number;
      kind: PromptRequestedArtifact["kind"];
      matchIndex: number;
      path: string;
    }> = [];
    let nextGroup = 0;
    ...
    for (let matchIndex = 0; matchIndex < pathMatches.length; matchIndex += 1) {
      ...
      const intentContext =
        matchIndex === 0 &&
        (isPathList || !remainder || ["file", "output", "path"].includes(remainder))
          ? `${lines[lineIndex - 1] ?? ""}\n${localContext}`
          : localContext;
      ...
      const sameLineContinuation =
        ...
        PROMPT_ARTIFACT_LIST_CONNECTOR_PATTERN.test(localContext);
      const priorLineContinuation =
        matchIndex === 0 &&
        isPathList &&
        kind === listContinuation?.kind &&
        PROMPT_ARTIFACT_LIST_CONNECTOR_PATTERN.test(localContext);
      ...
      if (directIntent) {
        ...
        groupOptional.set(
          group,
          pendingRecordingAvailabilityForLine ||
            PROMPT_ARTIFACT_OPTIONAL_RECORDING_PATTERN.test(
              getPromptArtifactIntentClause(intentContext),
            ),
        );
      ...
      const trailingContext = getPromptArtifactTrailingClause(
        line.slice(end, pathMatches[matchIndex + 1]?.start ?? line.length),
      );
      const explicitlyOptional =
        directIntent &&
        (PROMPT_ARTIFACT_EXPLICIT_OPTIONAL_PATTERN.test(
          getPromptArtifactIntentClause(intentContext),
        ) ||
      ...
      if (
        ...
        referenceReading ||
        explicitlyOptional ||
        isLikelyInboundPromptArtifact(path)
      ) {
        previousGroup = undefined;
      ...
      candidates.push({
        continuedFromPriorLine: priorLineContinuation && !directIntent,
        group,
        kind,
        matchIndex,
        path,
    ...
    for (const candidate of candidates) {
      ...
      if (
        PROMPT_ARTIFACT_OPTIONAL_RECORDING_PATTERN.test(
          getPromptArtifactTrailingClause(line.slice(match.end, nextStart)),
        )
      ) {
    ...
    for (const candidate of candidates) {
      const required =
        candidate.kind === "screenshot" || groupOptional.get(candidate.group) !== true;
