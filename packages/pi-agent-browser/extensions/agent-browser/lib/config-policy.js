...
export const CONFIG_RELATIVE_PATH = /** @type {const} */ ([CONFIG_DIR_NAME, "config", "pi-agent-browser", "config.json"]);
export const GLOBAL_CONFIG_RELATIVE_PATH = /** @type {const} */ ([CONFIG_DIR_NAME, "config", "pi-agent-browser", "config.json"]);
...
export function validateAgentBrowserConfig(value, path, errors, warnings) {
	...
	for (const key of Object.keys(value)) {
		if (!["version", "webSearch", "browser"].includes(key)) {
			warnings.push(`${path}.${key} is not a recognized pi-agent-browser config field and was ignored.`);
		}
	}
	return config;
}
