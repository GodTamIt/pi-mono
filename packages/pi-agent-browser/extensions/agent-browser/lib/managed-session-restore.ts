...
function ownedContextMatches(
  ...
  return owned &&
    ...
    canonicalizeAgentBrowserNamespace(namespace) === owned.namespace
    ? owned
    : undefined;
}
...
export function isOwnedManagedSessionTarget(args: string[]): boolean {
  return (
    ownedContextMatches(extractExplicitSessionName(args), extractExplicitNamespace(args)) !==
    undefined
  );
}
...
export function getManagedSessionRestoreEnv(
  ...
  if (
    ...
    isManagedSessionRestoreIncompatible(policyOptions)
  )
    return {};
