...
export function reorderWindowsLeadingGlobalArgs(args: string[]): string[] {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (isKnownCommandToken(token)) {
      ...
      return WINDOWS_COMMANDS_WITH_ADJACENT_SUBCOMMAND.has(token) &&
        firstPositional &&
        !firstPositional.startsWith("-")
...
export function isWindowsAgentBrowserCommandMissing(stderr: string): boolean {
  ...
  return (
    normalized.includes(WINDOWS_AGENT_BROWSER_MISSING_MARKER.toLowerCase()) ||
    (normalized.includes("agent-browser.cmd") &&
...
export function shouldCommitManagedRestoreAfterWindowsProcess(input: {
  ...
  return (
    !input.spawnError &&
    !(input.exitCode !== 0 && isWindowsAgentBrowserCommandMissing(input.stderr))
  );
}

function terminateSpawnedChild(
...
async function socketDirEntriesAreOwned(
  ...
  for (const name of await readdir(socketDir)) {
    visited.count += 1;
    if (visited.count > 16_384) return false;
    try {
...
export async function runAgentBrowserProcess(options: {
  ...
  const requestedSocketDir =
    explicitSocketDir ?? parentEnv[PI_AGENT_BROWSER_SOCKET_DIR_ENV] ?? getAgentBrowserSocketDir();
    ...
    const finish = (exitCode: number) => {
      ...
      void pendingStdoutWrite.finally(async () => {
        ...
        const windowsMissingBinary =
          ...
          exitCode !== 0 &&
          isWindowsAgentBrowserCommandMissing(stderr);
        ...
        if (windowsMissingBinary && !spawnError) {
          ...
          processPlatform === "win32" &&
          shouldCommitManagedRestoreAfterWindowsProcess({ exitCode, spawnError, stderr })
        ) {
