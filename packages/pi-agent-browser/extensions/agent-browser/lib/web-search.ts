...
export const EXA_SEARCH_CATEGORIES = [
  "company",
  "people",
  "publication",
  "news",
...
export const WEB_SEARCH_PROVIDER_ADAPTERS: Readonly<
  Record<WebSearchProvider, WebSearchProviderAdapter>
> = {
  ...
  return {
    ...
    async execute(
      ...
      if (!runtimeConfigState.webSearchEnabled) {
        throw new Error("agent_browser_web_search is disabled by pi-agent-browser config.");
      }
