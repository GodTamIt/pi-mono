...
async function ensureCoordinationDirectory(
  path: string,
...
function getPolicyLockDigest(sessionName: string, namespace?: string): string {
  const identity = getAgentBrowserSessionIdentityKey(
    sessionName,
    canonicalizeAgentBrowserNamespace(namespace),
  );
...
export function getManagedSessionPolicyLockPath(sessionName: string, namespace?: string): string {
  return join(
    ...
    `.pi-agent-browser-policy-${getPolicyLockDigest(sessionName, namespace)}.lock-v3`,
  );
}
...
async function readClaim(path: string): Promise<PolicyLockClaim | undefined> {
  try {
    ...
    try {
      ...
      if (
        !ticketEntry.isFile() ||
        ticketEntry.isSymbolicLink() ||
        ticketEntry.size > POLICY_LOCK_MAX_BYTES
      )
        return undefined;
      if (process.platform !== "win32") {
        const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
...
export async function acquireManagedSessionPolicyLock(options: {
  ...
  try {
    ...
    const maxTicket = initialClaims.reduce(
      (max, claim) => (claim.ticket === null ? max : Math.max(max, claim.ticket)),
      0,
    );
    ...
    await writeFile(ticketCandidatePath, JSON.stringify(ticket), {
      encoding: "utf8",
      flag: "wx",
      mode: 0o600,
    });
