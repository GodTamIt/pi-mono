...
function findPackageRoot(startDir: string): string {
  ...
  for (;;) {
    ...
    if (parentDir === currentDir)
      throw new Error("Unable to resolve the pi-agent-browser package root.");
...
function resolveScriptWorkerPath(): string {
  ...
  if (!existsSync(workerPath))
    throw new Error(
      "Compiled script worker is missing; run npm run build or reinstall pi-agent-browser.",
    );
  return workerPath;
}
...
export function compileAgentBrowserScript(input: unknown): {
  compiled?: CompiledAgentBrowserScript;
  error?: string;
} {
...
function normalizeBrowserEnvelope(
  ...
  if (
    ...
    return buildRejectedCallEnvelope(
      "The ordinary agent_browser executor returned an invalid script envelope.",
      false,
    );
  }
  return {
    ...
    nextActions: Array.isArray(value.nextActions)
      ? (value.nextActions as AgentBrowserNextAction[])
      : undefined,
    ok: value.ok,
...
export async function runAgentBrowserScript(
  ...
  if (
    ...
    timeoutMs <= 0 ||
    timeoutMs > AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS
  ) {
    return buildFailedRun({
      callCount: 0,
      emitCount: 0,
      error: `script timeoutMs must be between 1 and ${AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS}.`,
      failureCategory: "validation-error",
      rejectedCallCount: 0,
      steps: [],
    });
    ...
    try {
      while (!stopping && messages.length > 0) {
        ...
        if (message.type === "complete") {
          ...
          await finish(
            { callCount, data, emitCount: emissions.length, ok: true, rejectedCallCount, steps },
            false,
          );
          return;
        }
