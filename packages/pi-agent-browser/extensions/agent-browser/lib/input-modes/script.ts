...
function findPackageRoot(startDir: string): string {
	...
	for (;;) {
		...
		if (parentDir === currentDir) throw new Error("Unable to resolve the pi-agent-browser package root.");
...
function resolveScriptWorkerPath(): string {
	...
	if (!existsSync(workerPath)) throw new Error("Compiled script worker is missing; run npm run build or reinstall pi-agent-browser.");
	return workerPath;
}
