...
export function getCompiledSemanticActionCommandIndex(
  compiled: CompiledAgentBrowserSemanticAction,
): number {
  return compiled.args[0] === "--session" ? 2 : 0;
}

export function getCompiledSemanticActionSessionPrefix(
  compiled: CompiledAgentBrowserSemanticAction,
): string[] {
...
export function compileAgentBrowserSemanticAction(input: unknown): {
  compiled?: CompiledAgentBrowserSemanticAction;
  error?: string;
} {
  if (!isRecord(input)) {
  ...
  const name =
    typeof input.name === "string" && input.name.trim().length === 0 ? undefined : input.name;
  const session = input.session;
  if (
    typeof action !== "string" ||
    !AGENT_BROWSER_SEMANTIC_ACTIONS.includes(action as AgentBrowserSemanticActionName)
  ) {
    return {
  ...
  if (action === "select") {
    ...
    if (typeof selector === "string" && selector.trim().length > 0) {
      ...
      const args =
        typeof session === "string"
          ? ["--session", session, "select", selector, ...(selectedValues.values as string[])]
          : ["select", selector, ...(selectedValues.values as string[])];
    ...
    if (locator === "role") {
      if (typeof role !== "string" || !/^(?:combobox|listbox)$/i.test(role)) {
        return {
          error: "semanticAction.role must be combobox or listbox for locator=role select.",
        };
      }
    ...
    if (optionValues.error) {
      return {
        error: optionValues.error.includes("required")
  ...
  if (selector !== undefined) {
    ...
    const directArgs =
      typeof session === "string" ? ["--session", session, action, selector] : [action, selector];
    if (action === "fill") directArgs.push(text as string);
    return {
      compiled: { action: action as AgentBrowserSemanticActionName, selector, args: directArgs },
    };
  }
  if (
    typeof locator !== "string" ||
    !AGENT_BROWSER_SEMANTIC_LOCATORS.includes(locator as AgentBrowserSemanticLocator)
  ) {
    return {
  ...
  if (role !== undefined && value !== undefined && role !== value) {
    return {
      error: "semanticAction.role must match value when both are provided for locator=role.",
    };
  }
  if (name !== undefined && (locator !== "role" || typeof name !== "string" || name.length === 0)) {
    return {
      error: "semanticAction.name is only supported as a non-empty string for locator=role.",
    };
  }
  const args =
    typeof session === "string"
      ? ["--session", session, "find", locator, locatorValue, action]
      : ["find", locator, locatorValue, action];
  if (action === "fill") {
  ...
  return {
    compiled: {
      action: action as AgentBrowserSemanticActionName,
      locator: locator as AgentBrowserSemanticLocator,
      args,
    },
  };
}
