...
export function compileAgentBrowserSourceLookup(input: unknown): {
  ...
  if (
    reactFiberId !== undefined &&
    (typeof reactFiberId !== "string" || reactFiberId.trim().length === 0)
  ) {
    return { error: "sourceLookup.reactFiberId must be a non-empty string when provided." };
  }
  if (
    componentName !== undefined &&
    (typeof componentName !== "string" || componentName.trim().length === 0)
  ) {
  ...
  if (
    rawMaxWorkspaceFiles !== undefined &&
    (typeof rawMaxWorkspaceFiles !== "number" ||
      !Number.isInteger(rawMaxWorkspaceFiles) ||
      rawMaxWorkspaceFiles <= 0)
  ) {
    return { error: "sourceLookup.maxWorkspaceFiles must be a positive integer when provided." };
  }
  if (
    typeof rawMaxWorkspaceFiles === "number" &&
    rawMaxWorkspaceFiles > SOURCE_LOOKUP_MAX_WORKSPACE_FILES
  ) {
    return {
      error: `sourceLookup.maxWorkspaceFiles must be ${SOURCE_LOOKUP_MAX_WORKSPACE_FILES} or less.`,
    };
  }
  const includeDomHints = input.includeDomHints !== false;
  const maxWorkspaceFiles =
    (rawMaxWorkspaceFiles as number | undefined) ?? SOURCE_LOOKUP_DEFAULT_MAX_WORKSPACE_FILES;
...
function addSourceLookupCandidate(
  ...
  candidate: AgentBrowserSourceLookupCandidate,
): void {
...
function collectSourceCandidatesFromValue(
  value: unknown,
  source: "react-inspect" | "dom-attribute",
  candidates: AgentBrowserSourceLookupCandidate[],
  evidence: string[],
  depth = 0,
): void {
...
function collectDomSourceCandidates(
  html: unknown,
  candidates: AgentBrowserSourceLookupCandidate[],
): void {
  ...
  collectSourceCandidatesFromValue(html, "dom-attribute", candidates, [
    "selector HTML contained source-like text",
  ]);
}
...
async function collectWorkspaceComponentCandidates(
  ...
  const componentPattern = new RegExp(
    `(?:function|class)\\s+${escapeRegExp(query.componentName)}\\b|(?:const|let|var)\\s+${escapeRegExp(query.componentName)}\\s*=|export\\s+default\\s+function\\s+${escapeRegExp(query.componentName)}\\b`,
  );
...
export async function analyzeSourceLookupResults(
  ...
  for (const item of items) {
    ...
    if (command[0] === "react" && command[1] === "inspect") {
      collectSourceCandidatesFromValue(result, "react-inspect", candidates, [
        "react inspect returned source-like metadata",
      ]);
    }
  ...
  const status: AgentBrowserSourceLookupStatus =
    candidates.length > 0 ? "candidates-found" : unsupported ? "unsupported" : "no-candidates";
...
export function compileAgentBrowserNetworkSourceLookup(input: unknown): {
  ...
  if (
    namespace !== undefined &&
    (typeof namespace !== "string" || (namespace !== "" && namespace.trim().length === 0))
  )
    return {
      error:
        "networkSourceLookup.namespace must be a non-empty string or the empty default namespace when provided.",
    };
  ...
  const maxWorkspaceFiles = validateLookupMaxWorkspaceFiles(
  ...
  const effectiveFilter =
    typeof filter === "string" ? filter : typeof url === "string" ? url : undefined;
  if (effectiveFilter) {
  ...
  return {
    compiled: {
      ...
      query: {
        filter,
        maxWorkspaceFiles: maxWorkspaceFiles.value as number,
        namespace,
        requestId,
        session,
        url,
...
function getFailedNetworkRequests(
  data: unknown,
  queryText?: string,
...
function addNetworkCandidate(
  candidates: AgentBrowserNetworkSourceLookupCandidate[],
  candidate: AgentBrowserNetworkSourceLookupCandidate,
): void {
  const key = [
    candidate.source,
    candidate.file ?? "",
  ...
  if (
    !candidates.some(
      (existing) =>
        [existing.source, existing.file ?? "", existing.line ?? "", existing.requestUrl ?? ""].join(
          ":",
        ) === key,
    )
  )
...
function collectInitiatorCandidates(
  data: unknown,
  ...
  for (const item of getBatchResultItems(data)) {
    ...
    for (const value of requestValues) {
      ...
      const requestId =
        typeof value.id === "string"
          ...
          : typeof value.requestId === "string"
            ? value.requestId
            : undefined;
      const correlatesWithFailedRequest =
        (requestId !== undefined && failedRequestIds.has(requestId)) ||
        (requestUrl !== undefined && failedRequestUrls.has(requestUrl));
      ...
      for (const field of [value.initiator, value.stack, value.source, value.trace]) {
        ...
        collectSourceCandidatesFromValue(field, "dom-attribute", localCandidates, [
          "failed network request included source-like initiator metadata",
        ]);
        for (const candidate of localCandidates) {
          addNetworkCandidate(candidates, {
            confidence: "medium",
...
export async function analyzeNetworkSourceLookupResults(
  ...
  await collectWorkspaceRequestCandidates(
    compiled.query,
    failedRequests,
    cwd,
    candidates,
  ...
  const status: AgentBrowserNetworkSourceLookupStatus =
