...
function getRequiredJobString(
  step: Record<string, unknown>,
  field: "path" | "selector" | "text" | "url",
...
function compileJobClickOrFillStep(
  step: Record<string, unknown>,
  action: "click" | "fill",
...
function getUnsupportedJobStepField(
  step: Record<string, unknown>,
...
function getUnsupportedJobStepFieldError(
  step: Record<string, unknown>,
  action: AgentBrowserJobStepAction,
  allowedFields: ReadonlySet<string>,
): string | undefined {
...
function compileJobTypeSteps(step: Record<string, unknown>): {
  error?: string;
  ...
  if (
    delayMs !== undefined &&
    typedChars.length > AGENT_BROWSER_JOB_TYPE_DELAYED_TEXT_MAX_CHARACTERS
  ) {
    return {
      error: `job step type delayMs supports at most ${AGENT_BROWSER_JOB_TYPE_DELAYED_TEXT_MAX_CHARACTERS} characters; split longer text into shorter calls or omit delayMs.`,
    };
  }
  ...
  if (delayMs === undefined) {
    ...
    if (typeof selector === "string")
      compiledSteps.push({
        action: "type",
        args: ["focus", selector],
        generatedFrom: "type.selector",
      });
    for (const [index, char] of typedChars.entries()) {
      compiledSteps.push({
        action: "type",
        args: ["keyboard", "type", char],
        generatedFrom: "type.delayMs",
      });
      if (index < typedChars.length - 1)
        compiledSteps.push({
          action: "wait",
          args: ["wait", String(delayMs)],
          generatedFrom: "type.delayMs",
        });
    }
  }
...
function compileOpenJobStep(step: Record<string, unknown>, index: number): CompileJobStepResult {
  ...
  if (step.loadState !== undefined) {
    if (
      ...
      !AGENT_BROWSER_QA_LOAD_STATES.includes(step.loadState as AgentBrowserQaLoadState)
    ) {
      return {
        error: `job.steps[${index}].loadState must be one of: ${AGENT_BROWSER_QA_LOAD_STATES.join(", ")}.`,
      };
    }
    extraSteps.push({
      action: "wait",
      args: ["wait", "--load", step.loadState],
      generatedFrom: "open.loadState",
    });
  }
...
function compilePathArtifactJobStep(
  step: Record<string, unknown>,
  action: "screenshot" | "waitForDownload",
): CompileJobStepResult {
  const result = getRequiredJobString(step, "path", action);
  if (result.error) return { error: result.error };
  return {
    args:
      action === "waitForDownload"
        ? ["wait", "--download", result.value as string]
        : ["screenshot", result.value as string],
  };
}
...
export function compileAgentBrowserJob(input: unknown): {
  compiled?: CompiledAgentBrowserJob;
  error?: string;
} {
  if (!isRecord(input)) {
  ...
  for (const [index, rawStep] of rawSteps.entries()) {
    ...
    if (
      ...
      !AGENT_BROWSER_JOB_STEP_ACTIONS.includes(action as AgentBrowserJobStepAction)
    ) {
      return {
        error: `job.steps[${index}].action must be one of: ${AGENT_BROWSER_JOB_STEP_ACTIONS.join(", ")}.`,
      };
    }
    const jobAction = action as AgentBrowserJobStepAction;
    const compile = JOB_STEP_COMPILERS[jobAction];
    const unsupportedFieldError = getUnsupportedJobStepFieldError(
      rawStep,
      jobAction,
      JOB_STEP_ALLOWED_FIELDS[jobAction],
    );
    ...
    if (compiledStep.error)
      return {
        error: compiledStep.error.startsWith(`job.steps[${index}]`)
          ? compiledStep.error
          : `job.steps[${index}]: ${compiledStep.error}`,
      };
    steps.push(
      {
        action: jobAction,
        args: compiledStep.args as string[],
        generatedFrom: compiledStep.generatedFrom,
      },
      ...(compiledStep.extraSteps ?? []),
    );
  }
  return {
    compiled: {
...
export function buildQaCompactPassText(options: {
  ...
  const pageParts = [options.page?.title, options.page?.url].filter(
    (part): part is string => typeof part === "string" && part.length > 0,
  );
  if (pageParts.length > 0) lines.push(`Page: ${pageParts.join(" — ")}`);
  lines.push(
    `Checks run: ${describeQaChecksRun(options.checks)} (${options.batchStepCount} batch step${options.batchStepCount === 1 ? "" : "s"})`,
  );
  if (
    options.checks.diagnosticsResetAtStart &&
    (options.checks.checkNetwork || options.checks.checkConsole || options.checks.checkErrors)
  ) {
    lines.push(
      "Diagnostic isolation: URL QA clears enabled network/console buffers, then snapshots any page-error residue before opening the target. Only unchanged residue is ignored because upstream page-error clear is not reliable.",
    );
  }
  if (
    options.checks.attached &&
    !options.checks.diagnosticsResetAtStart &&
    (options.checks.checkNetwork || options.checks.checkConsole || options.checks.checkErrors)
  ) {
    lines.push(
...
function qaVisibleTextWaitPassed(
  item: ReturnType<typeof getBatchResultItems>[number] | undefined,
...
function extractQaTextAssertionResultText(
  item: ReturnType<typeof getBatchResultItems>[number] | undefined,
): string | undefined {
...
function subtractQaBaselineErrors(
  errors: unknown[],
  baselineErrors: unknown[],
): { ignoredCount: number; novelErrors: unknown[] } {
...
function isDiagnosticResetCommand(item: Record<string, unknown>): boolean {
  ...
  if (
    ...
    !command.every((token): token is string => typeof token === "string")
  )
    return false;
...
export function analyzeQaPresetTimeout(
  ...
  const failedChecks = compiled.checks.expectedText.map(
    (text) => `expected text was not verified before timeout: ${formatQaExpectedTextPreview(text)}`,
  );
  return {
    failedChecks,
    passed: false,
...
export function analyzeQaPresetResults(
  ...
  const baselineErrorIndex =
    compiled?.checks.diagnosticsResetAtStart && compiled.checks.checkErrors
  ...
  const baselineErrorResult = isRecord(baselineErrorItem?.result)
    ? baselineErrorItem.result
    : undefined;
  const baselineErrors = Array.isArray(baselineErrorResult?.errors)
    ? baselineErrorResult.errors
    : [];
  for (const [index, item] of items.entries()) {
    ...
    if (commandName === "console" && Array.isArray(result?.messages)) {
      const errorCount = result.messages.filter(
        (message) =>
          isRecord(message) && /error/i.test(String(message.type ?? message.level ?? "")),
      ).length;
    ...
    if (commandName === "network" && Array.isArray(result?.requests)) {
      ...
      if (networkFailures.actionableCount > 0)
        failedChecks.push(
          `${networkFailures.actionableCount} actionable failed network request(s)`,
        );
    ...
    summary:
      uniqueFailures.length === 0
        ? uniqueWarnings.length === 0
          ? "QA preset passed."
          : `QA preset passed with warnings: ${uniqueWarnings.join("; ")}.`
...
export function compileAgentBrowserQaPreset(input: unknown): {
  ...
  if (
    !expectedText ||
    expectedText.some((text) => typeof text !== "string" || text.trim().length === 0)
  ) {
    return {
      error:
  ...
  if (
    ...
    (typeof expectedSelector !== "string" || expectedSelector.trim().length === 0)
  ) {
  ...
  if (
    ...
    (typeof screenshotPath !== "string" || screenshotPath.trim().length === 0)
  ) {
  ...
  if (
    ...
    (typeof rawLoadState !== "string" ||
      !AGENT_BROWSER_QA_LOAD_STATES.includes(rawLoadState as AgentBrowserQaLoadState))
  ) {
