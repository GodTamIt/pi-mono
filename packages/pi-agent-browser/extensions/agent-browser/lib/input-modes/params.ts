  ...
  return Type.Object(
    {
      script: Type.Optional(
        Type.String({
          description:
            "One-shot JavaScript orchestration with async browser({ args, stdin?, timeoutMs? }) and emit(value); isolated session and no host access.",
      ...
      args: Type.Optional(
        Type.Array(Type.String(), {
          description:
            "Raw agent-browser argv only: no binary, shell operators, or --json. Start with open → snapshot -i → act on current @refs; re-snapshot after page changes.",
          minItems: 1,
        }),
      ),
        ...
        Type.Object(
          {
            ...
            locator: Type.Optional(
              StringEnum(AGENT_BROWSER_SEMANTIC_LOCATORS, {
                description: "Locator for check/click/fill; select supports role or label.",
              }),
            ),
            value: Type.Optional(
              Type.String({
                description:
                  "Locator value or one select option; for select by label, this is the label text.",
              }),
            ),
            values: Type.Optional(
              Type.Array(Type.String(), {
                description: "Select options; required for select by label.",
                minItems: 1,
              }),
            ),
            ...
            role: Type.Optional(
              Type.String({ description: "Role locator; alternative to value." }),
            ),
      ...
      qa: Type.Optional(
        Type.Union(
          ...
          {
            description:
              "QA a URL or current session (attached=true). Default readiness: domcontentloaded; use networkidle only without long-lived requests. URL diagnostics default on, attached diagnostics off.",
          },
        ),
      ),
        ...
        Type.Object(
          {
            ...
            reactFiberId: Type.Optional(
              Type.String({ description: "React fiber id; requires --enable react-devtools." }),
            ),
            componentName: Type.Optional(
              Type.String({ description: "Component for local source search." }),
            ),
            includeDomHints: Type.Optional(
              Type.Boolean({ description: "Inspect DOM source hints; default true." }),
            ),
            maxWorkspaceFiles: Type.Optional(
              Type.Number({
                description: "Source scan cap; default 2000.",
                minimum: 1,
                maximum: SOURCE_LOOKUP_MAX_WORKSPACE_FILES,
              }),
            ),
          },
          {
            additionalProperties: false,
            description: "EXPERIMENTAL UI-to-source candidates; not guaranteed mappings.",
          },
        ),
      ),
      networkSourceLookup: Type.Optional(
        Type.Object(
          ...
          {
            additionalProperties: false,
            description: "EXPERIMENTAL failed-request-to-source candidates; not proof.",
          },
        ),
      ),
      electron: Type.Optional(
        Type.Union(
            ...
            Type.Object(
              {
                ...
                query: Type.Optional(
                  Type.String({ description: "Case-insensitive app filter.", minLength: 1 }),
                ),
                maxResults: Type.Optional(
                  Type.Integer({
                    description: `Result cap; default ${ELECTRON_DISCOVERY_DEFAULT_MAX_RESULTS}, values over ${ELECTRON_DISCOVERY_MAX_RESULTS} are clamped.`,
                    minimum: 1,
                  }),
                ),
              },
            ...
            Type.Object(
              {
                ...
                appPath: Type.Optional(
                  Type.String({ description: "macOS .app path.", minLength: 1 }),
                ),
                appName: Type.Optional(
                  Type.String({ description: "Name from electron.list.", minLength: 1 }),
                ),
                bundleId: Type.Optional(
                  Type.String({ description: "Bundle id from electron.list.", minLength: 1 }),
                ),
                executablePath: Type.Optional(
                  Type.String({ description: "Executable path.", minLength: 1 }),
                ),
            ...
            Type.Object(
              {
                ...
                launchId: Type.Optional(
                  Type.String({ description: "Tracked launch id.", minLength: 1 }),
                ),
                timeoutMs: Type.Optional(Type.Integer({ minimum: 1 })),
              },
              { additionalProperties: false },
          ...
          {
            description:
              "Electron discovery, isolated-profile launch, status, probe, or cleanup. Launch defaults: handoff=snapshot, targetType=page; deny wins over allow; lifecycle/debug appArgs rejected.",
          },
        ),
      ),
      job: Type.Optional(
        Type.Object(
          {
            failFast: Type.Optional(
              Type.Boolean({ description: "Stop on first failure; default true." }),
            ),
            steps: Type.Array(
              Type.Object(
                {
                  ...
                  url: Type.Optional(
                    Type.String({
                      description:
                        "Open URL, or assertUrl exact URL/glob that distinguishes the expected state from the starting page.",
                    }),
                  ),
                  loadState: Type.Optional(
                    StringEnum(AGENT_BROWSER_QA_LOAD_STATES, {
                      description: "Readiness wait after open.",
                    }),
                  ),
                  selector: Type.Optional(Type.String({ description: "Selector or @ref." })),
                  locator: Type.Optional(
                    StringEnum(AGENT_BROWSER_SEMANTIC_LOCATORS, {
                      description: "Locator when selector is omitted.",
                    }),
                  ),
                  role: Type.Optional(Type.String({ description: "Role locator." })),
                  name: Type.Optional(Type.String({ description: "Accessible name." })),
                  text: Type.Optional(
                    Type.String({ description: "Fill text or assertText target." }),
                  ),
                  value: Type.Optional(
                    Type.String({ description: "Select option or locator value." }),
                  ),
                  values: Type.Optional(
                    Type.Array(Type.String(), { description: "Select options.", minItems: 1 }),
                  ),
                  path: Type.Optional(Type.String({ description: "Download or screenshot path." })),
                  delayMs: Type.Optional(
                    Type.Integer({
                      description: `Per-character type delay; text is capped at ${AGENT_BROWSER_JOB_TYPE_DELAYED_TEXT_MAX_CHARACTERS} characters.`,
                      minimum: 1,
                    }),
                  ),
                  press: Type.Optional(Type.String({ description: "Key to press after typing." })),
                  milliseconds: Type.Optional(
                    Type.Number({ description: "Wait duration in milliseconds." }),
                  ),
                },
                { additionalProperties: false },
              ),
      ...
      stdin: Type.Optional(
        Type.String({
          description:
            "Raw stdin for batch, eval --stdin, or auth save --password-stdin; unavailable with structured modes and electron.",
        }),
      ),
      outputPath: Type.Optional(
        Type.String({
          description:
            "Workspace-relative or absolute result-data path; keep it distinct from screenshot, download, recording, and other browser artifact destinations.",
          minLength: 1,
        }),
      ),
      timeoutMs: Type.Optional(
        Type.Integer({
          description:
            "Wrapper timeout in ms; exceed explicit waits. Electron uses electron.timeoutMs.",
          minimum: 1,
        }),
      ),
      sessionMode: Type.Optional(
        StringEnum(["auto", "fresh"] as const, {
          description:
            "auto reuses the managed session; fresh starts one for launch-only flags, then makes it the managed session.",
    ...
    },
    {
      additionalProperties: false,
      description:
        "Choose one input mode: script, args, semanticAction, job, qa, sourceLookup, networkSourceLookup, or electron.",
    },
  );
}
