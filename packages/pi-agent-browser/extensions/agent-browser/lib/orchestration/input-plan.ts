...
import {
  AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS,
...
export interface ResolvedAgentBrowserInvalidInput
  extends ResolvedAgentBrowserInputBase,
...
function redactCompiledElectron(
  ...
  if (compiled.action === "launch") {
    return {
      ...compiled,
      appArgs: compiled.appArgs ? redactInvocationArgs(compiled.appArgs) : undefined,
    };
  }
  return { ...compiled };
}

function redactCompiledJob(
  compiled: CompiledAgentBrowserJob | undefined,
): CompiledAgentBrowserJob | undefined {
  const redactedSteps = compiled?.steps.map((step) => ({
    ...step,
...
function redactCompiledSourceLookup(
...
function redactCompiledNetworkSourceLookup(
...
export function resolveAgentBrowserInput(options: {
  ...
  const toolArgs =
    compiledElectron || compiledScript
      ? []
      : (compiledSemanticAction?.args ??
        compiledGeneratedBatch?.args ??
        normalizedExplicitArgs.args);
  ...
  const timeoutMsError =
    ...
    (typeof params.timeoutMs !== "number" ||
      ...
      : compiledElectron && params.timeoutMs !== undefined
        ...
        : compiledScript &&
            params.timeoutMs !== undefined &&
            params.timeoutMs > AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS
          ? `script timeoutMs must be ${AGENT_BROWSER_SCRIPT_MAX_TIMEOUT_MS} or less.`
          : undefined;
  const scriptSessionModeError =
    compiledScript && params.sessionMode !== undefined
  ...
  const validationError =
    ...
    (compiledElectron || compiledScript
      ? undefined
      : (validateToolArgs(toolArgs) ?? getBatchPreflightValidationError(toolArgs, toolStdin)));
  ...
  const redactedCompiledNetworkSourceLookup = redactCompiledNetworkSourceLookup(
    compiledNetworkSourceLookup,
  );
  const redactedCompiledQaPreset =
    compiledQaPreset && redactedCompiledJob
