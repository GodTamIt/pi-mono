...
export async function collectNavigationSummary(options: {
  ...
  const urlChanged = options.priorTarget?.url
    ? normalizeComparableUrl(options.priorTarget.url) !== normalizeComparableUrl(url)
    : undefined;
...
function extractScrollPositionSnapshot(data: unknown): ScrollPositionSnapshot | undefined {
  ...
  const containers = Array.isArray(result.containers)
    ? result.containers.flatMap((entry, index): ScrollPositionSnapshot["containers"] => {
        ...
        const id =
          rawId && /^\d+:[a-z][a-z0-9-]*(?:\[role=[a-z-]+\])?$/i.test(rawId)
            ? rawId
            : `sample-${index}`;
        ...
        return scrollTop !== undefined && scrollLeft !== undefined
          ? [{ id, scrollLeft, scrollTop }]
          : [];
      })
    : [];
  return {
    containerCount:
      typeof result.containerCount === "number" ? result.containerCount : containers.length,
    containers,
    innerHeight,
    innerWidth,
    scrollHeight,
...
function extractComboboxFocusDiagnostic(data: unknown): ComboboxFocusDiagnostic | undefined {
  ...
  if (!isRecord(result) || result.comboboxLike !== true || !isRecord(result.activeElement))
    return undefined;
  const visibleListboxCount =
    typeof result.visibleListboxCount === "number" ? result.visibleListboxCount : 0;
  const visibleOptionCount =
    typeof result.visibleOptionCount === "number" ? result.visibleOptionCount : 0;
  const expanded =
    typeof result.activeElement.expanded === "string" ? result.activeElement.expanded : undefined;
  if (
  ...
  return {
    activeElement: {
      expanded,
      hasPopup:
        typeof result.activeElement.hasPopup === "string"
          ? result.activeElement.hasPopup
          : undefined,
      name:
        typeof result.activeElement.name === "string"
          ? redactSensitiveText(result.activeElement.name)
          : undefined,
      role: typeof result.activeElement.role === "string" ? result.activeElement.role : undefined,
      tagName:
        typeof result.activeElement.tagName === "string" ? result.activeElement.tagName : undefined,
    },
    message:
...
function isComboboxFocusDiagnosticCommand(
  ...
  const explicitlyTargetsCombobox = commandTokens.some((token) =>
    /^(?:combobox|listbox)$/i.test(token),
  );
...
function getOverlayBlockerCandidates(snapshotData: unknown): OverlayBlockerCandidate[] {
  ...
  if (!refs) return [];
  const hasOverlayContext = Object.values(refs).some(
    (entry) =>
      isRecord(entry) &&
      OVERLAY_CONTEXT_ROLES.has((typeof entry.role === "string" ? entry.role : "").toLowerCase()),
  );
...
export function formatOverlayBlockerText(diagnostic: OverlayBlockerDiagnostic): string {
  return [
    ...
    ...diagnostic.candidates.map(
      (candidate) =>
        `- ${candidate.ref}${candidate.role ? ` ${candidate.role}` : ""}${candidate.name ? ` ${JSON.stringify(candidate.name)}` : ""}: ${candidate.reason}`,
    ),
  ].join("\n");
}

export function buildOverlayBlockerNextActions(options: {
  diagnostic: OverlayBlockerDiagnostic;
  sessionName?: string;
}): AgentBrowserNextAction[] {
  return [
    {
...
export async function collectOverlayBlockerDiagnostic(options: {
  ...
  if (
    ...
    typeof options.data.clicked !== "string"
  )
    return undefined;
...
function parseSelectorTextVisibilityCandidates(
  ...
  const candidates = value.flatMap(
    (entry): NonNullable<SelectorTextVisibilityDiagnostic["visibleCandidates"]> => {
      ...
      const textPreview =
        typeof entry.textPreview === "string" && entry.textPreview.length > 0
          ? redactSensitiveText(entry.textPreview)
          : undefined;
      return [
        {
          index: entry.index,
...
function parseSelectorTextVisibilityProbe(
  ...
  return {
    ...
    firstVisibleTextPreview:
      typeof parsed.firstVisibleTextPreview === "string" &&
      parsed.firstVisibleTextPreview.length > 0
        ? redactSensitiveText(parsed.firstVisibleTextPreview)
        : undefined,
    matchCount,
    selector,
...
async function collectSelectorTextVisibilityDiagnosticForSelector(options: {
  ...
  const summary =
    parsed.firstMatchVisible === false
...
function getSuccessfulGetTextSelectors(options: {
  ...
  return options.commandInfo.command === "get" && options.commandInfo.subcommand === "text"
    ? [options.commandTokens[2]].filter(
...
export function formatSelectorTextVisibilityText(
  diagnostics: SelectorTextVisibilityDiagnostic[],
): string | undefined {
  ...
  return diagnostics
    .flatMap((diagnostic, index) => {
      ...
      if (diagnostic.firstVisibleTextPreview)
        lines.push(
          `First visible text preview: ${JSON.stringify(diagnostic.firstVisibleTextPreview)}`,
        );
      if (diagnostic.visibleCandidates && diagnostic.visibleCandidates.length > 0) {
        ...
        for (const candidate of diagnostic.visibleCandidates) {
          ...
          const previewPart = candidate.textPreview
...
export function formatElectronBroadGetTextScopeText(
  diagnostics: ElectronBroadGetTextScopeDiagnostic[],
): string | undefined {
...
export async function getArtifactCleanupGuidance(options: {
  ...
  for (const entry of explicitEntries) {
    ...
    const absolutePath =
      entry.absolutePath ??
      (isAbsolute(entry.path) ? entry.path : resolve(options.cwd, entry.path));
    try {
...
export async function collectElectronManagedSessionTarget(options: {
  ...
  const title = boundElectronProbeString(
    extractStringResultField(titleResult.data, "result") ??
      extractStringResultField(titleResult.data, "title"),
    160,
  );
  return {
    sessionName: options.sessionName,
...
export async function collectQaAttachedTarget(options: {
  ...
  if (options.currentTarget?.title || options.currentTarget?.url)
    return {
      sessionName: options.sessionName,
      title: options.currentTarget.title,
      url: options.currentTarget.url,
    };
...
export function formatQaAttachedTargetText(
  ...
  return ["QA attached target:", target.sessionName, target.title, target.url]
    .filter((part): part is string => typeof part === "string" && part.length > 0)
    .join(" — ");
}

export function buildQaAttachedRecoveryNextActions(
  sessionName: string | undefined,
): AgentBrowserNextAction[] {
...
function getTopLevelFillInvocation(
  commandTokens: string[],
): { expected: string; refId?: string; selector: string } | undefined {
  ...
  return selector && expected.length > 0
    ? { expected, ...(refId ? { refId } : {}), selector }
    : undefined;
}

function shouldVerifyContenteditableFill(
  fill: { refId?: string } | undefined,
...
export async function collectFillVerificationDiagnostic(options: {
  commandTokens: string[];
  cwd: string;
  forceValueVerification?: boolean;
  namespace?: string;
  refSnapshot?: SessionRefSnapshot;
  sessionName?: string;
  signal?: AbortSignal;
  ...
  const actualPreview =
    actual.length > 0 ? `"${boundElectronProbeString(actual, 80)}"` : `an empty ${method}`;
  ...
  diagnostic.nextActionIds = buildFillVerificationNextActions(diagnostic, options.sessionName).map(
    (action) => action.id,
  );
  return diagnostic;
}
...
export async function collectElectronHandoff(options: {
  ...
  return {
    ...
    refSnapshot,
    snapshot,
    ...(snapshotRetryCount > 0 ? { snapshotRetryCount } : {}),
    tabs,
  };
}

function getTimeoutProgressSteps(
  compiledJob: CompiledAgentBrowserJob | undefined,
  command: string | undefined,
  stdin: string | undefined,
): Array<{ args: string[]; generatedFrom?: string; index: number }> {
  if (compiledJob)
    return compiledJob.steps.map((step, index) => ({
      args: step.args,
      generatedFrom: step.generatedFrom,
      index: index + 1,
    }));
  if (command !== "batch" || !stdin) return [];
  return parseValidBatchStepEntries(stdin).map(({ index, step }) => ({
    args: step,
    index: index + 1,
  }));
}
...
function getPlannedCurrentPageUrl(
  steps: Array<{ args: string[]; index: number }>,
): string | undefined {
...
function currentUrlMatchesNavigationStep(
...
function buildTimeoutProgressSteps(options: {
  ...
  const progressSteps = options.steps.map((step): TimeoutProgressStep => {
    ...
    const command = step.args[0];
    const navigationUrl =
      isOpenNavigationCommand(command) || command === "pushstate"
        ? getLastPositionalToken(step.args)
        : undefined;
    if (stepArtifacts.some((artifact) => artifact.exists)) {
      return {
        ...step,
        reason: "Declared artifact exists on disk after timeout.",
        status: "completed",
      };
    }
    if (
      options.currentPageSource === "live" &&
      currentUrlMatchesNavigationStep(options.currentPageUrl, navigationUrl)
    ) {
    ...
    return {
      ...
      reason:
        stepArtifacts.length > 0
          ? "Declared artifact was not present when the watchdog fired."
          : undefined,
      status: "unknown",
    };
  });
  const highestCompletedIndex = Math.max(
    0,
    ...progressSteps.filter((step) => step.status === "completed").map((step) => step.index),
  );
  for (const step of progressSteps) {
    if (step.status === "unknown" && step.index < highestCompletedIndex) {
      step.status = "completed";
      step.reason =
        "Later step completion evidence indicates the batch advanced past this step before timeout.";
    }
  }
  for (const step of progressSteps) {
    const command = step.args[0];
    ...
    if (!retryStep) {
      ...
      retryStep = {
        ...
        reason:
          step.reason ??
          (retry
            ? "Likely active when the wrapper watchdog fired."
            : "Likely active when the wrapper watchdog fired; executable retry omitted because this step may have already mutated page state."),
        retry,
        status: "failed",
      };
  ...
  return {
    openedButPostOpenTimedOut:
      lastCompletedNavigationIndex !== undefined &&
      retryStep !== undefined &&
      retryStep.index > lastCompletedNavigationIndex,
    retryStep,
    steps: progressSteps,
  };
}
...
export async function collectTimeoutPartialProgress(options: {
  ...
  const currentPageSource = recoveredUrl
    ? ("live" as const)
    : plannedUrl
      ? ("planned" as const)
      : title
        ? ("live" as const)
        : undefined;
  const stepProgress = buildTimeoutProgressSteps({
    artifacts,
    currentPageSource: recoveredUrl ? "live" : undefined,
    currentPageUrl: recoveredUrl,
    steps: rawSteps,
  });
  ...
  const pageStateSummary =
    recoveredUrl || title ? " and current page state" : plannedUrl ? " and planned page URL" : "";
  const retrySummary = stepProgress.retryStep
    ? ` Retry step ${stepProgress.retryStep.index} is the first incomplete step.`
    : "";
  return {
    artifacts,
    currentPage: url || title ? { source: currentPageSource, title, url } : undefined,
    liveUrlRecovered: recoveredUrl !== undefined,
    openedButPostOpenTimedOut: stepProgress.openedButPostOpenTimedOut,
    retryStep: stepProgress.retryStep,
    steps: stepProgress.steps.length > 0 ? stepProgress.steps : undefined,
    summary: `Timed out before upstream returned final results; recovered ${completedSteps}/${rawSteps.length} planned step state${rawSteps.length === 1 ? "" : "s"} and ${foundArtifacts}/${artifacts.length} declared artifact path${artifacts.length === 1 ? "" : "s"}${pageStateSummary}.${retrySummary}`,
  };
}
  ...
  return path
    ...
    .map((segment) =>
      segment === "/" || segment === "\\" || /^[/\\]+$/.test(segment)
        ? segment
        : redactSensitiveText(segment) !== segment ||
...
function sanitizeCurrentPageUrlForTimeoutDiagnostic(url: string): string {
  try {
    ...
    parsedUrl.pathname = parsedUrl.pathname
      .split("/")
      .map((segment) => redactSensitivePathSegmentsForDiagnostic(segment))
      .join("/");
...
export function formatTimeoutPartialProgressText(progress: TimeoutPartialProgress): string {
  ...
  const currentPageTitle = progress.currentPage?.title
    ? redactSensitivePathSegmentsForDiagnostic(redactSensitiveText(progress.currentPage.title))
    : undefined;
  const currentPageUrl = progress.currentPage?.url
    ? sanitizeCurrentPageUrlForTimeoutDiagnostic(progress.currentPage.url)
    : undefined;
  ...
  if (progress.steps && progress.steps.length > 0) {
    ...
    for (const step of shownSteps) {
      ...
      lines.push(
        `- Step ${step.index} [${step.status}${generatedFrom}]: ${commandText}${step.reason ? ` — ${redactSensitivePathSegmentsForDiagnostic(redactSensitiveText(step.reason))}` : ""}`,
      );
    }
    if (progress.steps.length > shownSteps.length)
      lines.push(
        `- ... ${progress.steps.length - shownSteps.length} more step${progress.steps.length - shownSteps.length === 1 ? "" : "s"} omitted`,
      );
  }
  if (progress.retryStep?.retry?.args) {
    lines.push(
      `Retry failed step: ${JSON.stringify({ args: redactInvocationArgs(progress.retryStep.retry.args) })}`,
    );
  }
  for (const artifact of progress.artifacts)
    lines.push(
      `Artifact from step ${artifact.stepIndex}: ${redactSensitivePathSegmentsForDiagnostic(artifact.path)} (${artifact.exists ? `exists${typeof artifact.sizeBytes === "number" ? `, ${artifact.sizeBytes} bytes` : ""}` : "missing"})`,
    );
  return lines.join("\n");
}
