...
function findFirstPositionalArgument(commandTokens: string[]): string | undefined {
  for (let index = 1; index < commandTokens.length; index += 1) {
    ...
    if (
      ...
      ["true", "false"].includes(commandTokens[index + 1] ?? "")
    ) {
      index += 1;
      continue;
    }
...
function readUsesActivePageUrl(commandTokens: string[]): boolean {
  return (
    findFirstPositionalArgument(commandTokens) === undefined &&
    commandTokens.some(
      (token) => token === "--require-md" || token === "--llms" || token.startsWith("--llms="),
    )
  );
}
...
function commandTimeoutBudgetMs(
  ...
  return commandTokens[0] === "read"
    ? timeoutMs * readRequestBudget(commandTokens, activePageUrl)
    : timeoutMs;
}
