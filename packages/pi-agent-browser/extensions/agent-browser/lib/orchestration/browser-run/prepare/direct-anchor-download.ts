...
function getDirectDownloadRequest(
...
export async function tryDirectAnchorDownload(options: {
  ...
  try {
    ...
    if (
      !isRecord(probe) ||
      probe.status !== "fetched-anchor" ||
      typeof probe.href !== "string" ||
      typeof probe.pageUrl !== "string" ||
      typeof probe.bodyBase64 !== "string"
    )
      return undefined;
    ...
    if (
      !isLoopbackHttpUrl(pageUrl) ||
      !isLoopbackHttpUrl(href) ||
      !isLoopbackHttpUrl(responseUrl) ||
      href.origin !== pageUrl.origin ||
    ...
    const mediaType =
      typeof probe.contentType === "string" && probe.contentType.length > 0
        ? probe.contentType
        : undefined;
    ...
    const artifactRetentionSummary = artifactManifest
    ...
    const savedFile = {
      command: "download" as const,
      kind: "download" as const,
      metadata: {
        download: probe.download,
        href: redactSensitiveText(href.href),
        method: "direct-anchor-fetch",
      },
      path: absolutePath,
    };
    return {
