...
function attachRenderedTextMatchRefs(
  matches: RenderedTextSearchMatch[],
...
function formatRenderedTextSearchMatches(
...
function buildSnapshotDiff(
  ...
  if (!previous)
    return {
      addedRefs: Object.keys(currentRefs),
      changedRefs: [],
      removedRefs: [],
      summary: `Snapshot diff: no previous snapshot; ${Object.keys(currentRefs).length} current refs recorded.`,
      unchangedRefs: 0,
    };
...
function filterSnapshotData(
  ...
  const visibleLines = lines.filter((line) => {
    ...
    return [...matchingRefIds].some(
      (refId) => line.includes(`[ref=${refId}]`) || line.includes(`ref=${refId}`),
    );
  });
  const filteredRefs = Object.fromEntries(
    Object.entries(refs).filter(([refId]) => matchingRefIds.has(refId)),
  );
  const description = [
    request.role ? `role=${request.role}` : undefined,
    request.search ? `search=${JSON.stringify(request.search)}` : undefined,
  ]
...
export async function trySnapshotFilter(options: {
  ...
  const filtered =
    request.role || request.search
      ? filterSnapshotData(snapshotData, request)
      : isRecord(snapshotData)
        ? {
            data: snapshotData,
            matchedRefs: isRecord(snapshotData.refs) ? Object.keys(snapshotData.refs).length : 0,
            totalLines:
              typeof snapshotData.snapshot === "string"
                ? snapshotData.snapshot.split(/\r?\n/).filter((line) => line.length > 0).length
                : 0,
            totalRefs: isRecord(snapshotData.refs) ? Object.keys(snapshotData.refs).length : 0,
            visibleLines:
              typeof snapshotData.snapshot === "string"
                ? snapshotData.snapshot.split(/\r?\n/).filter((line) => line.length > 0).length
                : 0,
  ...
  if (renderedTextSearch)
    renderedTextSearch.matches = attachRenderedTextMatchRefs(
      renderedTextSearch.matches,
      snapshotData,
    );
  ...
  const presentation = await buildSnapshotPresentation(
    filtered.data,
  ...
  const prefix = [summary, renderedTextSearchText, diffText, viewportText]
        ...
        snapshotFilter:
          request.role || request.search
            ? {
                ...
                matchedRefs: filtered.matchedRefs,
                renderedTextMatches: renderedTextSearch?.matches,
                renderedTextTotalMatches: renderedTextSearch?.totalMatches,
                renderedTextTruncated: renderedTextSearch?.truncated,
                role: request.role,
                search: request.search,
                totalLines: filtered.totalLines,
                totalRefs: filtered.totalRefs,
                visibleLines: filtered.visibleLines,
              }
            : undefined,
