...
export async function acquireOwnedManagedSessionDaemonPolicy(options: {
	...
	if (!lock) {
		return signal?.aborted
			? {}
			: {
				error: "Managed-session policy coordination is unavailable or busy. Retry after the current operation finishes, repair the private policy-lock directory, and on POSIX verify that ps is available on PATH.",
			};
	}
