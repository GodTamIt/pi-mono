...
import {
  ...
  formatClickDispatchDiagnosticText,
...
export function formatSemanticActionCandidateText(
  actions: AgentBrowserNextAction[],
): string | undefined {
  const candidateActions = actions.filter(
    (action) => SEMANTIC_ACTION_CANDIDATE_ACTION_IDS.has(action.id) && action.params?.args,
  );
  if (candidateActions.length === 0) return undefined;
  return [
    "Agent-browser candidate fallbacks:",
    ...candidateActions.map(
      (action) =>
  ...
  if (locator === "text" && compiled.action === "click") {
    return [
      buildRoleCandidate(
        "button",
        "try-button-name-candidate",
        "Retry against a button with the same accessible name when text lookup misses.",
      ),
      buildRoleCandidate(
        "link",
        "try-link-name-candidate",
        "Retry against a link with the same accessible name when text lookup misses.",
      ),
    ];
  }
  return [];
...
export function redactToolDetails(
  ...
  return redactSensitiveValue(redactExactSensitiveValue(details, sensitiveValues)) as Record<
    string,
    unknown
  >;
}
...
export function redactRecoveryHint(
  recoveryHint: AgentBrowserExecutionPlan["recoveryHint"] | undefined,
): AgentBrowserExecutionPlan["recoveryHint"] | undefined {
...
export function buildJsonVisibleContent(options: {
  ...
  if (
    ...
    typeof presentation.data.wsUrl === "string"
  )
    payload.data.wsUrl = presentation.data.wsUrl;
  const images = presentation.content.filter(
...
function formatElectronLaunchFailureDiagnostics(
  failure: ElectronLaunchFailure | undefined,
): string | undefined {
  ...
  if (diagnostics.pid !== undefined)
    lines.push(
      `- PID: ${diagnostics.pid} (${diagnostics.pidAlive === undefined ? "state unknown" : diagnostics.pidAlive ? "alive before cleanup" : "not alive before cleanup"}).`,
    );
  if (diagnostics.exitCode !== undefined || diagnostics.exitSignal !== undefined) {
    const exitParts = [
      diagnostics.exitCode !== undefined ? `code ${diagnostics.exitCode}` : undefined,
      diagnostics.exitSignal ? `signal ${diagnostics.exitSignal}` : undefined,
    ]
      .filter(Boolean)
      .join(", ");
...
export function buildElectronHostFailureResult(options: {
  ...
  const text = [
    ...
    options.launchFailure?.cleanupError
...
export function formatElectronLaunchText(options: {
  ...
  if (options.handoff?.handoff === "snapshot")
    lines.push(
      options.handoff.refSnapshot && options.handoff.refSnapshot.refIds.length > 0
    ...
    lines.push(
      "Tabs handoff completed: safer diagnostic starting point; no interactive refs were captured.",
    );
...
export function buildRedactedPresentationContent(options: {
  ...
  const contentWithSessionWarnings =
    userRequestedJson && !plainTextInspection
...
export async function prepareFinalResultRecoveryState(options: {
  ...
  if (categoryDetails.failureCategory === "selector-not-found") {
    const selectorRecoveryCommandTokens =
      options.presentation.batchFailure?.failedStep.command ?? options.commandTokens;
    visibleRefFallbackDiagnostic = await collectVisibleRefFallbackDiagnostic({
      commandTokens: selectorRecoveryCommandTokens,
  ...
  const richInputRecoveryDiagnostic = buildRichInputRecoveryDiagnostic(
    visibleRefFallbackDiagnostic,
  );
  const noActivePageSnapshotFailure =
    categoryDetails.resultCategory === "failure" &&
    (isNoActivePageSnapshotFailure(
  ...
  const executionSessionKey = getSessionContextKey(
    options.executionPlan.sessionName,
    options.executionPlan.namespace,
  );
  if (noActivePageSnapshotFailure && executionSessionKey) {
    const refUpdate = options.sessionPageState.applyRefSnapshotInvalidation({
      invalidation: buildNoActivePageRefSnapshotInvalidation(),
      sessionName: executionSessionKey,
      update: options.sessionPageStateUpdate,
    });
    currentRefSnapshot = refUpdate.refSnapshot;
    currentRefSnapshotInvalidation = refUpdate.refSnapshotInvalidation;
...
function buildTimeoutPartialProgressNextActions(
  options: FinalResultInput,
): AgentBrowserNextAction[] {
  ...
  return [
    {
      ...
      params: {
        args: withOptionalSessionArgs(options.executionPlan.sessionName, ["snapshot", "-i"]),
      },
...
function buildResultNextActions(options: FinalResultInput): AgentBrowserNextAction[] | undefined {
  ...
  if (options.electronPostCommandHealth) {
    const electronRecord = options.electronLaunchRecords.get(
      options.electronPostCommandHealth.launchId,
    );
    if (electronRecord) appendUnique(buildElectronLifecycleNextActions(electronRecord));
  }
  if (options.electronSessionMismatch) {
    const electronRecord = options.electronLaunchRecords.get(
      options.electronSessionMismatch.launchId,
    );
    if (electronRecord)
      appendUnique(
        buildElectronMismatchNextActions(
          electronRecord,
          options.electronSessionMismatch.liveTarget,
        ),
      );
  }
  if (
    ...
    const candidateActions = buildSemanticActionCandidateActions(
      options.redactedCompiledSemanticAction,
    );
  ...
  if (options.fillVerificationDiagnostic)
    appendUnique(
      buildFillVerificationNextActions(
        options.fillVerificationDiagnostic,
        options.executionPlan.sessionName,
      ),
    );
...
function formatFailureNextActionsText(
  options: FinalResultInput,
...
function buildAgentBrowserResultDetails(
  options: FinalResultInput,
  nextActions: AgentBrowserNextAction[] | undefined,
): Record<string, unknown> {
  const publicVisibleRefFallbackDiagnostic = options.visibleRefFallbackDiagnostic
    ? sanitizeVisibleRefFallbackDiagnostic(options.visibleRefFallbackDiagnostic)
    : undefined;
  const rawPageChangeSummary =
  ...
  return {
    ...
    artifactRetentionSummary:
      options.presentation.artifactRetentionSummary ??
    ...
    electron: options.electronLaunchRecord
      ? {
          ...
          status: options.succeeded ? ("succeeded" as const) : ("failed" as const),
    ...
    fullOutputPath:
      options.parseFailureOutput.fullOutputPath ?? options.presentation.fullOutputPath,
...
export function buildFinalAgentBrowserToolResult(
  ...
  const semanticActionCandidateText = nextActions
    ? formatSemanticActionCandidateText(nextActions)
    : undefined;
  const clickDispatchText = options.clickDispatchDiagnostic
    ? formatClickDispatchDiagnosticText(options.clickDispatchDiagnostic)
    : undefined;
  const overlayBlockerText = options.overlayBlockerDiagnostic
  ...
  const electronRefFreshnessText = formatElectronRefFreshnessText(
    options.electronRefFreshnessDiagnostic,
  );
  const selectorTextVisibilityText = formatSelectorTextVisibilityText(
    options.selectorTextVisibilityDiagnostics,
  );
  const electronBroadGetTextScopeText = formatElectronBroadGetTextScopeText(
    options.electronBroadGetTextScopeDiagnostics,
  );
  const scrollNoopDiagnosticText = formatScrollNoopDiagnosticText(options.scrollNoopDiagnostic);
  const comboboxFocusDiagnosticText = formatComboboxFocusDiagnosticText(
    options.comboboxFocusDiagnostic,
  );
  const recordingDependencyWarningText = formatRecordingDependencyWarningText(
    options.recordingDependencyWarning,
  );
  ...
  const timeoutPartialProgressText = options.timeoutPartialProgress
    ? formatTimeoutPartialProgressText(options.timeoutPartialProgress)
    : undefined;
  ...
  const rawAppendedDiagnosticText = [
    visibleRefFallbackText,
    richInputRecoveryText,
    semanticActionCandidateText,
    clickDispatchText,
    overlayBlockerText,
    fillVerificationText,
    electronRefFreshnessText,
    selectorTextVisibilityText,
    electronBroadGetTextScopeText,
    scrollNoopDiagnosticText,
    comboboxFocusDiagnosticText,
    recordingDependencyWarningText,
    evalStdinHintText,
    evalResultWarningText,
    artifactCleanupText,
    timeoutPartialProgressText,
    managedSessionOutcomeText,
    failureNextActionsText,
  ]
    .filter((item): item is string => item !== undefined)
    .join("\n\n");
  const appendedDiagnosticText = redactSensitiveText(
    redactExactSensitiveText(rawAppendedDiagnosticText, options.exactSensitiveValues),
  );
  const shouldAppendDiagnosticText =
    appendedDiagnosticText.length > 0 &&
    (!options.userRequestedJson || options.plainTextInspection);
  let content =
    shouldAppendDiagnosticText && options.redactedContent[0]?.type === "text"
      ? [
          {
            ...options.redactedContent[0],
            text: `${options.redactedContent[0].text}\n\n${appendedDiagnosticText}`,
          },
          ...options.redactedContent.slice(1),
        ]
      : options.redactedContent;
  ...
  const result = {
    content,
    details: redactToolDetails(details, options.exactSensitiveValues),
    isError: !options.succeeded,
  };
  return options.compiledNetworkSourceLookup
    ? (redactNetworkSourceLookupSurface(result) as typeof result)
    : result;
}
...
export async function buildMissingBinaryFailureResult(options: {
  ...
  const managedSessionRecoveryNextActions =
    applyNamespaceToNextActions(
  ...
  const textParts = [
    errorText,
    managedSessionOutcomeText,
    missingBinaryElectronCleanup
      ? `Electron cleanup after failed attach: ${missingBinaryElectronCleanup.summary}`
      : undefined,
  ...
  return {
    ...
    details: {
      ...
      namespace: options.executionPlan.namespace,
      nextActions:
        managedSessionRecoveryNextActions.length > 0
          ? managedSessionRecoveryNextActions
          : undefined,
      sessionMode: options.sessionMode,
      sessionTabCorrection: options.sessionTabCorrection,
      ...buildAgentBrowserResultCategoryDetails({
