...
export function buildManagedSessionOutcome(options: {
  ...
  if (isCloseCommand(command)) {
    ...
    status = "preserved";
    summary =
      sessionMode === "fresh" && attemptedSessionName !== previousSessionName
...
export function formatManagedSessionOutcomeText(
  ...
  if (outcome.replacedSessionClosed === false) {
    ...
    return outcome.succeeded
      ...
      : [
          formatManagedSessionOutcomeHeadline(outcome),
          formatManagedSessionOutcomeRecoveryGuidance(outcome),
          cleanupWarning,
        ].join("\n");
  }
...
export function buildManagedSessionFreshFailureNextActions(
  ...
  if (
    ...
    const currentSessionArgs = (args: string[]) =>
      withOptionalNamespaceArgs(
        outcome.currentSessionNamespace,
        withOptionalSessionArgs(outcome.currentSessionName, args),
      );
    actions.push(
      buildNextToolAction({
...
export function buildAboutBlankRecoveryHint(): string {
  return "agent_browser detected that the active tab became about:blank while this session still had a prior intended tab. Run tab list for this session and re-select the intended tab, or retry with sessionMode:fresh if the tab is gone.".replace(
    "sessionMode:fresh",
    "sessionMode=fresh",
  );
}
...
export function getGuardedRefUsage(
  ...
  stdin?: string,
  options: { includeRefsAfterBatchSnapshot?: boolean } = {},
): string[] {
  const collectFromStep = (step: readonly string[]) =>
    isRefGuardedCommand(step[0]) ? collectRefsFromTokens(step) : [];
  if (commandTokens[0] !== "batch") {
...
export function buildStaleRefPreflight(options: {
  ...
  const usedRefIds = options.refSnapshotInvalidation
    ? [
        ...new Set(
          getGuardedRefUsage(options.commandTokens, options.stdin, {
  ...
  if (options.refSnapshotInvalidation) {
    return {
      message:
        options.refSnapshotInvalidation.reason === "page-transition"
...
export function buildPinnedBatchPlan(options: {
  ...
  const includeNavigationSummary = isNavigationObservableCommandName(
    options.command,
    options.commandTokens[1],
  );
  const tabSelectionStep: BatchCommandStep = ["tab", options.selectedTab];
...
function selectAnySessionTargetTab(options: {
  ...
  const targetUrl =
    typeof options.target.url === "string" ? normalizeComparableUrl(options.target.url) : undefined;
  if (!targetUrl) return undefined;
  const matchingTabs = options.tabs.filter(
    (tab) => normalizeComparableUrl(tab.url ?? "") === targetUrl,
  );
  const targetTitle = options.target.title?.trim() ?? "";
  const selectedTab =
    (targetTitle ? matchingTabs.find((tab) => tab.title?.trim() === targetTitle) : undefined) ??
    matchingTabs[0];
  const selection = selectedTab ? getTabSelection(selectedTab) : undefined;
  return selection
    ? { ...selection, ...(targetTitle ? { targetTitle } : {}), targetUrl }
    : undefined;
}
...
export function unwrapPinnedSessionBatchEnvelope(options: {
  ...
  if (!Array.isArray(options.envelope.data)) {
    return {
      parseError:
        "agent-browser returned an unexpected response while applying the wrapper's tab-pinning batch.",
    };
  }
  ...
  const navigationSummary = normalizeSessionTabTarget(
    extractNavigationSummaryFromData(navigationSummaryStep?.result),
  );
  return {
    envelope: {
...
export async function runSessionCommandData(options: {
  ...
  const processResult = await runAgentBrowserProcess({
    args: [
      "--json",
      ...(namespace !== undefined || pinNamespace ? ["--namespace", namespace ?? ""] : []),
      "--session",
      sessionName,
      ...args,
    ],
  ...
  try {
    ...
    if (parsed.parseError || parsed.envelope?.success === false) {
      if (throwOnFailure)
        throw new Error(
          parsed.parseError
            ? "agent-browser returned invalid structured output"
...
export async function collectOpenResultTabCorrection(options: {
  ...
  const tabData = await runSessionCommandData({
    args: ["tab", "list"],
    cwd,
    namespace,
    sessionName,
...
function mapTabData(tabData: unknown):
  | Array<{
      active?: boolean;
      index?: number;
      label?: string;
...
export function getActiveElectronRecords(
  ...
  return [...records.values()].filter(
    (record) =>
      record.cleanupState === "active" ||
      record.cleanupState === "dead" ||
      record.cleanupState === "partial" ||
...
export function buildElectronSessionMismatch(options: {
  ...
  if (isAboutBlankUrl(options.managedSession.url)) {
    ...
    options.record.sessionName &&
    options.record.sessionName !== options.managedSession.sessionName
  ) {
  ...
  const managedDescription =
    options.managedSession.url ??
    options.managedSession.title ??
    options.managedSession.sessionName;
...
export function buildElectronPostCommandHealthDiagnostic(options: {
  ...
  if (options.status.pidAlive === false) reason = "process-dead";
    ...
    isAboutBlankUrl(options.target?.url) &&
    getLiveElectronRendererTargets(options.status.targets).length === 0
  )
...
export function formatElectronPostCommandHealthText(
  ...
  lines.push(
    `Status: ${diagnostic.status.portAlive ? "debug port alive" : "debug port dead"}${diagnostic.status.pidAlive === undefined ? "" : diagnostic.status.pidAlive ? ", pid alive" : ", pid dead"}; ${diagnostic.status.targets.length} CDP target(s).`,
  );
  lines.push(
...
export function buildElectronIdentifiers(record: ElectronLaunchRecord): {
  appName: string;
  launchId: string;
  sessionName?: string;
} {
  ...
  return [
    {
      ...
      params: {
        args: sessionName ? ["--session", sessionName, "snapshot", "-i"] : ["snapshot", "-i"],
      },
      reason:
