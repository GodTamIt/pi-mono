...
function deleteNamespaceEntries(
  ...
  try {
    ...
    if (prepared.pinnedBatchUnwrapMode) {
      const pinnedBatchResult = unwrapPinnedSessionBatchEnvelope({
        ...
        includeNavigationSummary: prepared.includePinnedNavigationSummary,
        mode: prepared.pinnedBatchUnwrapMode,
      });
    ...
    const processSucceeded =
      !processResult.aborted && !processResult.spawnError && processResult.exitCode === 0;
    ...
    if (
      ...
      (shouldCaptureNavigationSummary(
        ...
        (prepared.executionPlan.commandInfo.command === "tab" &&
          prepared.executionPlan.commandInfo.subcommand === "close"))
    ) {
    ...
    > = [];
    const timeoutPartialProgress = processResult.timedOut
    ...
    if (sessionStateKey) {
      const sessionClosed =
        (isCloseCommand(prepared.executionPlan.commandInfo.command) && succeeded) ||
        nestedBatchClosed;
      if (sessionClosed) {
        ...
        const pageTransitionInvalidation = unsettledWebMcpMutation
          ...
          : processResult.agentBrowserStarted &&
            ...
            : failedTransitionReverification
              ? buildPageTransitionRefSnapshotInvalidation(
                  "A failed eval/back/forward/reload/connect/state-load/tab command may still have changed the page, so the prior snapshot refs were invalidated. Run snapshot -i before using page-scoped refs.",
                )
        ...
        if (currentSessionTabTarget) {
          ...
          processResult.agentBrowserStarted &&
          (resultingPageState.pageUrlUnknown || resultingPageState.pageTargetMayHaveChanged)
        ) {
          ...
          : prepared.executionPlan.commandInfo.command === "batch"
            ...
            : succeeded
              ? prepared.executionPlan.commandInfo.command === "snapshot"
                ...
                : (prepared.resolvedSemanticActionRefSnapshot ?? overlayBlockerDiagnostic?.snapshot)
              : undefined;
        if (refSnapshot) {
    ...
    const commandClosesSession =
      isCloseCommand(prepared.executionPlan.commandInfo.command) ||
      nestedBatchClosed ||
      closeAllClosesPriorManagedSession;
    ...
    let errorText = getAgentBrowserErrorText({
      ...
      wrapperRecoveryHint: buildWrapperRecoveryHint({
        pinnedBatchUnwrapMode: prepared.pinnedBatchUnwrapMode,
        sessionTabCorrection,
      }),
    });
    if (errorText) {
    ...
    const sourceLookup = prepared.compiledSourceLookup
      ? await analyzeSourceLookupResults(
          ...
          cwd,
          { electronContext: sourceLookupElectronContext, workspaceRoot: cwd },
        )
      : undefined;
    ...
    const result = buildFinalAgentBrowserToolResult({
      ...
      inspectionText,
      managedSessionHeadedAutosaveDisabled: resultHeadedManagedAutosaveDisabled || undefined,
