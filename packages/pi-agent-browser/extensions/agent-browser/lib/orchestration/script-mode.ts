...
export function getScriptSessionLeasesFromBranch(
  ...
  for (const entry of branch) {
    if (
      !isRecord(entry) ||
      entry.type !== "custom" ||
      entry.customType !== SCRIPT_SESSION_ENTRY_TYPE ||
      !isRecord(entry.data)
    )
      continue;
    ...
    leases.set(sessionName, {
      cleanup,
      closeCommandArgs: expectedCloseCommandArgs,
      launchAttempted: true,
      sessionName,
    });
  }
...
async function readVerifiedScriptSpill(
...
function getScriptCompatibleNextActions(
  value: unknown,
  ...
  const actions = value.flatMap((action): AgentBrowserNextAction[] => {
    ...
    if (!isRecord(action.params))
      return typeof action.artifactPath === "string"
        ? [action as unknown as AgentBrowserNextAction]
        : [];
    if (Object.keys(action.params).some((key) => key !== "args" && key !== "stdin")) return [];
    if (
      !Array.isArray(action.params.args) ||
      action.params.args.some((token) => typeof token !== "string")
    )
      return [];
...
function scriptBrowserEnvelopeFitsIpc(envelope: AgentBrowserScriptBrowserEnvelope): boolean {
  try {
    return (
      Buffer.byteLength(
        JSON.stringify({ envelope, id: Number.MAX_SAFE_INTEGER, type: "response" }),
        "utf8",
      ) +
        1 <=
      AGENT_BROWSER_SCRIPT_IPC_MESSAGE_MAX_BYTES
...
function buildOversizedScriptBrowserEnvelope(): AgentBrowserScriptBrowserEnvelope {
  const text =
    "Browser result exceeds the script IPC response limit; narrow the inner browser call output.";
  return {
    data: null,
...
export async function buildScriptBrowserEnvelope(
  result: AgentBrowserToolResult,
  args: string[],
  scriptSessionName?: string,
  ...
  const resultCategory =
    details?.resultCategory === "failure" || result.isError === true ? "failure" : "success";
  const text = truncateText(
    redactSensitiveText(getToolResultText(result)),
    SCRIPT_BROWSER_TEXT_MAX_CHARS,
  );
  const summary = truncateText(
    ...
    SCRIPT_BROWSER_SUMMARY_MAX_CHARS,
  );
  const compatibleNextActions = getScriptCompatibleNextActions(
    details?.nextActions,
    scriptSessionName,
  );
  const redactedNextActions = getScriptCompatibleNextActions(
    redactPresentationData(commandInfo, compatibleNextActions),
    undefined,
  );
  const failureCategory =
    resultCategory === "failure" && typeof details?.failureCategory === "string"
      ? (details.failureCategory as AgentBrowserScriptBrowserEnvelope["failureCategory"])
      : undefined;
  const successCategory =
    resultCategory === "success" && typeof details?.successCategory === "string"
  ...
  const envelope: AgentBrowserScriptBrowserEnvelope = {
    ...
    failureCategory,
    nextActions: Array.isArray(redactedNextActions)
      ? (redactedNextActions as AgentBrowserNextAction[])
      : undefined,
...
function collectUniqueArtifacts(
  results: AgentBrowserToolResult[],
  ...
  const unique = new Map(
    records.map((record) => [
      String(record.absolutePath ?? record.path ?? JSON.stringify(record)),
      record,
    ]),
  );
...
function collectScriptArtifactVerification(
  results: AgentBrowserToolResult[],
): ArtifactVerificationSummary | undefined {
  const entries = results.flatMap((result) => {
    const details = isRecord(result.details) ? result.details : undefined;
    const verification = isRecord(details?.artifactVerification)
  ...
  const unique = [
    ...new Map(
      entries.map((entry) => [
        String(entry.absolutePath ?? entry.path ?? JSON.stringify(entry)),
        entry,
      ]),
    ).values(),
  ];
...
function buildScriptCloseNextAction(sessionName: string): AgentBrowserNextAction {
  return {
    ...
    reason:
      "Retry closing the isolated wrapper-owned script session after automatic cleanup failed.",
    safety:
      "Use these exact args; do not add profile, state, restore, namespace, or connection flags.",
    tool: "agent_browser",
  };
}
...
export function buildScriptToolResult(options: {
  ...
  try {
    ...
    if (
      ...
      Buffer.byteLength(serializedData, "utf8") > AGENT_BROWSER_SCRIPT_FINAL_OUTPUT_MAX_BYTES
    ) {
      throw new RangeError(
        `Redacted script output exceeds ${AGENT_BROWSER_SCRIPT_FINAL_OUTPUT_MAX_BYTES} bytes.`,
      );
    }
  } catch {
    data = undefined;
  ...
  const steps = options.run.steps.map((step) => ({
    ...step,
    summary: redactSensitiveText(step.summary),
  }));
  const rejectedFailureCategory =
    steps.find((step) => step.failureCategory === "policy-blocked")?.failureCategory ??
    (options.run.rejectedCallCount > 0 ? "validation-error" : undefined);
  const failureCategory = options.cleanupError
    ? "cleanup-failed"
    : outputError
      ? "validation-error"
      : (options.run.failureCategory ?? rejectedFailureCategory);
  ...
  const cleanupSuffix =
    options.sessionName && !options.cleanupError ? " Isolated script session closed." : "";
  const summary = `${
    options.cleanupError
      ...
      : outputError
        ...
        : options.run.ok
          ...
          : `Script failed: ${redactSensitiveText(options.run.error ?? "sandbox execution failed")}`
  }${cleanupSuffix}`;
  const dataText = serializedData === undefined ? "" : `\n\n${serializedData}`;
  const cleanupActionText =
    options.cleanupError && options.sessionName
  ...
  const artifactRetentionSummary = getLatestScriptResultDetail(
    options.innerResults,
  ...
  const nextActions =
    options.cleanupError && options.sessionName
      ? [buildScriptCloseNextAction(options.sessionName)]
      : undefined;
  return {
