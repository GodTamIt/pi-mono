...
- Do not block runtime browser calls for version drift. Doctor may recommend the maintained baseline, but drift is advisory.
...
- Local trusted-checkout smoke testing should use explicit CLI loading such as `pi --approve --no-extensions -e .` or `pi --approve --no-extensions -e /absolute/path/to/pi-agent-browser`; Pi settings are bypassed in this mode and code edits require a process restart for validation. Omit `--approve` only when the test is meant to cover Pi's Project Trust prompt.
...
- Always-on model guidance stays lean: only essential workflow invariants are injected each turn, and detailed workflows live in the installed docs the agent reads on demand. This keeps recurring prompt/context overhead bounded.
