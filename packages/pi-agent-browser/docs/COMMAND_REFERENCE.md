...
> Fork note: this file is retained from the pinned [`pi-agent-browser-native`](https://github.com/fitchmultz/pi-agent-browser-native) donor (see [`../THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md)). References to `npm run docs` / `npm run verify` drift checks describe the donor repository, not this fork's scripts.
...
The 0.35.1 baseline is a bug-fix release with no new commands or flags. Runtime calls do not enforce this maintained capability baseline; valid version drift is advisory.
...
`pi-agent-browser` has package-owned config under Pi-scoped paths. This is separate from upstream `agent-browser` config and from Pi package settings:

- global: `~/.pi/config/pi-agent-browser/config.json`
- project-local: `.pi/config/pi-agent-browser/config.json`
...
cat > ~/.pi/config/pi-agent-browser/config.json <<'JSON'
{
  "version": 1,
  "webSearch": {
    "enabled": true,
...
```json
{
  "query": "pi-agent-browser agent_browser_web_search searchType defaults",
