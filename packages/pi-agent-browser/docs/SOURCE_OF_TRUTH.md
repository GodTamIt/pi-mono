...
| Maintainer package validation and stable publication process | [`RELEASE.md`](RELEASE.md) | Package-scoped checks gate each release; publication runs through the root Changesets flow (`release:version`, `release:plan`, `release:publish`) with non-releasing workspaces ignored; donor release evidence in retained docs is historical. |
