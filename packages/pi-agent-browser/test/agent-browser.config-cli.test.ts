...
const DOCUMENTED_CONFIG_HELPER_PREFIX =
  "npm exec --yes --package @ohgodtamit/pi-agent-browser@latest -- pi-agent-browser-config";
...
async function createFixture() {
  ...
  return {
    ...
    globalPath: join(home, ".pi", "config", "pi-agent-browser", "config.json"),
    projectPath: join(cwd, ".pi", "config", "pi-agent-browser", "config.json"),
    root,
  };
}
...
function documentedNpmExecArgs(command: string): { args: string[]; input?: string } {
  ...
  assert.equal(
    ...
    "@ohgodtamit/pi-agent-browser@latest",
...
test("config CLI prints Pi-scoped paths and pass-through setup help", async () => {
  ...
  assert.match(stdout, /\.pi\/config\/pi-agent-browser\/config\.json/);
...
test("published package config docs never document donor package specs or direct helper invocations", async () => {
  ...
  for (const path of markdownFiles) {
    ...
    for (const [lineIndex, line] of text.split("\n").entries()) {
      ...
      if (/package pi-agent-browser-native/.test(line)) {
        violations.push(`${path}:${lineIndex + 1}: ${line.trim()}`);
      }
    }
  }
...
test("config CLI writes project env source, project profile, and project executable path", async () => {
  ...
  const projectPath = join(fixture.cwd, ".pi", "config", "pi-agent-browser", "config.json");
