...
import {
  buildScriptBrowserEnvelope,
  buildScriptToolResult,
} from "../extensions/agent-browser/lib/orchestration/script-mode.js";
...
const resolveInput = (params: Parameters<typeof resolveAgentBrowserInput>[0]["params"]) =>
...
test("script input schema and runtime validation enforce one top-level mode", () => {
  ...
  assert.equal(
    Check(AGENT_BROWSER_PARAMS, { script: "x".repeat(AGENT_BROWSER_SCRIPT_CODE_MAX_BYTES + 1) }),
    false,
  );
  ...
  assert.match(
    compileAgentBrowserScript("💥".repeat(AGENT_BROWSER_SCRIPT_CODE_MAX_BYTES / 2)).error ?? "",
    /65536 bytes or less/,
  );
  assert.ok(
    AGENT_BROWSER_SCRIPT_SPILL_MAX_BYTES * 2 <= AGENT_BROWSER_SCRIPT_IPC_MESSAGE_MAX_BYTES,
    "rehydrated spills must leave room for the response envelope",
  );
});
...
test("script browser envelopes expose actionable rejected-call errors and emit aggregation is deterministic", async () => {
  const rejected = await runAgentBrowserScript({
    ...
    dispatch: async () => {
      throw new Error("rejected calls must not dispatch");
    },
  });
  ...
  const missingParams = await runAgentBrowserScript({
    ...
    dispatch: async () => {
      throw new Error("invalid calls must not dispatch");
    },
  });
  assert.deepEqual(missingParams.data, {
    error: "script browser(params) requires an object.",
    ok: false,
  });
  assert.equal(missingParams.rejectedCallCount, 1);

  for (const code of [`emit(undefined);`, `emit(() => 1);`]) {
    const invalidEmit = await runAgentBrowserScript({
      code,
      dispatch: async () => successEnvelope(null),
    });
  ...
  const replayableEnvelope = await buildScriptBrowserEnvelope(
    {
      ...
      details: {
        ...
        nextActions: [
          {
            id: "inspect",
            params: { args: ["--namespace", "", "--session", sessionName, "snapshot", "-i"] },
            reason: "Inspect.",
            tool: "agent_browser",
          },
          {
            id: "local",
            params: { args: ["--session", sessionName, "profiles"] },
            reason: "Local command.",
            tool: "agent_browser",
          },
          {
            id: "lookup",
            params: { networkSourceLookup: { requestId: "req-1" } },
            reason: "Top-level mode.",
            tool: "agent_browser",
          },
          {
            artifactPath: "/tmp/example.png",
            id: "artifact",
  ...
  assert.deepEqual(
    replayableEnvelope.nextActions?.map((action) => action.id),
    ["inspect", "artifact"],
  );
  ...
  const serializedBoundedEnvelope = JSON.stringify({
  ...
  assert.ok(
    Buffer.byteLength(serializedBoundedEnvelope, "utf8") + 1 <=
      AGENT_BROWSER_SCRIPT_IPC_MESSAGE_MAX_BYTES,
  );
  ...
  const escapeHeavyEnvelope = await buildScriptBrowserEnvelope(
    {
      ...
      details: {
        data: "\0".repeat(AGENT_BROWSER_SCRIPT_SPILL_MAX_BYTES),
        resultCategory: "success",
        successCategory: "completed",
      },
    },
...
emit({
  ...
  assert.match(
    dynamicImport.error ?? "",
    /sandbox exited before completion|dynamic import callback/i,
  );
});
...
test("script result rendering stays compact and preserves structured failure accounting", () => {
  ...
  const compactText =
    compactResult.content[0]?.type === "text" ? compactResult.content[0].text : "";
  assert.ok(
    Buffer.byteLength(compactText, "utf8") < AGENT_BROWSER_SCRIPT_FINAL_OUTPUT_MAX_BYTES + 200,
  );
  ...
  const deepDetails = deepResult.details as
    | { data?: unknown; failureCategory?: string }
    | undefined;
  assert.equal(deepDetails?.failureCategory, "validation-error");
  assert.match(
    deepResult.content[0]?.type === "text" ? deepResult.content[0].text : "",
    /could not be safely rendered as bounded JSON/,
  );
  assert.equal(deepDetails?.data, undefined);
});

test("script runner reports a missing compiled worker clearly", {
  concurrency: false,
}, async () => {
...
test("script inner policy rejects identity, lifecycle, batch, local, and persistent launch controls", () => {
  ...
  assert.deepEqual(
    validateAgentBrowserScriptBrowserParams({
      args: ["--allowed-domains", "example.test", "open", "https://example.test"],
    }).params?.args,
    ["--allowed-domains", "example.test", "open", "https://example.test"],
  );
  assert.match(
...
test("script policy rejections fail the top-level result with disjoint counters", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /Script failed: 1 browser call was rejected before dispatch/,
    );
...
test("session_shutdown aborts and reaps an active sandbox child", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(
          (result.details?.scriptRun as { aborted?: boolean } | undefined)?.aborted,
          true,
        );
      },
    );
...
test("session_tree aborts and fully cleans an active browser-bearing script", {
  concurrency: false,
}, async () => {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        for (
          let index = 0;
          index < 100 &&
          !harness.appendedEntries.some(
            (entry) => (entry.data as { cleanup?: string }).cleanup === "active",
          );
          index += 1
        )
          await delay(10);
        ...
        for (let index = 0; index < 100; index += 1) {
          ...
          if (
            invocations.some((entry) => entry.args.includes("get") && entry.args.includes("title"))
          )
            break;
          await delay(10);
        }
        assert.ok(
          ...
          "browser subprocess should start before branch abort",
        );
        ...
        assert.ok(
          ...
          "branch change should abort instead of waiting for the script deadline",
        );
        assert.equal(result.details?.failureCategory, "aborted");
        assert.equal(
          (result.details?.scriptSession as { cleanup?: string } | undefined)?.cleanup,
          "closed",
        );
        assert.deepEqual(
          harness.appendedEntries.map((entry) => (entry.data as { cleanup?: string }).cleanup),
          ["active", "closed"],
        );
        const invocations = await readInvocationLog(logPath);
...
test("script inner calls and cleanup clear ambient upstream launch controls", {
  concurrency: false,
}, async () => {
...
if (args.includes("session") && args.includes("info")) process.stdout.write(JSON.stringify({ success: true, data: { active: false, runtime: null } }));
  ...
  try {
    const ambientEnv = Object.fromEntries(
      ambientNames.map((name) => [
        name,
        name.includes("PROXY") || name === "https_proxy"
          ? "http://proxy.invalid"
          : "ambient-control",
      ]),
    );
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(
          (result.details?.scriptRun as { callCount?: number } | undefined)?.callCount,
          2,
        );
        assert.equal(result.details?.attachedBrowserSession, undefined);
        const sessionName = (result.details?.scriptSession as { sessionName?: string } | undefined)
          ?.sessionName;
        const invocations = (
        ...
        ).filter((entry) => entry.args.includes(sessionName ?? "missing"));
        assert.ok(
          invocations.some((invocation) => invocation.args.includes("snapshot")),
          "script should replay the policy-compatible next action",
        );
        for (const invocation of invocations) {
          for (const name of ambientNames) {
            ...
            assert.notEqual(
              value,
              "ambient-control",
              `${name} leaked into ${JSON.stringify(invocation.args)}`,
            );
            assert.notEqual(
              value,
              "http://proxy.invalid",
...
test("script cleanup retires its active recording reservation", {
  concurrency: false,
}, async () => {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        const manifest = result.details?.artifactManifest as
          ...
          | undefined;
        assert.equal(
          manifest?.entries?.some((entry) => entry.subcommand === "start"),
          false,
        );
        const reuse = await executeRegisteredTool(harness.tool, harness.ctx, {
          args: ["pdf", "script.webm"],
        });
...
test("agentBrowserExtension injects an isolated script session, persists its lease before spawn, and closes it", {
  concurrency: false,
}, async () => {
        ...
        const harness = createExtensionHarness({
          ...
          onAppendEntry(customType, data) {
            if (
              ...
              (data as { cleanup?: string }).cleanup === "active"
            )
              writeFileSync(leaseMarkerPath, "active");
          },
        });
        await runExtensionEvent(harness.handlers, "session_start", { reason: "new" }, harness.ctx);
        const beforeScript = await executeRegisteredTool(harness.tool, harness.ctx, {
          args: ["get", "title"],
        });
...
emit(values);`,
        ...
        const scriptSession = result.details?.scriptSession as
          | { cleanup?: string; sessionName?: string }
          | undefined;
        assert.equal(scriptSession?.cleanup, "closed");
        assert.match(scriptSession?.sessionName ?? "", /^piab-script-[0-9a-f-]{36}$/);
        const closeCommandArgs = [
          "--namespace",
          "",
          "--session",
          scriptSession?.sessionName,
          "close",
        ];
        assert.deepEqual(
          harness.appendedEntries.map((entry) => entry.data),
          [
            {
              cleanup: "active",
              closeCommandArgs,
              launchAttempted: true,
              sessionName: scriptSession?.sessionName,
            },
            {
              cleanup: "closed",
              closeCommandArgs,
        ...
        assert.deepEqual(scriptSession, {
          cleanup: "closed",
          closeCommandArgs,
          launchAttempted: true,
          sessionName: scriptSession?.sessionName,
        ...
        const secondSessionName = (
          secondResult.details?.scriptSession as { sessionName?: string } | undefined
        )?.sessionName;
        assert.notEqual(
          secondSessionName,
          scriptSession?.sessionName,
          "every script call must receive a unique isolated session",
        );
        const afterScript = await executeRegisteredTool(harness.tool, harness.ctx, {
          args: ["get", "title"],
        });
        assert.equal(
          afterScript.details?.sessionName,
          implicitSessionName,
          "script must not replace the current implicit session",
        );
        ...
        const scriptInvocations = invocations.filter((entry) =>
          entry.args.includes(scriptSession?.sessionName ?? "missing"),
        );
        assert.ok(
          scriptInvocations.every((entry) => entry.leasePresent === true),
          "lease must exist before every isolated-session fake upstream spawn",
        );
        const contentInvocations = scriptInvocations.filter(
          (entry) => entry.args.includes("get") && entry.args.includes("title"),
        );
        ...
        for (const invocation of contentInvocations) {
          assert.deepEqual(
            invocation.args.slice(
              invocation.args.indexOf("--namespace"),
              invocation.args.indexOf("--namespace") + 4,
            ),
            ["--namespace", "", "--session", scriptSession?.sessionName],
          );
        }
...
test("agentBrowserExtension closes its isolated session after a script timeout", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: args.includes("close") ? { closed: true } : { title: "ok" } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        (thrown.content[0] as { text: string }).text,
        /Script failed: Error: intentional cleanup probe/,
      );
      ...
      assert.equal(
        (thrown.details?.scriptSession as { cleanup?: string } | undefined)?.cleanup,
        "closed",
      );
    });
  } finally {
...
test("agentBrowserExtension rehydrates only its verified compact spill for script logic", {
  concurrency: false,
}, async () => {
...
if (args.includes("close")) process.stdout.write(JSON.stringify({ success: true, data: { closed: true } }));
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        (
          result.details?.artifactManifest as { entries?: Array<{ kind?: string }> } | undefined
        )?.entries?.some((entry) => entry.kind === "spill"),
      );
      await runExtensionEvent(
        harness.handlers,
        "session_shutdown",
...
test("script still runs fail-closed cleanup when the main browser command never starts", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { title: "should not run" } }));`,
  ...
  try {
    await withPatchedEnv(
      { PATH: `${tempDir}:${basePath}`, PI_AGENT_BROWSER_SOCKET_DIR: socketDir },
      async () => {
        const harness = createExtensionHarness({
          cwd: tempDir,
          sessionFile: join(tempDir, "session.jsonl"),
        ...
        assert.equal(
          (result.details?.data as { failureCategory?: string } | undefined)?.failureCategory,
          "validation-error",
        );
        assert.match(
          (result.details?.data as { error?: string } | undefined)?.error ?? "",
          /Unix socket path would be/,
        );
        assert.equal(
          (result.details?.scriptSession as { cleanup?: string } | undefined)?.cleanup,
          "closed",
        ...
        assert.ok(
          ...
          "cleanup must close helpers that could have started the isolated browser",
        );
      },
    );
  } finally {
...
test("script cleanup failure is durable and exposes the exact close action", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { title: "ok" } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(closeAction?.params?.args, [
        "--namespace",
        "",
        "--session",
        scriptSession?.sessionName,
      ...
      assert.equal(
        (harness.appendedEntries.at(-1)?.data as { cleanup?: string } | undefined)?.cleanup,
        "failed",
      );
    });
  } finally {
...
test("session_start restores failed script leases and retries close in a new extension process", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      const harness = createExtensionHarness({
        branch: [
          ...
          },
          {
            ...
            data: {
              cleanup: "active",
              closeCommandArgs: ["--session", "other", "close"],
              launchAttempted: true,
              sessionName,
            },
          },
          {
            type: "custom",
            customType: "agent-browser-script-session",
            data: { sessionName: "piab-script-not-a-uuid", cleanup: "active", args: ["secret"] },
          },
        ],
        cwd: tempDir,
        sessionFile: join(tempDir, "session.jsonl"),
      ...
      assert.ok(
        invocations.some(
          (entry) => entry.args.includes("close") && entry.args.includes(sessionName),
        ),
      );
      assert.equal(
        invocations.some((entry) => entry.args.includes("piab-script-not-a-uuid")),
        false,
      );
    });
  } finally {
