...
function spawnElectronFixtureProcess(userDataDir: string): ChildProcess {
  const child = spawn(
    ...
    [
      ...
      "pi-agent-browser-electron-fixture",
...
test("agentBrowserExtension blocks page-scoped ref reuse after navigation before upstream can recycle it", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension invalidates prior refs when a failed transition command keeps the page verified", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rehydrates page-scoped refs from the current tree branch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rehydrates managed browser session state from the current tree branch", {
  concurrency: false,
}, async () => {
...
if (args.includes("open")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const branchB = [
        createToolBranchEntry({ details: secondOpen.details ?? {}, isError: false }),
      ];
...
test("agentBrowserExtension rehydrates artifact manifest state from the current tree branch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps Electron cleanup ownership after session_tree switches away from the launch branch", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        invocations.some(
          (entry) =>
            entry.args.join("\0") === ["--session", electronSessionName, "close"].join("\0"),
        ),
      );
...
test("agentBrowserExtension does not double-clean a branch-restored Electron cleanup during shutdown", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const cleanedBranch = [
        createToolBranchEntry({ details: launchDetails, isError: false }),
        createToolBranchEntry({
          details: electronCleanupDetails(electronSessionName, electronRecord),
          isError: false,
        }),
      ];
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "branch-cleaned", oldLeafId: "branch-open" },
        harness.ctx,
      );
      await runExtensionEvent(
        harness.handlers,
...
test("agentBrowserExtension clears namespaced attachment context after Electron cleanup replay", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { result: "https://safe.example/", url: "https://safe.example/" } }));`,
  ...
  try {
    await withPatchedEnv(
      { PATH: `${tempDir}:${basePath}`, PI_AGENT_BROWSER_TEST_PRESERVE_INTERNAL_LAUNCH_FLAGS: "1" },
      async () => {
          ...
          createToolBranchEntry({
            details: {
              ...
              electron: {
                cleanup: {
                  results: [
                    { steps: [{ resource: "managed-session", sessionName, state: "removed" }] },
                  ],
                },
              },
              namespace,
        ...
        const invocation = (await readInvocationLog(logPath)).find(
          (entry) => entry.args.includes("get") && entry.args.at(-1) === "url",
        );
...
test("agentBrowserExtension keeps same-process re-owned Electron resources despite stale branch cleanup evidence", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "branch-open", oldLeafId: "branch-cleaned" },
        harness.ctx,
      );
...
test("agentBrowserExtension preserves branch ownership of untouched Electron launch after targeted cleanup", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const branchBCleaned = [
        ...
        createToolBranchEntry({ details: launchDetailsB, isError: false }),
        createToolBranchEntry({
          details: electronCleanupDetails(sessionNameB, recordB),
          isError: false,
        }),
      ];
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "b-cleaned", oldLeafId: "both-active" },
        harness.ctx,
      );
      await runExtensionEvent(
        harness.handlers,
      ...
      const invocations = await readInvocationLog(logPath);
      const closeArgs = invocations
        .map((entry) => entry.args)
        .filter((args) => args.includes("close"));
      const closedSessions = closeArgs
        .map((args) => {
...
test("agentBrowserExtension preserves branch ownership of Electron launch after failing explicit-session command", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "cleaned", oldLeafId: "open" },
        harness.ctx,
      );
      await runExtensionEvent(
        harness.handlers,
...
test("agentBrowserExtension exposes off-branch owned Electron records to status, probe, and cleanup by launchId", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const harness = createExtensionHarness({
        branch: [
          createToolBranchEntry({
            details: electronManagedSessionDetails(electronSessionName, electronRecord),
            isError: false,
          }),
        ],
        cwd: tempDir,
      ...
      const status = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "status", launchId: electronRecord.launchId },
      });
      ...
      const probe = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "probe", launchId: electronRecord.launchId },
      });
...
test("agentBrowserExtension restores headed autosave policy for an off-current Electron session", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(probe.isError, false, JSON.stringify(probe));
        assert.equal(
          probe.details?.managedSessionHeadedAutosaveDisabled,
          true,
        ...
        await withPatchedEnv({ AGENT_BROWSER_AUTOSAVE_INTERVAL_MS: "1000" }, async () => {
          const blockedProbe = await executeRegisteredTool(
            resumedHarness.tool,
            resumedHarness.ctx,
            { electron: { action: "probe", launchId: electronRecord.launchId } },
          );
        ...
        const resumedProbe = await executeRegisteredTool(resumedHarness.tool, resumedHarness.ctx, {
          electron: { action: "probe", launchId: electronRecord.launchId },
        });
        assert.equal(resumedProbe.isError, false, JSON.stringify(resumedProbe));
        const cleanup = await executeRegisteredTool(resumedHarness.tool, resumedHarness.ctx, {
          electron: { action: "cleanup", launchId: electronRecord.launchId },
        });
        assert.equal(cleanup.isError, false, JSON.stringify(cleanup));
        ...
        assert.equal(
          invocations.every((entry) => entry.autosave === "0"),
          true,
...
test("agentBrowserExtension does not reuse current Electron managed session after cleanup", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        ...
        [electronSessionName, followUpSessionName],
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("agentBrowserExtension keeps Electron cleanup post-close reservation across same-process session_tree restore", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "cleanup", oldLeafId: "live" },
        harness.ctx,
      );
      const firstFollowUp = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["get", "url"],
      });
      ...
      const closeFirstFollowUp = await executeRegisteredTool(harness.tool, harness.ctx, {
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "cleanup", oldLeafId: "follow-up" },
        harness.ctx,
      );
      const secondFollowUp = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["get", "url"],
      });
...
test("agentBrowserExtension does not restore Electron managed session after cleanup result", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { result: "ok", url: "app://restore-cleanup" } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        invocations.map((entry) => entry.sessionName),
        [restoredGeneratedSessionName, restoredGeneratedSessionName, finalSessionName],
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("agentBrowserExtension preserves active branch Electron launch across reload", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { closed: args.includes("close"), result: "ok", url: "app://reload-active" } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (await readInvocationLog(logPath)).some((entry) => entry.args.includes("close")),
        false,
      );
      ...
      const followUp = await executeRegisteredTool(reloadedHarness.tool, reloadedHarness.ctx, {
        args: ["get", "url"],
      });
      assert.equal(followUp.isError, false, JSON.stringify(followUp));
...
test("agentBrowserExtension cleans off-branch Electron launches during reload", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension preserves off-branch Electron profile when reload cleanup is partial", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "branch-empty", oldLeafId: "branch-a" },
        harness.ctx,
      );
      await runExtensionEvent(
        harness.handlers,
...
test("agentBrowserExtension preserves off-branch Electron profile when quit cleanup is partial", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension does not promote unrelated off-branch Electron launches after targeted cleanup", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { closed: args.includes("close"), result: "ok" } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const branchA = [
        createToolBranchEntry({
          details: electronManagedSessionDetails(sessionA, recordA),
          isError: false,
        }),
      ];
      const branchB = [
        createToolBranchEntry({
          details: electronManagedSessionDetails(sessionB, recordB),
          isError: false,
        }),
      ];
      const harness = createExtensionHarness({ branch: branchA, cwd: tempDir });
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "branch-empty", oldLeafId: "branch-b" },
        harness.ctx,
      );

      const cleanupA = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "cleanup", launchId: "electron-target-a" },
      });
      assert.equal(cleanupA.isError, false, JSON.stringify(cleanupA));
      assert.equal(pidIsAlive(childA.pid), false);
      ...
      assert.ok(
        invocations.some(
          (entry) => entry.args.join("\0") === ["--session", sessionB, "close"].join("\0"),
        ),
      );
...
test("agentBrowserExtension serializes explicit Electron cleanup behind in-flight managed commands", {
  concurrency: false,
}, async () => {
...
if (args.includes("snapshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const cleanupPromise = executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "cleanup", launchId: electronRecord.launchId },
      });
      await delay(50);
      assert.equal(
...
test("agentBrowserExtension untracks managed sessions after partial Electron cleanup closes the session", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps network request diagnostics from replacing the active page target", {
  concurrency: false,
}, async () => {
...
if (args.includes("snapshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        (networkRequest.details?.refSnapshot as { refIds?: string[] } | undefined)?.refIds,
        ["e1"],
      );
      ...
      assert.deepEqual(
        (networkSourceLookup.details?.refSnapshot as { refIds?: string[] } | undefined)?.refIds,
        ["e1"],
      );

      const clickAfterNetworkSourceLookup = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["click", "@e1"],
      });
      assert.equal(
...
test("agentBrowserExtension ignores restored diagnostic session targets that contain request URLs", {
  concurrency: false,
}, async () => {
...
if (args.includes("click")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        "order" in ((click.details?.refSnapshot as Record<string, unknown> | undefined) ?? {}),
        false,
      );
...
test("agentBrowserExtension restores empty successful batch snapshots as ref state", {
  concurrency: false,
}, async () => {
...
if (args.includes("click")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        (click.details?.refSnapshot as { refIds?: string[] } | undefined)?.refIds,
        [],
      );
...
test("agentBrowserExtension treats successful snapshots without refs as empty ref state", {
  concurrency: false,
}, async () => {
...
if (args.includes("snapshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        (staleClick.details?.refSnapshot as { refIds?: string[] } | undefined)?.refIds,
        [],
      );
...
test("agentBrowserExtension blocks stale refs after page-changing steps inside a batch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps pending WebMCP targets unknown and trusts a later batch snapshot", {
  concurrency: false,
}, async () => {
...
if (args.includes("batch")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (failedCancel.details?.refSnapshotInvalidation as { reason?: string } | undefined)?.reason,
        "page-transition",
      );
...
test("agentBrowserExtension invalidates direct and batched refs when record start opens a fresh page", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        ...
        /cannot be used yet\. A recording command \(record start, or record restart with a URL\) replaced or navigated the active page/,
      );
      ...
      const guardedDiffSelector = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["diff", "snapshot", "--selector", "@e1"],
      });
      ...
      assert.equal(
        invocations.filter((entry) => entry.args.includes("click") && !entry.args.includes("find"))
          .length,
        1,
      );
      ...
      assert.equal(
        invocations.filter((entry) => entry.args.includes("record") && entry.args.includes("start"))
          .length,
        2,
      );
      assert.equal(
...
test("agentBrowserExtension invalidates refs when a batch recording start times out before returning rows", {
  concurrency: false,
}, async () => {
...
if (args.includes("snapshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (timedOutBatch.details?.refSnapshotInvalidation as { reason?: string } | undefined)?.reason,
        "page-transition",
...
test("agentBrowserExtension allows same-snapshot form fills before a batch click", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension allows same-snapshot form control batches before a hard invalidating click", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension allows batch stdin ref steps after snapshot following an invalidating step", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension records snapshot refs returned inside a successful batch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects batched getter refs after same-page rerender changes current snapshot identity", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects refs absent from the latest same-page snapshot", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension pins raw argument batch steps and guards their refs under restore pinning", {
  concurrency: false,
}, async () => {
...
if (args.includes("get") && args.includes("url")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const batchInvocations = (await readInvocationLog(logPath)).filter((entry) =>
        entry.args.includes("batch"),
      );
      assert.equal(batchInvocations.length, 1, JSON.stringify(batchInvocations));
      const dispatched = JSON.parse(
        (batchInvocations[0] as { stdin?: string }).stdin ?? "[]",
      ) as string[][];
      assert.deepEqual(
        dispatched,
        [
      ...
      assert.equal(
        rawArgvStaleRef.details?.failureCategory,
        "stale-ref",
        JSON.stringify(rawArgvStaleRef),
      );
      const batchAfterStale = (await readInvocationLog(logPath)).filter((entry) =>
        entry.args.includes("batch"),
      );
      ...
      const bailBatches = (await readInvocationLog(logPath))
        .filter((entry) => entry.args.includes("batch"))
        .slice(1);
      ...
      for (const invocation of bailBatches) {
        ...
        const bailDispatched = JSON.parse(
          (invocation as { stdin?: string }).stdin ?? "[]",
        ) as string[][];
        assert.deepEqual(
          bailDispatched,
          [
...
test("agentBrowserExtension keeps upstream-ignored batch stdin out of artifact and screenshot preflights", {
  concurrency: false,
}, async () => {
...
if (args.includes("snapshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (await readInvocationLog(logPath)).filter((entry) => entry.args.includes("batch")).length,
        1,
      );
