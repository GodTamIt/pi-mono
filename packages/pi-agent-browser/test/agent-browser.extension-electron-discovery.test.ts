...
test("agentBrowserExtension supports Electron launch handoff modes", {
  concurrency: false,
}, async () => {
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      for (const [handoff, expectedCommands] of [
        ...
        const result = await executeRegisteredTool(harness.tool, harness.ctx, {
          electron: { action: "launch", appPath: app.appPath, handoff },
        });
        ...
        const launchId = (result.details?.electron as { launch?: { launchId: string } } | undefined)
          ?.launch?.launchId;
        assert.ok(launchId);
...
test("agentBrowserExtension targets Electron webviews and keeps host cleanup after close failures", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension aborts Electron launch before and during app startup", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension blocks Electron launch by caller policy without spawning", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension cleans Electron resources when launch fails before upstream attach", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension cleans Electron resources when upstream connect cannot spawn", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps restored Electron profile when process ownership is unverified", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    assert.equal(
      cleanupResult.steps.find((step) => step.resource === "user-data-dir")?.state,
      "skipped",
    );
...
test("agentBrowserExtension restores Electron launch records and cleans them on shutdown", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const restoredHarness = createExtensionHarness({
        cwd: tempDir,
        branch: [
          createToolBranchEntry({ details: launchResult.details as Record<string, unknown> }),
        ],
      });
      await runExtensionEvent(
      ...
      const statusResult = await executeRegisteredTool(restoredHarness.tool, restoredHarness.ctx, {
