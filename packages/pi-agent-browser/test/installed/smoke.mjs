...
try {
  ...
  mkdirSync(packDir); mkdirSync(installDir);
  execFileSync("npm", ["run", "build"], { cwd: packageDir, stdio: "inherit" });
  const packResult = JSON.parse(execFileSync("npm", ["pack", "--json", "--pack-destination", packDir], { cwd: packageDir, encoding: "utf8" }));
  const packed = (Array.isArray(packResult) ? packResult : [packResult, ...Object.values(packResult)]).find((item) => typeof item?.filename === "string");
  ...
  const extension = await import(pathToFileURL(join(installed, "dist", "extensions", "agent-browser", "index.js")).href);
