    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(
          opened.isError,
          false,
          `orphan-daemon setup open failed: ${opened.content[0]?.text ?? ""}`,
        );
        sessionName =
        ...
        await runExtensionEvent(
          emptyTranscriptHarness.handlers,
          "session_start",
          { reason: "new" },
          emptyTranscriptHarness.ctx,
        );
        const blocked = await executeRegisteredTool(
          emptyTranscriptHarness.tool,
          emptyTranscriptHarness.ctx,
          {
        ...
        assert.equal(
          closed.isError,
          false,
          `orphan-daemon cleanup close failed: ${closed.content[0]?.text ?? ""}`,
        );
...
if (!REAL_UPSTREAM_ENABLED) {
  test(
    "real upstream agent-browser contract suite is opt-in",
    { skip: REAL_UPSTREAM_SKIP_REASON },
    () => undefined,
  );
  test(
    "real upstream agent-browser plugin list probe is opt-in",
    { skip: REAL_UPSTREAM_SKIP_REASON },
    () => undefined,
  );
} else {
    ...
    try {
      ...
      await withPatchedEnv(
        ...
        async () => {
          ...
          const readResult = await executeRegisteredTool(harness.tool, harness.ctx, {
          ...
          const closedInfo = await execFileAsync(
            ...
            ["--json", "--namespace", "", "--session", managedSessionName ?? "", "session", "info"],
            {
              ...
              env: {
                ...process.env,
                AGENT_BROWSER_NAMESPACE: "redirected",
                AGENT_BROWSER_SOCKET_DIR: getAgentBrowserSocketDir() ?? socketDir,
                HOME: tempDir,
              },
            },
          );
