...
test("agentBrowserExtension rejects ambiguous or incomplete semantic actions before spawning agent-browser", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: "should not run" }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        (jobWithStdin.content[0] as { text: string }).text,
        /Do not provide stdin with job/,
      );
      ...
      assert.match(
        (ambiguousJobArgs.content[0] as { text: string }).text,
        /Provide exactly one of script, args, semanticAction, job, qa, sourceLookup, networkSourceLookup, or electron/,
      );
      ...
      assert.match(
        (ambiguousJobSemanticAction.content[0] as { text: string }).text,
        /Provide exactly one of script, args, semanticAction, job, qa, sourceLookup, networkSourceLookup, or electron/,
      );
      ...
      const missingJobText = await executeRegisteredTool(harness.tool, harness.ctx, {
        job: {
          steps: [{ action: "open", url: "https://example.test/" }, { action: "assertText" }],
        },
      });
      assert.equal(missingJobText.isError, true);
      assert.match(
        (missingJobText.content[0] as { text: string }).text,
        /job step assertText requires a non-empty text string/,
      );
      ...
      assert.match(
        (invalidJobWait.content[0] as { text: string }).text,
        /wait requires a positive integer milliseconds/,
      );
      ...
      assert.match(
        (invalidJobSelect.content[0] as { text: string }).text,
        /job\.steps\[0\]\.value or job\.steps\[0\]\.values is required for select/,
      );
      ...
      assert.match(
        (invalidSourceLookup.content[0] as { text: string }).text,
        /sourceLookup requires selector, reactFiberId, or componentName/,
      );
      ...
      assert.match(
        (oversizedSourceLookup.content[0] as { text: string }).text,
        /maxWorkspaceFiles must be 5000 or less/,
      );
      ...
      assert.match(
        (sourceLookupWithArgs.content[0] as { text: string }).text,
        /Provide exactly one of script, args, semanticAction, job, qa, sourceLookup, networkSourceLookup, or electron/,
      );
      ...
      assert.match(
        (sourceLookupWithStdin.content[0] as { text: string }).text,
        /Do not provide stdin with job, qa, sourceLookup, or networkSourceLookup/,
      );
      ...
      assert.match(
        (networkSourceLookupWithArgs.content[0] as { text: string }).text,
        /Provide exactly one of script, args, semanticAction, job, qa, sourceLookup, networkSourceLookup, or electron/,
      );
      ...
      assert.match(
        (networkSourceLookupWithStdin.content[0] as { text: string }).text,
        /Do not provide stdin with job, qa, sourceLookup, or networkSourceLookup/,
      );
      ...
      assert.match(
        (emptyNetworkSourceLookup.content[0] as { text: string }).text,
        /networkSourceLookup requires requestId, filter, or url/,
      );
      ...
      assert.deepEqual(
        invocations.filter((entry) => entry.args.includes("find")),
        [],
      );
    });
  } finally {
...
test("agentBrowserExtension returns rich input recovery when semanticAction fill misses current editable refs", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        ...
        ["refresh-interactive-refs", "focus-current-editable-ref", "click-current-editable-ref"],
      );
...
test("agentBrowserExtension suggests current snapshot refs when raw find role locators miss", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension offers current ref fallback for failed semantic find steps inside batch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension returns a safe semantic retry action only for stale-ref find shorthand failures", {
  concurrency: false,
}, async () => {
