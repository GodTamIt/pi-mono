...
test("agentBrowserExtension re-selects the navigated tab after profiled opens when restored tabs steal focus", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension pins the intended tab inside a follow-up command when reconnect drift would otherwise steal focus", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const logPath = join(tempDir, "invocations.log");
...
if (args.includes("batch")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const tabRecoveryActions = (
        clickedSelector.details?.nextActions as
      ...
      assert.deepEqual(
        tabRecoveryActions?.map((action) => action.id),
        [
          "list-tabs-for-tab-drift-recovery",
          "select-intended-tab-after-drift",
          "snapshot-after-tab-recovery",
      ...
      assert.deepEqual(
        tabRecoveryActions?.map((action) => action.params?.args),
        [
...
test("agentBrowserExtension recovers and preserves the prior target when a command returns about:blank", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension records about:blank and blocks stale refs when about:blank has no recoverable tab", {
  concurrency: false,
}, async () => {
...
if (args.includes("batch")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        (staleRefRetry.details?.refSnapshot as { target?: unknown } | undefined)?.target,
        {
          title: "Example Domain",
          url: "https://example.com/",
        },
...
test("agentBrowserExtension accepts about:blank after a batch close reactivates the session", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension allows explicit navigation to about:blank", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension serializes overlapping same-session opens and keeps the newer target", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const logPath = join(tempDir, "invocations.log");
...
test("agentBrowserExtension serializes a newer unverified target after prior navigation", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension propagates caller aborts during live page verification", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension re-selects the intended tab after a successful command when focus drifts afterward", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const logPath = join(tempDir, "invocations.log");
