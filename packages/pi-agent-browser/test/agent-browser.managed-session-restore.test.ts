...
test("Android restore identity stays stable without hard links or reliable birth time", {
...
test("passive agent-browser config preserves automatic restore while explicit overrides suppress it", () => {
  ...
  try {
    ...
    assert.equal(
      agentBrowserConfigBlocksManagedRestore(cwd, { AGENT_BROWSER_CONFIG: "", HOME: home }),
      true,
    );
...
test("managed restore rejects relative HOME and USERPROFILE paths", () => {
  ...
  try {
    ...
    assert.equal(
      agentBrowserConfigBlocksManagedRestore(cwd, { USERPROFILE: "relative-profile" }, [], "win32"),
      true,
    );
  } finally {
    rmSync(cwd, { recursive: true, force: true });
...
test("managed restore validates encryption keys and secures its POSIX state directory", {
  skip: process.platform === "win32",
}, () => {
  clearManagedSessionRestoreDisabled();
  ...
  try {
    ...
    assert.equal(
      ensureManagedSessionRestoreStorageIsSecure({ HOME: home }, posixFixturePlatform, "Team Name"),
      true,
    );
    assert.equal(
      statSync(join(home, ".agent-browser", "namespaces", "team-name", "state", "sessions")).mode &
        0o077,
      0,
    );
  } finally {
...
test("managed restore pins a trusted canonical HOME and rejects writable ancestry", {
  skip: process.platform === "win32",
}, async () => {
...
test("managed restore rejects writable checkout ancestry", {
  skip: process.platform === "win32",
}, () => {
...
test("managed restore rejects symlinks and files along POSIX restore state paths", {
  skip: process.platform === "win32",
}, () => {
...
test("owned snapshot manifest converges concurrent process writers without a blocking lock", async () => {
  ...
  try {
    ...
    for (const path of paths) writeFileSync(path, "{}");
    assert.equal(
      pruneOwnedManagedSessionRestoreSnapshots({ cwd, parentEnv, statePath: paths[0] }),
      0,
    );
    const manifestName = readdirSync(sessions).find((name) =>
      name.startsWith(".pi-agent-browser-owned-snapshots-v2-"),
...
test("managed restore opt-out avoids state-directory permission changes and disables the spawned identity", {
  skip: process.platform === "win32",
}, () => {
  clearManagedSessionRestoreDisabled();
