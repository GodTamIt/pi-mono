...
test("unverified page transitions still require a live URL before content access", () => {
  assert.equal(
    getPageTargetValidationError({ args: ["get", "url"], pageUrlUnknown: true }),
    undefined,
  );
  assert.equal(
    getPageTargetValidationError({ args: ["tab", "list"], pageUrlUnknown: true }),
    undefined,
  );
  assert.equal(
    getPageTargetValidationError({ args: ["tab", "t2"], pageUrlUnknown: true }),
    undefined,
  );
  assert.match(
  ...
  assert.equal(
    getExplicitSessionPageVerificationRequirement({
      args: ["--session", "external", "session", "info"],
    }),
    undefined,
  );
  assert.match(
...
test("WebMCP page tools require a verified target and may change it", () => {
  ...
  assert.deepEqual(
    getResultingPageTargetState({
      args: ["webmcp", "invoke", "set_message"],
  ...
  assert.equal(
    getPageTargetValidationError({
      args: ["webmcp", "result", "invocation-1"],
      pageUrlUnknown: true,
    }),
    undefined,
  );
  assert.equal(
    getPageTargetValidationError({
      args: ["webmcp", "cancel", "invocation-1"],
      pageUrlUnknown: true,
    }),
    undefined,
  );
  const batchStdin = JSON.stringify([
...
test("raw batch command arguments exclusively determine validation and resulting targets", () => {
  const ignoredStdin = JSON.stringify([
    ["open", "file:///ignored.html"],
    ["snapshot", "-i"],
  ]);
  assert.equal(
    getPageTargetValidationError({
  ...
  assert.equal(
    commandRequiresLivePageVerification(
      ["batch", "get url"],
      JSON.stringify([["eval", "document.title"]]),
    ),
    false,
  );
  assert.deepEqual(
...
test("page-state tracking survives capability pass-through", () => {
  assert.deepEqual(
    getResultingPageTargetState({ args: ["batch"], stdin: JSON.stringify([["connect", "9222"]]) }),
    {
      pageTargetMayHaveChanged: true,
      pageUrlUnknown: true,
    },
...
test("batch command strings match upstream ASCII-space and quoting rules", () => {
  assert.deepEqual(parseBatchCommandArgument("open https://example.com").step, [
    "open",
    "https://example.com",
  ]);
  assert.deepEqual(parseBatchCommandArgument("open\thttps://example.com").step, [
  ...
  assert.deepEqual(parseBatchCommandArgument("get text 'main content'").step, [
    "get",
    "text",
    "main content",
  ]);
  assert.deepEqual(parseBatchCommandArgument("type #name Ada\\ Lovelace").step, [
    "type",
    "#name",
    "Ada Lovelace",
  ]);
