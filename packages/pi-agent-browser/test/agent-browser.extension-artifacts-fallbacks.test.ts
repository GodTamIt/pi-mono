...
test("agentBrowserExtension saves simple anchor downloads directly to the requested path", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (
          result.details?.artifactManifest as
            | { entries?: Array<{ path?: string; storageScope?: string }>; liveCount?: number }
            | undefined
        )?.liveCount,
        1,
      );
      assert.equal(
        (
          result.details?.artifactManifest as
            | { entries?: Array<{ path?: string; storageScope?: string }> }
            | undefined
        )?.entries?.[0]?.storageScope,
        "explicit-path",
      );
      assert.equal(
        (
      ...
      assert.equal(
        (result.details?.artifactVerification as { verified?: boolean } | undefined)?.verified,
        true,
      );
      assert.equal(
        (result.details?.downloadRecovery as { method?: string } | undefined)?.method,
        "direct-anchor-fetch",
      );
      const invocations = await readInvocationLog(logPath);
      assert.equal(
        invocations.some((entry) => entry.args.includes("download")),
        false,
      );
...
test("agentBrowserExtension falls back when a non-loopback page points at loopback", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(harness.handlers, "session_start", { reason: "new" }, harness.ctx);
      const open = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["open", "https://remote.example/"],
      });
      assert.equal(open.isError, false);
      ...
      assert.ok(
        (evalInvocation.stdin ?? "").indexOf("not-loopback-page") <
          (evalInvocation.stdin ?? "").indexOf("fetch(anchorUrl.href"),
      );
...
test("agentBrowserExtension falls back to upstream download on anchor redirects", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension returns retry next actions for failed direct download verification", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps stale-ref guidance when tab pinning wraps a command in batch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension keeps stale-ref guidance for user batch stdin wrapped by tab pinning", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports direct fallback failures with the effective invocation", {
  concurrency: false,
}, async () => {
...
process.exit(1);`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        text,
        /^agent-browser --json --session \S+ open https:\/\/example\.com\/? reported failure \(exit code 1\)\.$/m,
      );
      assert.match(text, /inspect-page-after-navigation-error/);
      assert.deepEqual((result.details?.effectiveArgs as string[] | undefined)?.slice(0, 3), [
        "--json",
        "--session",
        result.details?.sessionName,
      ]);
...
test("agentBrowserExtension reports wrapper-assisted fallback failures with effective batch context", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const logPath = join(tempDir, "invocations.log");
...
process.stdout.write(JSON.stringify({ success: true, data: { tabs: [
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(JSON.parse(String((await readInvocationLog(logPath))[2]?.stdin ?? "[]")), [
        ["tab", "t1"],
        ["get", "title"],
      ]);
    });
...
test("agentBrowserExtension discards oversized malformed output instead of persisting browser secrets", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension discards malformed spills when only a session directory is available", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
...
test("agentBrowserExtension discards malformed spills without session artifacts", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
