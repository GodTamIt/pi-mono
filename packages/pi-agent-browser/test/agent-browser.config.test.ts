/**
 * Purpose: Verify pi-agent-browser package config loading, credential classification, safety rules, and redaction helpers.
 */
...
test("uses project config over global config and allows project-local Brave credential sources", async () => {
  ...
  const plaintextState = await loadAgentBrowserConfig({
    cwd: fixture.cwd,
  ...
  const commandState = await loadAgentBrowserConfig({
    cwd: fixture.cwd,
    env: { ...fixture.env, GLOBAL_BRAVE_KEY: "global-secret" },
  });
  ...
  const envState = await loadAgentBrowserConfig({
    cwd: fixture.cwd,
