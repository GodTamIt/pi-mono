...
test("secure temp cleanup can recreate and track a later temp root", {
  concurrency: false,
}, async () => {
...
test("stale temp pruning removes roots whose marker PID was reused", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
  const staleTime = new Date(Date.now() - 2 * 24 * 60 * 60 * 1_000);
...
test("stale temp pruning does not remove a live root when owner identity is unavailable", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
...
test("secure temp process-exit cleanup preserves protected child directories", {
  concurrency: false,
}, async () => {
  const childScript = `
		import { dirname, join } from "node:path";
  ...
  try {
    ...
    const result = await readChildStdoutJsonLine<{ profile: string; root: string; spill: string }>(
      child,
    );
    root = result.root;
...
test("stale temp pruning preserves profile children protected by prior process metadata", {
  concurrency: false,
}, async () => {
  const childAScript = `
		import { dirname, join } from "node:path";
...
test("writeSecureTempFile enforces the aggregate temp-root disk budget", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
