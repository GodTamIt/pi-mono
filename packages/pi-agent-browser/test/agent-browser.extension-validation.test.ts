...
import {
  ...
  mergeBrowserRunArtifactManifest,
} from "../extensions/agent-browser/index.js";
...
test("agentBrowserExtension injects a compact essential browser prompt", () => {
  ...
  const guidelines = harness.tool.promptGuidelines;
  const text = guidelines.join("\n");
  assert.equal(guidelines.length, 6);
  assert.ok(text.length < 1_250, `prompt guidance was ${text.length} characters`);
  assert.match(text, /agent_browser over direct agent-browser bash/);
  assert.match(text, /exactly one agent_browser input mode/);
  assert.match(text, /open → snapshot -i → current @refs/);
  assert.match(text, /sessionMode=fresh/);
  assert.match(text, /exact paths/);
  assert.match(text, /details\.nextActions/);
  assert.match(text, /COMMAND_REFERENCE\.md/);
});

test.skip("upstream prompt inventory remains available in installed docs", async () => {
  ...
  await withPatchedEnv(
    ...
    async () => {
      ...
      assert.match(
        ...
        /sourceLookup, networkSourceLookup, or electron/,
      );
      ...
      assert.equal(
        harness.tool.promptGuidelines.includes(
      ...
      assert.equal(
        harness.tool.promptGuidelines.includes(SHARED_BROWSER_PLAYBOOK_GUIDELINES[12]),
        false,
      );
      ...
      assert.equal(
        browserTurn?.systemPrompt.includes("Project rule: when browser automation is needed"),
        true,
      );
...
test("built extension prompt doc pointers resolve to package-root docs", {
  ...
  const pi = {
    ...
    registerTool: (tool: { name: string; promptGuidelines?: string[] }) =>
      tools.push({ name: tool.name, promptGuidelines: tool.promptGuidelines ?? [] }),
  };
  (extension.default as (api: typeof pi) => void)(pi);

  const guideline = tools
    .find((tool) => tool.name === "agent_browser")
    ?.promptGuidelines.find((line) => line.includes("COMMAND_REFERENCE.md"));
  assert.ok(guideline);
...
test.skip("legacy global config prompt guidance is replaced by lean docs guidance", async () => {
  const isolatedHome = await mkdtemp(join(tmpdir(), "pi-agent-browser-executable-guidance-test-"));
  const configPath = join(isolatedHome, ".pi", "config", "pi-agent-browser", "config.json");
...
test.skip("legacy static project-shadow prompt guidance is replaced by runtime guidance", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-agent-browser-project-shadow-guidance-test-"));
  try {
    const cwd = join(root, "repo");
    const isolatedHome = join(root, "home");
    const globalConfigPath = join(isolatedHome, ".pi", "config", "pi-agent-browser", "config.json");
    const projectConfigPath = join(cwd, ".pi", "config", "pi-agent-browser", "config.json");
...
test.skip("legacy project-local prompt expansion is replaced by lean runtime guidance", async () => {
  const root = await mkdtemp(join(tmpdir(), "pi-agent-browser-project-guidance-test-"));
  try {
    const cwd = join(root, "repo");
    const isolatedHome = join(root, "home");
    const configPath = join(cwd, ".pi", "config", "pi-agent-browser", "config.json");
    await mkdir(dirname(configPath), { recursive: true });
    ...
    try {
      await withPatchedEnv({ HOME: isolatedHome, PI_AGENT_BROWSER_CONFIG: undefined }, async () => {
        ...
        const [browserTurn] = await runExtensionEventResults<{ systemPrompt: string }>(
          ...
          "before_agent_start",
          {
            prompt: "Open https://example.com with the configured browser profile.",
            systemPrompt: "Base system prompt",
          },
          harness.ctx,
        );
...
test("agentBrowserExtension rejects unsupported public schema fields", () => {
  ...
  assert.equal(
    Check(schema, {
      semanticAction: { action: "click", locator: "role", role: "button", name: "Open" },
    }),
    true,
  );
  ...
  assert.equal(
    Check(schema, {
      networkSourceLookup: { namespace: "review", url: "https://example.test/api" },
    }),
    true,
  );
  assert.equal(
...
test("agentBrowserExtension rejects unsupported extra press/key args before upstream spawn", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects duplicate explicit artifact destinations inside one batch", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension handles bare wait commands through artifact preflight", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports no-op scroll diagnostics with recovery next actions", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const evalCallsBeforeLaunchScopedScroll = (await readInvocationLog(logPath)).filter((entry) =>
...
test("agentBrowserExtension bounds dialog recovery commands and exposes recovery actions", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension scrolls explicit CSS containers before falling back to page scroll", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension handles scroll to end before upstream page scroll", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension scrolls the document directly before upstream wheel fallback", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (result.details?.scrollPage as { request?: { direction?: string } } | undefined)?.request
          ?.direction,
        "down",
      );
...
test("agentBrowserExtension filters snapshot refs with wrapper search and role flags", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { ok: true } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (result.details?.snapshotFilter as { search?: string; matchedRefs?: number } | undefined)
          ?.search,
        "checkout",
      );
      assert.equal(
      ...
      assert.deepEqual((result.details?.refSnapshot as { refIds?: string[] } | undefined)?.refIds, [
        "e1",
        "e2",
        "e3",
      ]);
      ...
      assert.ok(
        invocations.some((entry) => entry.args.includes("snapshot") && entry.args.includes("-i")),
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("agentBrowserExtension surfaces rendered text missing from the accessibility snapshot", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports wrapper snapshot diffs against previous refs", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports wrapper snapshot viewport metadata", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { ok: true } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (result.details?.artifactManifest as { entries?: unknown[] } | undefined)?.entries?.length,
        1,
      );
      const invocations = await readInvocationLog(logPath);
      assert.equal(
      ...
      assert.ok(
        invocations.some((entry) => entry.args.includes("eval") && entry.args.includes("--stdin")),
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("agentBrowserExtension filters network requests to the current page origin", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(
        ...
        ["https://shop.example/app.js", "https://shop.example/api/cart"],
      );
      const invocations = await readInvocationLog(logPath);
      assert.equal(
...
test("agentBrowserExtension reports focused combobox diagnostics with option-opening next actions", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const comboboxResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        semanticAction: { action: "click", locator: "role", value: "combobox", name: "Datasource" },
        sessionMode: "fresh",
      });
...
test("agentBrowserExtension preserves combobox diagnostics after semanticAction visible-ref resolution", {
  concurrency: false,
}, async () => {
...
if (command === "open") {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        invocations.some((entry) => entry.args.at(-2) === "click" && entry.args.at(-1) === "@e65"),
      );
      assert.equal(
...
test("mergeSessionArtifactManifest retains the active restart when the recent window is one", {
  concurrency: false,
}, async () => {
  await withPatchedEnv(
...
test("agentBrowserExtension keeps a direct restart pending through outer manifest merge and replay", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension makes close --all exclusive within its namespace", {
  concurrency: false,
}, async () => {
...
if (command === "close" && commandArgs.includes("--all")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${nodeBinDir}` }, async () => {
      ...
      const otherNamespace = executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["--namespace", "other", "--session", "other-session", "tab", "list"],
      });
      const overlappingCaller = executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["--session", "same-namespace", "tab", "list"],
      });
      const overlappingManaged = executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["get", "url"],
      });
      const [closeResult, otherResult, callerResult, overlapResult] = await Promise.all([
        closeAll,
        otherNamespace,
        overlappingCaller,
        overlappingManaged,
      ...
      assert.ok(
        events.indexOf("close-end") < events.indexOf("default:tab-start"),
        events.join(","),
      );
      assert.ok(
        events.indexOf("close-end") < events.indexOf("default:get-start"),
        events.join(","),
      );
...
test("agentBrowserExtension warns after record start when ffmpeg is missing", {
  concurrency: false,
}, async () => {
  ...
  const missingFfmpegPath =
    process.platform === "android" ? join(tempDir, "node-only") : nodeBinDir;
  if (process.platform === "android") {
...
if (firstCallFailure) process.exit(1);`,
  ...
  try {
    await withPatchedEnv(
      {
        PATH: `${tempDir}:${missingFfmpegPath}`,
        PI_AGENT_BROWSER_SESSION_ARTIFACT_MANIFEST_MAX_ENTRIES: "1",
      },
      async () => {
        const firstCallHarness = createExtensionHarness({
          cwd: tempDir,
          prompt: "Test failed post-close launch ownership.",
          sessionFile: join(tempDir, "first-call-session.jsonl"),
        });
        ...
        const harness = createExtensionHarness({
          cwd: tempDir,
          prompt: "Record a browser workflow.",
        ...
        const reservedScriptOutputPath = await executeRegisteredTool(harness.tool, harness.ctx, {
          script: "emit({ ok: true });",
          outputPath: "demo.webm",
        });
        ...
        assert.deepEqual(missingDetails.recordingDependencyWarning, {
          ...
          dependency: "ffmpeg",
          message:
            "record start can begin recording, but record stop needs ffmpeg on PATH to encode the WebM output.",
        ...
        const replayHarness = createExtensionHarness({
          branch: [activeBeforeCombined, combinedStartClose].map((result) => ({
            type: "message",
            message: {
        ...
        const orderedManifest = orderedBatchRestart.details?.artifactManifest as
          | { entries?: Array<{ path?: string; subcommand?: string }> }
          | undefined;
        assert.equal(
          orderedManifest?.entries?.some(
...
test("agentBrowserExtension retires recording reservations by namespace plus session", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension persists cross-branch recording close tombstones across reload", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension renders long TUI output compactly without changing model-facing content", async () => {
  ...
  assert.doesNotMatch(expandedScriptCallText, UNSAFE_SCRIPT_DISPLAY_PATTERN);
  ...
  const maliciousCallText = renderCall(
    ...
    .render(200)
    .join("\n");
  assert.doesNotMatch(maliciousCallText, DISPLAY_CONTROL_PATTERN);
  ...
  const scalarText = renderResult(
    ...
    .render(120)
    .join("\n");
  assert.doesNotMatch(scalarText, DISPLAY_CONTROL_PATTERN);
  ...
  const fallbackText = renderResult(
    ...
    .render(120)
    .join("\n");
  assert.doesNotMatch(fallbackText, DISPLAY_CONTROL_PATTERN);
...
test("agentBrowserExtension blocks direct and wrapped agent-browser bash unless the prompt, env, or package dev cwd explicitly allows it", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "@ohgodtamit/pi-agent-browser" }),
    "utf8",
  );
