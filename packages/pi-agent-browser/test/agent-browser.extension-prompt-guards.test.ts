...
test("agentBrowserExtension does not turn prompt stop-boundary text into click blocks", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      const harness = createExtensionHarness({
        cwd: tempDir,
        prompt: "Stop on the checkout overview page; do not place the order.",
      });
      ...
      const refClick = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["click", "@e9"],
      });
...
test("agentBrowserExtension blocks close until required prompt screenshot artifacts are saved", {
  concurrency: false,
}, async () => {
...
if (args.includes("screenshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const blockedClose = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["close"],
      });
      ...
      const firstScreenshot = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["screenshot", firstScreenshotPath],
      });
      ...
      const stillBlockedClose = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["close"],
      });
      ...
      const secondScreenshot = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["screenshot", secondScreenshotPath],
      });
...
test("agentBrowserExtension allows close for reference and negated screenshot paths", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: { closed: true } }));`,
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const invocations = await readInvocationLog(logPath);
      assert.equal(
        invocations.filter((entry) => entry.args.includes("close")).length,
        prompts.length,
      );
    });
  } finally {
...
test("agentBrowserExtension resolves relative prompt screenshot paths before allowing close", {
  concurrency: false,
}, async () => {
...
if (args.includes("screenshot")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      const harness = createExtensionHarness({
        cwd: tempDir,
        prompt: `Save a screenshot here: ${relativeScreenshotPath}`,
      });
      await runExtensionEvent(harness.handlers, "session_start", { reason: "new" }, harness.ctx);
      ...
      const screenshot = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["screenshot", relativeScreenshotPath],
      });
