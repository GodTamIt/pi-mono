...
test("getCommandAwareProcessTimeoutMs extends process timeout for wait, read, and WebMCP budgets", async () => {
  await withPatchedEnv({ PI_AGENT_BROWSER_PROCESS_TIMEOUT_MS: "10000" }, async () => {
    assert.equal(
      getCommandAwareProcessTimeoutMs(["open", "https://example.com"], undefined),
      undefined,
    );
    ...
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "https://example.com", "--timeout", "7000"],
        undefined,
      ),
      33000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "--timeout", "8000", "https://example.com"],
        undefined,
      ),
      37000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "https://example.com/a/b", "--timeout", "7000"],
        undefined,
      ),
      47000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "--session", "custom", "--timeout", "8000", "https://example.com/a/b"],
        undefined,
      ),
      53000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "--restore", "https://example.com/a/b", "--timeout", "8000"],
        undefined,
      ),
      53000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "--session", "custom", "--require-md", "--timeout", "8000"],
        undefined,
    ...
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "https://example.com/a/b", "--raw", "--timeout", "7000"],
        undefined,
      ),
      12000,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "https://example.com/a/b", "--llms", "index", "--timeout", "7000"],
        undefined,
      ),
      26000,
    );
    ...
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["webmcp", "cancel", "invocation-1", "--timeout", "8000"],
        undefined,
      ),
      undefined,
    );
    ...
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["batch", "webmcp result invocation-1 --timeout 7000"],
        undefined,
      ),
      12000,
    );
    ...
    assert.equal(
      commandTimeoutNeedsActivePageUrl(["read", "--require-md", "--timeout", "8000"], undefined),
      true,
    );
    assert.equal(
      commandTimeoutNeedsActivePageUrl(
    ...
    assert.equal(
      commandTimeoutNeedsActivePageUrl(
        ["read", "--restore", "https://example.com", "--require-md", "--timeout", "8000"],
        undefined,
      ),
      false,
    );
    assert.equal(
      commandTimeoutNeedsActivePageUrl(
        ["read", "--require-md", "--timeout", "8000", "https://example.com"],
        undefined,
      ),
      false,
    );
    assert.equal(
      commandTimeoutNeedsActivePageUrl(
        ["batch"],
        batch([["read", "--llms", "index", "--timeout", "8000"]]),
      ),
      true,
    );
    assert.equal(
      commandTimeoutNeedsActivePageUrl(["batch", "read --llms index --timeout 8000"], undefined),
      true,
    );
    assert.equal(
      commandTimeoutNeedsActivePageUrl(
    ...
    assert.equal(
      getCommandAwareProcessTimeoutMs(["wait", "--timeout", "bad"], undefined),
      undefined,
    );
    assert.equal(
      getCommandAwareProcessTimeoutMs(
        ["read", "https://example.com", "--timeout", "bad"],
        undefined,
      ),
      undefined,
    );
