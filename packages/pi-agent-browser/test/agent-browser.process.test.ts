...
test("reorderWindowsLeadingGlobalArgs preserves supported global flag values", () => {
  ...
  assert.deepEqual(reorderWindowsLeadingGlobalArgs(["--restore", "open", "https://example.com"]), [
    "open",
    "--restore",
    "https://example.com",
  ]);
  ...
  assert.deepEqual(
    getWindowsExplicitDefaultNamespaceEnv(
      ["--namespace", "", "open", "https://example.com"],
      "prod",
      "win32",
    ),
  ...
  assert.deepEqual(
    getWindowsExplicitDefaultNamespaceEnv(["open", "https://example.com"], "prod", "win32"),
    {},
  );
  assert.deepEqual(
    getWindowsExplicitDefaultNamespaceEnv(
...
test("runAgentBrowserProcess passes upstream browser configuration and file access through", {
  concurrency: false,
}, async () => {
...
test("buildAgentBrowserSpawnCommand uses the npm cmd shim on Windows", () => {
  assert.deepEqual(
    buildAgentBrowserSpawnCommand(
      ["--json", "--session", "managed", "open", "https://example.com"],
      "win32",
    ),
    {
      ...
      args: [
        ...
        "Bypass",
        "-Command",
        "$agentBrowser = Get-Command agent-browser.cmd -ErrorAction SilentlyContinue; if (-not $agentBrowser) { [Console]::Error.WriteLine('PI_AGENT_BROWSER_COMMAND_NOT_FOUND:agent-browser.cmd'); exit 127 }; & $agentBrowser.Source 'open' '--json' '--session' 'managed' 'https://example.com'",
      ],
    },
  );
  assert.match(
  ...
  assert.deepEqual(buildAgentBrowserSpawnCommand(["--version"], "darwin"), {
    command: "agent-browser",
    args: ["--version"],
  });
});

test("process start identity discovery prefers PATH, deduplicates fallbacks, and fails closed", async () => {
  ...
  assert.equal(
    await resolveProcessStartIdentityFromCommands(posixCommands, async (command) => {
      ...
      if (command.file === "ps") return "path-only-identity";
  ...
  const androidFiles = buildProcessStartIdentityCommands(123, "android").map(
    (command) => command.file,
  );
  assert.deepEqual(androidFiles, [
    ...new Set([
      join(dirname(process.execPath), "ps"),
      "ps",
      "/system/bin/ps",
      "/bin/ps",
      "/usr/bin/ps",
  ...
  assert.equal(
    await resolveProcessStartIdentityFromCommands(posixCommands, async (command) => {
      if (command.file === "ps") throw new Error("spawn failed");
      if (command.file === "/bin/ps") return undefined;
      return "fallback-identity";
    }),
    "fallback-identity",
  );
  assert.equal(
    await resolveProcessStartIdentityFromCommands(posixCommands, async () => {
  ...
  assert.equal(normalizeProcessStartIdentity("record-one\nrecord-two\n"), undefined);
  assert.equal(normalizeProcessStartIdentity("record-one\0record-two\n"), undefined);
...
test("Windows managed restore commit excludes PowerShell command-not-found wrappers", () => {
  const missing =
    "& : The term 'agent-browser.cmd' is not recognized as the name of a cmdlet. CategoryInfo: ObjectNotFound CommandNotFoundException";
  assert.equal(isWindowsAgentBrowserCommandMissing(missing), true);
  assert.equal(
    isWindowsAgentBrowserCommandMissing("PI_AGENT_BROWSER_COMMAND_NOT_FOUND:agent-browser.cmd"),
    true,
  );
  assert.equal(
    shouldCommitManagedRestoreAfterWindowsProcess({ exitCode: 1, stderr: missing }),
    false,
  );
  assert.equal(
    shouldCommitManagedRestoreAfterWindowsProcess({ exitCode: 1, stderr: "selector not found" }),
    true,
  );
  ...
  assert.equal(
    shouldCommitManagedRestoreAfterWindowsProcess({
      exitCode: 127,
      spawnError: new Error("spawn powershell ENOENT"),
      stderr: "",
    }),
    false,
  );
...
test("runAgentBrowserProcess uses the Pi-scoped socket directory without trusting ambient upstream configuration", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        const parsed = await parseAgentBrowserEnvelope(result.stdout);
        assert.equal(
          (parsed.envelope?.data as { socketDir?: string } | undefined)?.socketDir,
          socketPath,
        );
...
test("runAgentBrowserProcess stops spilling once the secure temp budget is exceeded", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension removes oversized close stdout spill after fresh-session rotation", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  ...
  try {
    await withPatchedEnv({ HOME: tempDir, PATH: `${tempDir}${delimiter}${basePath}` }, async () => {
      ...
      assert.equal(
        ...
        createManagedSessionRestoreKey(tempDir, getManagedSessionRestoreScope(firstSessionName)),
      );
      ...
      assert.ok(
        ownershipRecord,
        JSON.stringify({ close: closeDebug, ownershipEntries: await readdir(ownershipDirectory) }),
      );
      assert.match(
...
test("agentBrowserExtension removes oversized navigation-summary stdout spills after failed helper commands", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
...
test("runAgentBrowserProcess pins managed restore identity while preserving caller configuration", {
  concurrency: false,
}, async () => {
  await cleanupSecureTempArtifacts();
...
test("runAgentBrowserProcess refuses a changed checkout identity before spawning", {
  concurrency: false,
}, async () => {
...
test("runAgentBrowserProcess refuses incompatible environment changes after planning", {
  concurrency: false,
}, async () => {
...
test("runAgentBrowserProcess passes upstream state, session, file, and launch capabilities through", {
  concurrency: false,
}, async () => {
...
test("runAgentBrowserProcess suppresses visible restore autosave tabs for headed managed launches", {
  concurrency: false,
}, async () => {
...
test("runAgentBrowserProcess forwards the parent environment while preserving wrapper overrides", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const basePath = process.env.PATH ?? "";
