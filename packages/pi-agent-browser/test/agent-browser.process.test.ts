...
test("process start identity discovery prefers PATH, deduplicates fallbacks, and fails closed", async () => {
	...
	assert.deepEqual(posixCommands.map((command) => command.file), ["ps", "/bin/ps", "/usr/bin/ps"]);
	...
	const androidFiles = buildProcessStartIdentityCommands(123, "android").map((command) => command.file);
	assert.deepEqual(androidFiles, [...new Set([join(dirname(process.execPath), "ps"), "ps", "/system/bin/ps", "/bin/ps", "/usr/bin/ps"])]);
	assert.equal(await resolveProcessStartIdentityFromCommands(posixCommands, async (command) => {
		if (command.file === "ps") throw new Error("spawn failed");
		if (command.file === "/bin/ps") return undefined;
		return "fallback-identity";
	}), "fallback-identity");
	assert.equal(await resolveProcessStartIdentityFromCommands(posixCommands, async () => { throw new Error("timeout"); }), undefined);
