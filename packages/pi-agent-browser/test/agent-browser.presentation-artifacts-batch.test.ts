...
test("buildToolPresentation formats download results as saved-file summaries", async () => {
  ...
  assert.match(
    ...
    /Download reported; file not verified: \/tmp\/report\.pdf/,
  );
  ...
  assert.equal(
    ...
    "Artifact verification failed: requested download was not found at /tmp/report.pdf.",
  );
...
test("buildToolPresentation renders metadata-first summaries for file artifact commands", async () => {
  ...
  for (const item of cases) {
    ...
    assert.equal(
      presentation.summary,
      `Artifact verification failed: requested ${item.expectedKind} was not found at ${join("/tmp/pi-agent-browser-artifact-tests", item.data.path)}.`,
    );
    assert.equal(presentation.resultCategory, "failure");
    assert.equal(presentation.failureCategory, "artifact-missing");
    ...
    assert.equal(
      presentation.artifacts?.[0]?.absolutePath,
      join("/tmp/pi-agent-browser-artifact-tests", item.data.path),
    );
...
test("buildToolPresentation records path-bearing diff screenshots without inlining them as trusted screenshots", async () => {
  ...
  assert.equal(
    presentation.summary,
    "Artifact verification failed: requested image was not found at /tmp/pi-agent-browser-artifact-tests/diff.png.",
  );
  assert.equal(presentation.resultCategory, "failure");
  assert.equal(presentation.failureCategory, "artifact-missing");
  assert.equal(presentation.content[0]?.type, "text");
  ...
  assert.equal(
    presentation.artifacts?.[0]?.absolutePath,
    join("/tmp/pi-agent-browser-artifact-tests", "diff.png"),
  );
  assert.equal(presentation.artifactVerification?.artifacts[0]?.state, "missing");
...
test("buildToolPresentation renders record start as a lifecycle state without missing-file copy", async () => {
  ...
  assert.equal(
    presentation.summary,
    "Recording started in a fresh active page; output will be written on stop: recording.webm",
  );
  assert.equal(presentation.content[0]?.type, "text");
  const text = (presentation.content[0] as { text: string }).text;
  assert.match(
  ...
  assert.match(
    text,
    /prior in-page DOM and JavaScript state does not carry over.*fresh snapshot/is,
  );
  ...
  assert.equal(
    presentation.artifacts?.[0]?.absolutePath,
    join("/tmp/pi-agent-browser-artifact-tests", "recording.webm"),
  );
  ...
  const presentation = await buildToolPresentation({
    ...
    envelope: {
      ...
      data: [
        ...
        {
          command: ["wait", "--download", "/tmp/export.csv"],
          result: { path: "/tmp/export.csv", elapsedMs: 75 },
          success: true,
        },
      ],
    },
...
test("buildToolPresentation skips oversized inline image attachments", {
  concurrency: false,
}, async () => {
