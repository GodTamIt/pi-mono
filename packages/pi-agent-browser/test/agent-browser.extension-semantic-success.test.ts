...
test("agentBrowserExtension enriches semanticAction click success with page state probe and prose parity", {
  concurrency: false,
}, async () => {
...
if (command === "find") {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        (semanticClick.details?.pageChangeSummary as { observed?: boolean } | undefined)?.observed,
        false,
      );
