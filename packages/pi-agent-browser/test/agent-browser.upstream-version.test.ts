...
test("upstream version output accepts every well-formed version and rejects malformed output", () => {
	...
	for (const version of [MINIMUM_AGENT_BROWSER_VERSION, TARGET_AGENT_BROWSER_VERSION, "0.1.0", "1.0.0", "0.35.1-beta.1"]) {
...
test("browser-backed calls run despite upstream version drift", { concurrency: false }, async () => {
...
`);
		await withPatchedEnv({
			...
			const result = await executeRegisteredTool(harness.tool, harness.ctx, { args: ["open", "https://example.com"] });
