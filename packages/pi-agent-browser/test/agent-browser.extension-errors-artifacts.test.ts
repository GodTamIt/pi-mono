...
test("agentBrowserExtension rejects dangling value-taking flags before spawning agent-browser", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension passes through managed state capabilities and list rows", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension passes through file access, local navigation, and protected-looking paths", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension live-verifies caller-owned explicit sessions before content reads", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension serializes caller-owned live verification with same-session commands", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        for (
          let attempt = 0;
          attempt < 100 && !beforeRelease.some((entry) => entry.args.includes("caller-other"));
          attempt += 1
        ) {
          await new Promise((resolve) => setTimeout(resolve, 10));
...
test("agentBrowserExtension keeps failed navigation targets unverified", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports local-page navigation and continues normally", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension exposes and targets wrapper-prefixed live sessions", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects incompatible launch reuse of an active restore-enabled managed daemon", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(
          (cloudflareFollowup.details?.compatibilityWorkaround as { id?: string } | undefined)?.id,
          "cloudflare-headless-user-agent",
        );
        ...
        await runExtensionEvent(
          orphanHarness.handlers,
          "session_start",
          { reason: "new" },
          orphanHarness.ctx,
        );
        const orphanedDaemonReuse = await executeRegisteredTool(
          orphanHarness.tool,
          orphanHarness.ctx,
          {
            args: [
        ...
        assert.match(
          String(orphanedDaemonReuse.details?.validationError ?? ""),
...
test("agentBrowserExtension revalidates daemon policy after cross-instance lock contention", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension re-inspects a locally known daemon after an external restart", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension serializes caller-owned artifact writes and preserves their aggregate manifest", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        harness.setBranch(
          [concurrentResults[0], concurrentResults[1]].map((result) => ({
            type: "message",
            message: {
        ...
        const restoredEntries =
          (
            restored.details?.artifactManifest as
              | { entries?: Array<{ absolutePath?: string; path: string }> }
              | undefined
          )?.entries ?? [];
        assert.deepEqual(
        ...
        const entries =
          (
            finalScreenshot.details?.artifactManifest as
              | { entries?: Array<{ absolutePath?: string; path: string }> }
              | undefined
          )?.entries ?? [];
...
test("agentBrowserExtension rejects an externally replaced restore-disabled daemon", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects an unproven restore-disabled daemon", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension blocks a prior checkout generation daemon after path reuse", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv(
      ...
      async () => {
        ...
        await runExtensionEvent(
          noIdentityHarness.handlers,
          "session_start",
          { reason: "new" },
          noIdentityHarness.ctx,
        );
        const noIdentityBlocked = await executeRegisteredTool(
          noIdentityHarness.tool,
          noIdentityHarness.ctx,
          { args: ["open", "https://example.com"] },
        );
        assert.equal(noIdentityBlocked.isError, true);
        assert.match(
          String(noIdentityBlocked.details?.validationError ?? ""),
          /does not match the requested managed-restore policy/,
        );

        initializeGitProject(tempDir);
        assert.notEqual(createManagedSessionRestoreKey(tempDir), priorKey);
        const sameInstanceReplacementBlocked = await executeRegisteredTool(first.tool, first.ctx, {
          args: ["open", "https://example.com"],
        });
        assert.equal(
        ...
        assert.equal(
          (await readInvocationLog(logPath)).filter((entry) => entry.args.includes("open")).length,
          1,
        );
...
test("agentBrowserExtension blocks incompatible reuse when an explicit restore key remains active", {
  concurrency: false,
}, async () => {
...
for (const testCase of [
  ...
  test(`agentBrowserExtension reuses restore-disabled managed sessions with ${testCase.name}`, {
    concurrency: false,
  }, async () => {
      ...
      await withPatchedEnv(
        ...
        async () => {
          ...
          const branch = [
            {
              type: "message",
              message: { details: opened.details, isError: false, toolName: "agent_browser" },
            },
          ];
          ...
          if (testCase.name === "documented restore opt-out") {
            ...
            const reusedAfterIdleRestart = await executeRegisteredTool(resumed.tool, resumed.ctx, {
              args: ["get", "url"],
            });
            assert.equal(
...
test("agentBrowserExtension does not sticky-disable restore when a suppressed spawn fails", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension rejects malformed JSON envelopes that omit success", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const basePath = process.env.PATH ?? "";
...
test("agentBrowserExtension forwards long waits and extends the subprocess watchdog from explicit wait timeouts", {
  concurrency: false,
}, async () => {
...
setTimeout(() => process.stdout.write(JSON.stringify({ success: true, data: { ok: true } })), delay);`,
  ...
  try {
    await withPatchedEnv(
      { PATH: `${tempDir}:${basePath}`, PI_AGENT_BROWSER_PROCESS_TIMEOUT_MS: "50" },
      async () => {
        const harness = createExtensionHarness({ cwd: tempDir });
        await runExtensionEvent(harness.handlers, "session_start", { reason: "new" }, harness.ctx);
...
test("agentBrowserExtension warns when eval stdin returns an empty object from a function-shaped snippet", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension normalizes eval --stdin scripts misplaced in args", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension retains and closes a fresh daemon when its first non-batch command fails", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports managed-session outcomes after failed fresh launches", {
  concurrency: false,
}, async (context) => {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        await withPatchedEnv({ PATH: missingBinaryDir }, async () => {
          ...
          const abandonedMissingBinaryNextActions = abandonedMissingBinary.details?.nextActions as
            | Array<{ id?: string; params?: { args?: string[] } }>
            | undefined;
          assert.ok(
...
test("agentBrowserExtension writes eval and get output data to requested files", {
  concurrency: false,
}, async () => {
...
if (args.includes("eval")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.deepEqual(JSON.parse(await readFile(join(tempDir, "logs/eval-state.json"), "utf8")), {
...
test("agentBrowserExtension reports partial progress and artifacts after job timeout", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.deepEqual(timeoutProgress?.retryStep?.args, [
          "wait",
          "--download",
          "dogfood/export.csv",
        ]);
        ...
        assert.match(
          text,
          /Retry failed step: \{"args":\["wait","--download","dogfood\/export\.csv"\]\}/,
        );
...
test("agentBrowserExtension retries fresh timed-out navigation in a new session when no live URL is recovered", {
  concurrency: false,
}, async () => {
    ...
    await withPatchedEnv(
      ...
      async () => {
        ...
        const result = await executeRegisteredTool(harness.tool, harness.ctx, {
          job: {
            steps: [
              { action: "open", url: "https://example.test/fresh-timeout" },
        ...
        assert.deepEqual(retryAction?.params, {
          args: ["open", "https://example.test/fresh-timeout"],
          sessionMode: "fresh",
        });
      },
    );
...
test("collectTimeoutPartialProgress recovers live page state when session probes succeed", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  const basePath = process.env.PATH ?? "";
  ...
  try {
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        ...
        /Step 2 \[failed, generated from open\.loadState\]: wait --load domcontentloaded/,
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("collectTimeoutPartialProgress reads page context for local URLs", {
  concurrency: false,
}, async () => {
...
test("collectTimeoutPartialProgress falls back to the planned page URL when live page recovery is unavailable", {
  concurrency: false,
}, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "pi-agent-browser-test-"));
  try {
...
test("agentBrowserExtension forwards wait --download saved-file metadata in details", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        (result.content[0] as { text: string }).text,
        /Artifact verification failed: requested download was not found at \/tmp\/export\.csv/,
      );
      assert.match(
        (result.content[0] as { text: string }).text,
...
test("agentBrowserExtension reports artifact lifecycle guidance on close", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension warns when get text may read hidden selector matches", {
  concurrency: false,
}, async () => {
...
if (args.includes("get") && args.includes("text")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const unquotedSecretSelectorResult = await executeRegisteredTool(harness.tool, harness.ctx, {
      ...
      assert.deepEqual(
        (
          batchResult.details?.selectorTextVisibilityAll as Array<{ selector?: string }> | undefined
...
test("agentBrowserExtension surfaces overlay blockers in snapshot actionability metadata", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension surfaces likely overlay blockers after a no-op click", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension does not report overlay blockers from unrelated page chrome after a successful same-page click", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension returns tab-drift next actions for early tab re-selection failures", {
  concurrency: false,
}, async () => {
