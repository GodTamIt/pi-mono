...
async function waitForLoggedCommand(
  logPath: string,
  command: string,
  timeoutMs = 5_000,
): Promise<void> {
...
test("agentBrowserExtension accepts action-specific electron schema and routes list without upstream spawn", {
  concurrency: false,
}, async () => {
...
process.stdout.write(JSON.stringify({ success: true, data: "should not run" }));`,
  ...
  if (process.platform === "darwin") {
    ...
    await writeFile(
      join(desktopDir, "obsidian.desktop"),
      `[Desktop Entry]\nType=Application\nName=Obsidian\nExec=${executablePath}\n`,
      "utf8",
    );
  }
  ...
  try {
    await withPatchedEnv({ HOME: tempDir, PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "list", maxResults: 10, query: "code" },
        }),
        true,
      );
      assert.equal(
      ...
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "status", launchId: "launch-1", timeoutMs: 1_000 },
        }),
        true,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "status", timeoutMs: 1_000 } }),
        true,
      );
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "cleanup", all: true, timeoutMs: 1_000 },
        }),
        true,
      );
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "status", all: true, launchId: "launch-1" },
        }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "cleanup", all: true, launchId: "launch-1" },
        }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "status", all: false } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "cleanup", all: false } }),
        false,
      );
      ...
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "probe", launchId: "launch-1", timeoutMs: 1_000 },
        }),
        true,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "probe", timeoutMs: 0 } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "probe", handoff: "tabs" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "status", handoff: "tabs" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "cleanup", handoff: "tabs" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "list", launchId: "launch-1" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "list", query: 42 } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "list", query: "" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "list", maxResults: "10" } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "list", maxResults: 1.5 } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, { electron: { action: "launch", allow: [""] } }),
        false,
      );
      assert.equal(
        Check(harness.tool.parameters, {
      ...
      assert.equal(
        Check(harness.tool.parameters, {
          electron: { action: "launch", appName: "Code", launchId: "launch-1" },
        }),
        false,
      );
      ...
      assert.equal(
        (listResult.details?.electron as { action?: string; status?: string } | undefined)?.action,
        "list",
      );
      assert.equal(
        (listResult.details?.electron as { action?: string; status?: string } | undefined)?.status,
        "succeeded",
      );
      ...
      const missingAction = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: {},
      });
      ...
      const probeWithListField = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "probe", query: "demo" },
      });
      ...
      const probeWithoutSession = await executeRegisteredTool(harness.tool, harness.ctx, {
      ...
      const missingLaunchTarget = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "launch" },
      });
      ...
      const reservedAppArg = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: {
          action: "launch",
...
test("agentBrowserExtension cleans Electron after post-launch managed policy rejection", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.PolicyReject",
      launchLogPath,
      name: "Policy Reject",
    });
    await writeFakeAgentBrowserBinary(
...
test("agentBrowserExtension allows local Electron snapshot handoff", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.ProtectedHandoff",
      launchLogPath,
      name: "Protected Handoff",
    });
    await writeFakeAgentBrowserBinary(
      tempDir,
      fakeAgentBrowserLifecycleScript(upstreamLogPath, {
        sessionUrl: protectedUrl,
        snapshotTitle: "SECRET LOCAL CONTENT",
        snapshotUrl: protectedUrl,
      }),
    );
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const invocations = await readInvocationLog(upstreamLogPath);
      assert.equal(
        invocations.some((entry) => entry.args.includes("snapshot") || entry.args.includes("tab")),
        true,
      );
      const launch = JSON.parse((await readFile(launchLogPath, "utf8")).trim()) as {
        pid: number;
...
test("agentBrowserExtension cleans an Electron launch canceled during snapshot handoff", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.CanceledHandoff",
      launchLogPath,
...
if (command === "snapshot") setInterval(() => {}, 1000);
  ...
  process.stdout.write(JSON.stringify({ success: true, data }));
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const resultPromise = executeRegisteredTool(
        harness.tool,
        harness.ctx,
        { electron: { action: "launch", appPath: app.appPath } },
        controller.signal,
...
test("agentBrowserExtension launches Electron with isolated profile, snapshot handoff, status, and cleanup", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.DemoElectron",
      launchLogPath,
      name: "Demo Electron",
    });
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        launchDetails.nextActions.some(
          (action) =>
            ...
            action.params?.electron?.launchId === launchDetails.electron.launch.launchId,
        ),
      );
      assert.ok(
        launchDetails.nextActions.some(
          (action) =>
            action.id === "snapshot-electron-session" && action.params?.args?.includes("snapshot"),
        ),
      );
      ...
      assert.deepEqual(
        statusInvocations.map((entry) => entry.args.at(-1)),
        ["url", "title"],
      );
      assert.equal(
        statusInvocations.every(
          (entry) => entry.args[entry.args.indexOf("--namespace") + 1] === "",
        ),
        true,
      );
      assert.equal(
        statusInvocations.every((entry) => (entry as { restore?: string | null }).restore === null),
        true,
      );
      ...
      const probeResult = await withPatchedEnv({ AGENT_BROWSER_NAMESPACE: "redirected" }, () =>
        executeRegisteredTool(harness.tool, harness.ctx, {
          electron: { action: "probe", timeoutMs: 10_000 },
        }),
      );
      ...
      assert.deepEqual(
        probeInvocations.map((entry) => entry.args.at(-2)),
        ["get", "get", "eval", "tab", "snapshot"],
      );
      assert.deepEqual(
        probeInvocations.slice(0, 2).map((entry) => entry.args.at(-1)),
        ["url", "title"],
      );
      assert.equal(
        probeInvocations.every((entry) => entry.args[entry.args.indexOf("--namespace") + 1] === ""),
        true,
      );
      assert.equal(
        probeInvocations.every((entry) => (entry as { restore?: string | null }).restore === null),
        true,
      );
      ...
      harness.setBranch([
        {
          type: "message",
          message: {
            details: { ...launchResult.details, namespace: "team" },
            isError: false,
      ...
      await runExtensionEvent(
        harness.handlers,
        "session_tree",
        { newLeafId: "namespaced", oldLeafId: null },
        harness.ctx,
      );
      const namespacedProbe = await executeRegisteredTool(harness.tool, harness.ctx, {
      ...
      const restoredPageState = SessionPageState.fromBranch([
        {
          type: "message",
          message: { details: namespacedProbe.details, isError: false, toolName: "agent_browser" },
        },
      ]);
      ...
      assert.equal(
        namespacedLaunchIdProbeInvocations.every(
          (entry) => entry.args[entry.args.indexOf("--namespace") + 1] === "team",
        ),
        true,
      );

      const broadTextResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["get", "text", "body"],
      });
      ...
      const recordingResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["record", "start", "electron-cleanup.webm"],
      });
      ...
      assert.equal(
        (stillActive.details?.electron as { statuses?: Array<{ portAlive?: boolean }> } | undefined)
...
test("agentBrowserExtension retains headed autosave policy for Electron cleanup close", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.HeadedCleanup",
      launchLogPath,
      name: "Headed Cleanup",
    });
    await writeFakeAgentBrowserBinary(tempDir, fakeAgentBrowserLifecycleScript(upstreamLogPath));
    await withPatchedEnv(
      ...
      async () => {
        ...
        assert.equal(
          helperInvocations.every((entry) => entry.autosave === "0"),
          true,
...
test("agentBrowserExtension applies managed restore policy to every current-session electron.probe helper", {
  concurrency: false,
}, async () => {
...
if (args.includes("session") && args.includes("info")) {
  ...
  process.exitCode = 2;
        ...
        const failedProbe = await executeRegisteredTool(harness.tool, harness.ctx, {
          electron: { action: "probe" },
        });
...
test("agentBrowserExtension allows follow-up inspection on local file pages", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const openResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["open", "file:///tmp/normal-browser-fixture.html"],
        sessionMode: "fresh",
      });
      assert.equal(openResult.isError, false);

      const textResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["get", "text", "body"],
      });
...
test("agentBrowserExtension follows a live Electron probe onto local pages", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension reports Electron session mismatch and launchId-aware probe", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.MismatchElectron",
      launchLogPath,
      name: "Mismatch Electron",
    });
    await writeFakeAgentBrowserBinary(
      tempDir,
      fakeAgentBrowserLifecycleScript(upstreamLogPath, {
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const currentUrlDetails = currentUrlResult.details as {
        ...
        nextActions?: Array<{
          ...
          params?: {
            args?: string[];
            electron?: { action?: string; launchId?: string };
            sessionMode?: string;
          };
        }>;
      };
      ...
      const statusResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "status", launchId },
      });
      assert.equal(statusResult.isError, false);
      assert.match(
      ...
      const statusDetails = statusResult.details as {
        ...
        nextActions?: Array<{
          id: string;
          params?: { electron?: { action?: string; launchId?: string } };
        }>;
      };
      ...
      assert.ok(
        statusDetails.nextActions?.some(
          (action) =>
            action.id === "probe-electron-launch" && action.params?.electron?.launchId === launchId,
        ),
      );
      assert.ok(
      ...
      assert.ok(
        statusActionIds.indexOf("reattach-electron-launch") <
          statusActionIds.indexOf("snapshot-electron-session"),
      );

      await rm(upstreamLogPath, { force: true });
      const currentProbeResult = await executeRegisteredTool(harness.tool, harness.ctx, {
      ...
      await rm(upstreamLogPath, { force: true });
      const launchProbeResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "probe", launchId, timeoutMs: 5_000 },
      });
      ...
      assert.equal(
        launchProbeInvocations.every(
          (entry) => entry.args.includes("--session") && entry.args.includes(sessionName),
        ),
        true,
      );
...
test("agentBrowserExtension surfaces Electron post-command death and fill verification", {
  concurrency: false,
}, async () => {
  ...
  try {
    ...
    const app = await writeFakeLaunchableElectronApp({
      applicationsDir,
      bundleId: "com.example.HealthElectron",
      launchLogPath,
      name: "Health Electron",
    });
    await writeFakeAgentBrowserBinary(
      tempDir,
      `const fs = require("node:fs");
    ...
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        clickDetails.nextActions?.some(
          (action) =>
            action.id === "status-electron-launch" &&
...
test("agentBrowserExtension applies electron.probe timeoutMs to bounded subprocess probes", {
  concurrency: false,
}, async () => {
...
setTimeout(() => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const safeOpen = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["open", "https://safe.example/"],
      });
      assert.equal(safeOpen.isError, false, JSON.stringify(safeOpen));

      const probeResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        electron: { action: "probe", timeoutMs: 25 },
      });
...
test("agentBrowserExtension recommends tab recovery after No active page snapshot failures", {
  concurrency: false,
}, async () => {
...
if (command === "connect") {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.equal(
        invocations.filter((entry) => entry.args.at(-2) === "click" && entry.args.at(-1) === "@e2")
          .length,
        1,
      );
...
test("agentBrowserExtension invalidates refs after No active page snapshot failures inside batch", {
  concurrency: false,
}, async () => {
