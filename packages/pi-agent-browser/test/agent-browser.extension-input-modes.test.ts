...
test("compileAgentBrowserJob preserves explicit assertUrl and assertText immediately after click", () => {
  ...
  assert.deepEqual(
    semanticJob.compiled?.steps.map((step) => step.args),
    [
  ...
  assert.match(
    compileAgentBrowserJob({
      steps: [{ action: "click", selector: "button", locator: "text", value: "Search" }],
    }).error ?? "",
    /either selector or semantic locator fields/,
  );
  ...
  assert.deepEqual(
    compiled?.steps?.map((step) => step.args),
    [
  ...
  const exactUrlJob = compileAgentBrowserJob({
    steps: [{ action: "assertUrl", url: "https://shop.example/shipping" }],
  });
  assert.deepEqual(exactUrlJob.compiled?.steps?.[0]?.args, [
    "wait",
    "--url",
    "https://shop.example/shipping",
  ]);
  const exactQueryUrlJob = compileAgentBrowserJob({
    steps: [{ action: "assertUrl", url: "https://shop.example/shipping?step=1&ref=a?b" }],
  });
  assert.deepEqual(exactQueryUrlJob.compiled?.steps?.[0]?.args, [
    "wait",
    "--url",
    "https://shop.example/shipping?step=1&ref=a?b",
  ]);
  assert.deepEqual(
    JSON.parse(compiled?.stdin ?? "[]"),
    compiled?.steps?.map((step) => step.args),
  );
});
...
test("compileAgentBrowserJob rejects unsupported fields for every constrained job action", () => {
  const invalidSteps = [
    [
      { action: "open", url: "https://example.test/", path: "ignored.png" },
      /job step open does not support path/,
    ],
    [
      { action: "click", selector: "#submit", text: "ignored" },
      /job step click does not support text/,
    ],
    [
      { action: "fill", selector: "#email", text: "user@example.test", values: ["ignored"] },
      /job step fill does not support values/,
    ],
    [
      { action: "type", selector: "#prompt", text: "go", url: "https://example.test/" },
      /job step type does not support url/,
    ],
    [
      { action: "select", selector: "#theme", values: ["dark"], text: "ignored" },
      /job step select does not support text/,
    ],
    [
      { action: "wait", milliseconds: 250, selector: "#spinner" },
      /job step wait does not support selector/,
    ],
    [
      { action: "assertText", text: "Welcome", url: "https://example.test/" },
      /job step assertText does not support url/,
    ],
    [
      { action: "assertUrl", url: "**/dashboard", text: "Welcome" },
      /job step assertUrl does not support text/,
    ],
    [
      { action: "waitForDownload", path: "report.csv", url: "https://example.test/report.csv" },
      /job step waitForDownload does not support url/,
    ],
    [{ action: "snapshot", selector: "body" }, /job step snapshot does not support selector/],
    [
      { action: "screenshot", path: "job.png", url: "https://example.test/" },
      /job step screenshot does not support url/,
    ],
  ] as const;
  ...
  for (const [step, expectedError] of invalidSteps) {
    ...
    assert.equal(
      result.compiled,
      undefined,
      `unexpected compile success for ${JSON.stringify(step)}`,
    );
  ...
  assert.deepEqual(
    validJob.compiled?.steps.map((step) => step.action),
    [
      "open",
      "wait",
      "click",
...
test("agentBrowserExtension compiles semantic actions to upstream find commands", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension resolves semantic role fills through one exact current editable ref", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension warns when contenteditable fill does not replace existing text", {
  concurrency: false,
}, async () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(
        invocations.some(
          (entry) =>
            entry.args.includes("get") && entry.args.includes("text") && entry.args.includes("@e1"),
        ),
      );
    });
  } finally {
...
test("agentBrowserExtension resolves semantic role clicks through current visible snapshot refs when available", {
  concurrency: false,
}, async () => {
...
if (args.includes("open")) {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.ok(invocations.some((entry) => entry.args.includes("snapshot")));
      assert.ok(
        invocations.some((entry) => entry.args.at(-2) === "click" && entry.args.at(-1) === "@e17"),
      );
      assert.equal(
        invocations.some((entry) => entry.args.includes("find")),
        false,
...
test("agentBrowserExtension resolves semantic locator select through current visible combobox refs", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension resolves semantic role fills after stored snapshot misses", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension compiles constrained jobs to upstream batch commands", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      assert.match(
        effectiveArgs?.[2] ?? "",
        process.platform === "android" ? /^piab-[a-f0-9]{20}$/ : /^piab-pi-agent-browser-job-/,
      );
      assert.equal(effectiveArgs?.[3], "batch");
      assert.equal(effectiveArgs?.[4], "--bail");
      const compiledJob = result.details?.compiledJob as
        | {
            args?: string[];
            failFast?: boolean;
            stdin?: string;
            steps?: Array<{ action: string; args: string[]; generatedFrom?: string }>;
          }
        | undefined;
      ...
      assert.match(
        ...
        /Step 4-8 — type #prompt \(succeeded\)\nTyped 2 chars with delayMs=20\.\nPressed Enter\./,
      );
      ...
      const redactedResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        job: {
          steps: [
            {
              action: "open",
              url: "https://user:secret@example.test/path?token=abc&ok=1#access_token=xyz",
            },
          ],
        },
      });
      const redactedCompiledJob = redactedResult.details?.compiledJob as
        | { stdin?: string; steps?: Array<{ args: string[] }> }
        | undefined;
      ...
      assert.deepEqual(
        JSON.parse(redactedCompiledJob?.stdin ?? "[]"),
        redactedCompiledJob?.steps?.map((step) => step.args),
      );
      ...
      assert.deepEqual(
        upstreamSteps.slice(0, 15),
        compiledJob?.steps?.slice(0, 15).map((step) => step.args),
      );
      ...
      const invalidTypeResult = await executeRegisteredTool(harness.tool, harness.ctx, {
        job: {
          steps: [{ action: "type", selector: "#prompt", text: "go", url: "https://example.test" }],
        },
      });
      ...
      assert.match(
        longDelayedTypeResult.content[0]?.text ?? "",
        /delayMs supports at most 200 characters/,
      );
    });
  } finally {
    await rm(tempDir, { force: true, recursive: true });
...
test("agentBrowserExtension reports failed fresh jobs as post-launch failures", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension compiles lightweight QA presets and fails diagnostics", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      await runExtensionEvent(harness.handlers, "session_start", { reason: "new" }, harness.ctx);
      assert.equal(
        Check(harness.tool.parameters, { qa: { attached: true, expectedText: "Welcome" } }),
        true,
      );
      assert.equal(
        Check(harness.tool.parameters, { qa: { attached: true, url: "https://example.test/" } }),
        false,
      );
      ...
      assert.deepEqual(
        (cleanResult.details?.qaPreset as { failedChecks?: string[] } | undefined)?.failedChecks,
        [],
      );
      assert.match(
        (cleanResult.content[0] as { text: string }).text,
        /QA preset passed with warnings/,
      );
      assert.match(
        (cleanResult.content[0] as { text: string }).text,
        /post-clear page error residue/,
      );
      assert.match(
        (cleanResult.content[0] as { text: string }).text,
      ...
      const batchSteps = cleanResult.details?.batchSteps;
      assert.ok(Array.isArray(batchSteps) && batchSteps.length > 0);
      ...
      assert.deepEqual(
        (benignNetworkResult.details?.qaPreset as { warnings?: string[] } | undefined)?.warnings,
        [
      ...
      assert.match(
        ...
        /QA preset passed with warnings: 1 benign network request failure\(s\) ignored; 1 post-clear page error residue row\(s\) ignored as unchanged\./,
      );
      assert.match(
        (benignNetworkResult.content[0] as { text: string }).text,
        /Full diagnostic matrix: see details\.qaPreset and details\.batchSteps\./,
      ...
      assert.equal(
        (missingQaScreenshotResult.details?.qaPreset as { passed?: boolean } | undefined)?.passed,
        true,
      );
      assert.match(
      ...
      const [realPiFailurePatch] = await runExtensionEventResults<{
      ...
      assert.match(
        realPiFailurePatch?.content?.[0]?.text ?? "",
      ...
      const [jsonFailurePatch] = await runExtensionEventResults<{
        content?: Array<{ text?: string; type: string }>;
        isError?: boolean;
      }>(harness.handlers, "tool_result", {
      ...
      const [proseJsonArgsFailurePatch] = await runExtensionEventResults<{
        content?: Array<{ text?: string; type: string }>;
        isError?: boolean;
      }>(harness.handlers, "tool_result", {
        content: [
      ...
      assert.match(
        proseJsonArgsFailurePatch?.content?.[0]?.text ?? "",
      ...
      assert.deepEqual(
        ...
        ["1 actionable failed network request(s)", "1 console error message(s)", "1 page error(s)"],
      );
      ...
      assert.equal(
        (attachedResult.details?.qaAttachedTarget as { title?: string; url?: string } | undefined)
          ?.title,
        "QA Page",
      );
      assert.equal(
      ...
      assert.deepEqual(
        (attachedResult.details?.qaPreset as { failedChecks?: string[] } | undefined)?.failedChecks,
        [],
      );
...
test("agentBrowserExtension allows qa.attached preflight when get title fails but get url succeeds", {
  concurrency: false,
}, async () => {
...
test("agentBrowserExtension allows qa.attached for non-http page URLs", {
  concurrency: false,
}, async () => {
