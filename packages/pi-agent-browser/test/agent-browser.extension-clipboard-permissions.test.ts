...
test("agentBrowserExtension redacts denied clipboard write payloads from all result surfaces", {
  concurrency: false,
}, async () => {
...
process.stdin.on("end", () => {
  ...
  try {
    await withPatchedEnv({ PATH: `${tempDir}:${basePath}` }, async () => {
      ...
      const standalone = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["clipboard", "write", "clipboard-secret"],
      });
      ...
      const multiline = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["clipboard", "write", "clipboard-secret\nsecond-secret"],
      });
      ...
      const objectError = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["clipboard", "write", "object-secret"],
      });
      ...
      const shortPayload = await executeRegisteredTool(harness.tool, harness.ctx, {
        args: ["clipboard", "write", "a"],
      });
      ...
      assert.equal(
        invocations.some(
          (entry) => entry.stdin === JSON.stringify([["clipboard", "write", "clipboard-secret"]]),
        ),
        true,
      );
