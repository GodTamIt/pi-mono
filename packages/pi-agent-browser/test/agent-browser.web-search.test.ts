...
async function createFixture() {
  ...
  return {
    ...
    projectConfigPath: join(cwd, ".pi", "config", "pi-agent-browser", "config.json"),
  };
}
...
test("does not register agent_browser_web_search without env or config credential", async () => {
  ...
  await withPatchedEnv(
    ...
    async () => {
      ...
      assert.equal(
        harness
          .getTool("agent_browser")
          ?.promptGuidelines.includes(
            "Prefer agent_browser over direct agent-browser bash commands for browser work.",
          ),
        true,
      );
    },
    ...
    async () => {
      await withTemporaryCwd(fixture.cwd, async () => {
        ...
        await assert.rejects(
          ...
          /agent_browser_web_search is disabled by pi-agent-browser config/,
        );
      });
    },
  );
    ...
    async () => {
      ...
      await withFakeFetch(
        ...
        async () => {
          await assert.rejects(
            () =>
              executeRegisteredTool(tool, harness.ctx, {
                query: "official docs",
  ...
  const braveNormalized = brave.normalizeResponse(
    {
      ...
      web: { results: [{ title: "Brave", url: "https://example.com/brave" }] },
    },
