...
test("buildPromptPolicy and getLatestUserPrompt derive direct agent-browser bash policy from prompt text without globals", () => {
  const prompt = getLatestUserPrompt([
    {
      type: "message",
      message: { role: "assistant", content: [{ type: "text", text: "Not relevant" }] },
    },
    {
      type: "message",
      message: {
...
test("shouldAppendBrowserSystemPrompt only targets clearly browser-oriented prompts", () => {
  assert.equal(
    shouldAppendBrowserSystemPrompt("Open https://example.com and take a snapshot."),
    true,
  );
  assert.equal(
    shouldAppendBrowserSystemPrompt("Do web research and read the live docs for this API."),
    true,
  );
  assert.equal(
    shouldAppendBrowserSystemPrompt("Search online for the current browser automation docs."),
    true,
  );
  assert.equal(shouldAppendBrowserSystemPrompt("Please review browser compatibility docs."), false);
  assert.equal(
    shouldAppendBrowserSystemPrompt(
      "Summarize the article at https://example.com/blog/post for the changelog.",
    ),
    false,
  );
  assert.equal(
    shouldAppendBrowserSystemPrompt("Please review the repository architecture."),
    false,
  );
});
